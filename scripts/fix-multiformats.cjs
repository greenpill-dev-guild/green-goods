...
function createMultiformatsBasicsShim() {
  ...
  if (!fs.existsSync(multiformatsPath)) {
    console.log('⏭️  Multiformats not found, skipping fix (might be in CI cache)');
    return;
  }
...
`;
  ...
  console.log('✅ Created multiformats/basics.js compatibility shim');
    ...
    try {
      ...
      if (packageJson.exports && typeof packageJson.exports === 'object') {
        ...
        if (needsPatch) {
          ...
          console.log('✅ Patched multiformats/package.json to add basics exports');
        }
      }
    } catch (err) {
      console.log('⚠️  Could not patch package.json, but basics.js shim was created:', err.message);
    }
  }
}
...
function patchUint8arrays() {
  ...
  if (fs.existsSync(tsBasesPath)) {
    ...
    if (content.includes("from 'multiformats/basics'")) {
      ...
      console.log('✅ Patched uint8arrays/src/util/bases.ts');
    }
  }
  ...
  if (fs.existsSync(jsBasesPath)) {
    ...
    if (content.includes('multiformats/basics')) {
      ...
      console.log('✅ Patched uint8arrays/dist/src/util/bases.js');
    }
  }
}
        ...
        try {
          ...
          if (content.includes('multiformats/basics')) {
            ...
            if (content !== originalContent) {
              ...
              console.log(`✅ Patched ${filePath.replace(walletConnectBase, '@walletconnect')}`);
            }
          }
        } catch (err) {
