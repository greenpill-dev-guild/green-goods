...
| `check-qa-id-ledger.mjs` | `bun run check:qa-id-ledger`, Supply Chain Guardrails, `agent-guidance` validation check | Append-only guard for `scripts/data/qa-test-id-ledger.json`: every Test ID present at the merge-base must survive into the working tree, and no id may repeat — the cross-revision half of the catalog's permanent-address guarantee |
| `check-qa-id-ledger.test.mjs` | `bun run test:review-guardrails` | Fixture tests for removed and duplicated ids, appends, an empty base, malformed ledgers, and a self-run against `HEAD` |
...
| `qa-report.ts` | `bun run qa:report` (root package.json) / `qa-session` close, `/qa-triage --call`, `qa-call-report` routine | Joins a pulled session (`tmp/qa-session/<slug>/qa-state.json`) to the catalog and writes `report.md`: results by priority and by kind in the Linear parent's line shape, the fail/blocked list with attributed notes, coverage gaps, an optional `--previous` baseline delta, standing state, per-tester coverage. `--public` adds `report.public.md` under the `qa:status` projection rule. Never reads the Blob store |
| `qa-report.test.ts` | `bun run test:agent-tools` (root package.json) | Proves session-window scoping, bucket reconciliation, template line parity, the public variant's privacy boundary, baseline delta semantics, and that the CLI names `qa:pull` and never echoes a malformed state file |
