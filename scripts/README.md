...
- `packages/contracts/script/` — Foundry scripts and their Bun CLIs. Commitment Pooling deploys in four ordered steps, each step's output being the next step's input: `deploy/testimony-resolver.ts` + `DeployTestimonyResolver.s.sol` (testimony resolver proxy; owner is read off the sibling resolvers, not `guardian`, so one sender can configure all three), `deploy/commitment-schemas.ts` + `DeployCommitmentSchemas.s.sol` (resumable assessment-v3 / community-testimony EAS registration), `deploy/pooling.ts` + `DeployPooling.s.sol` (module + register, deployed paused), and `deploy/pooling-configure.ts` + `ConfigurePooling.s.sol` (the five resolver calls that make the module live — without the work-approval bridge it is inert). Shared logic: `utils/pooling-release.ts` (deterministic schema UIDs, grouped upgrade keys, configuration planning, live `owner()` preflight) and `lib/PoolingConfiguration.sol` (the ordered, re-runnable configure sequence, driven by both the deploy script and the fork rehearsal). The release rehearsal is `test/fork/ArbitrumCommitmentPooling.t.sol` on an Arbitrum One fork, not a testnet; callers are the root `contracts:pooling:*` scripts. The settlement lane's transport check is `test/fork/CrossChainSettlementLane.t.sol` via `contracts:settlement:verify-lane` — read-only proof that the Arbitrum One ↔ Celo Mainnet CCIP lane is live, priced, and matches `deployments/networks.json`; no broadcast, no funds
