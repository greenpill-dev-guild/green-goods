...
Smart contract security auditing: vulnerability detection, access control patterns, static analysis, and pre-deployment security review.

**References**: See `CLAUDE.md` for deployment patterns. See `contracts` skill for development patterns.
...
| Trigger | Action |
|---------|--------|
| "security audit", "security review" | Full contract security review |
| "check for vulnerabilities" | Targeted vulnerability scan |
| "access control review" | Hats Protocol access control audit |
| Pre-mainnet deployment | Mandatory security checklist |
...
- `contracts` — Shows correct implementation patterns; this skill covers detection and audit. When fixing vulnerabilities, use `contracts` patterns; when reviewing for vulnerabilities, use this skill
- `deployment` — Pre-mainnet deployment checklist
- `migration` — Upgrade safety across packages (UUPS storage layout changes)
- `monitoring` — Post-deployment security monitoring and anomaly detection
- `testing` — Fuzz and invariant testing patterns that complement static analysis
