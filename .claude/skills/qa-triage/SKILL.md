...
2. **Resolve Linear handles by name** at the start of every run:
   ...
   - Label families: `protocol:green-goods`, `package:*`, `activity:qa`, `activity:maintenance`, `task:*`, `source:drive`, `source:qa-triage-pulse`, `agent:claude`, `agent:routine`. The per-week label `qa-sync:YYYY-MM-DD` is resolve-or-created on each run that needs it.
...
3a. **Sweep orphan Codex worktrees** from prior failed runs:
   ...
   for wt in /tmp/gg-codex-qa-*; do
     ...
     git branch -D "codex/qa-triage/$slug" 2>/dev/null
   done
   ```

   Phase 2 dispatches `$CODEX exec --full-auto` into a worktree that gets cleaned up in Phase 7. If a prior run died (kill -9, crash, OS reboot), the worktree and branch persist and the next run's `git worktree add` collides. This sweep is defensive — silent on success, surfaces the slugs cleaned in `report.md`.
...
0. **Resume from `qa-triage-pulse` pre-stage** — if invoked as `/qa-triage qa-sync:<YYYY-MM-DD>`, OR if no explicit path/slug was given but Linear has ≥1 open Customer Need carrying both `source:qa-triage-pulse` and a `qa-sync:<latest-date>` label, surface the resume prompt:
   ...
   On `resume`: populate `extraction.md` from the Customer Need bodies (one item per Need, the `## Need statement` and `## PostHog evidence` blocks already filled), skip Phases 2 and 3 (Codex dispatch + PostHog cross-ref already done by the routine — its safe-summary fields are intact in each Need's body), and jump straight to Phase 4 with the pre-staged set as the numbered triage list. On `fresh`: ignore the pre-stage and continue to step 1 below; the Phase 6 confirm step will detect duplicates against the existing Customer Needs.
...
2. Write `extraction.md`:
   ...
   Item types: `bug` → eligible for Linear Issue. `idea` / `feedback` → Customer Need only by default.
...
3. **Dispatch Codex automatically (required unless `--no-codex` OR `--fixture` is set).** The assistant MUST fire the worktree dispatch on every real run that doesn't carry the `--no-codex` flag — no judgment override, no "skipped to keep the flow tight". The parallel extraction pass exists specifically to catch what a single-agent extraction misses. Skipping defeats the dual-extraction design. **`--fixture` mode is exempt**: the fixture is synthetic and trivially verifiable against the source markdown, so the parallel pass adds no signal but burns ~30s + a worktree per dry-run. Pattern source: the user-level memory `feedback_claude_orchestrated_codex.md` (under `~/.claude/projects/-Users-afo-Code-greenpill-green-goods/memory/`, outside the repo):
   ...
   Cleanup at Phase 7: `git worktree remove --force "$WORKTREE" && git branch -D "$BRANCH"`. Skip cleanup on `--dry-run` so the worktree can be inspected.
...
> Which items should I file? Reply with numbers (e.g., `1, 3, 5`), `all`, or `none`. For each, append a tag if you want to override: `1:need-only` (Customer Need only, no Issue), `3:defer` (skip this run), `5:duplicate-of-PRD-1234`. Anything outside the listed numbers is out of scope for this run.
...
- Approved as Customer Need only: [2, 7]
...
3. **Customer Needs cannot be standalone.** Linear's API requires `Exactly one of projectId or issueId must be defined`. Every Customer Need this skill creates must link to an Issue via the `issue` parameter. There is no "Need-only" disposition.

### Disposition rules (no Need-only path)
...
| Idea / feedback / UX polish / strategic gap | **Lightweight attach-Issue** — `activity:maintenance` (or `activity:architecture` for strategic items) + `Backlog` + priority Low/Medium | Yes, linked via `issue` |
...
Title shape for attach-Issues: action-verb-led (e.g., "Bring back public-site Positions UI", not "Positions UI missing"). Body: shorter than a bug Issue — Summary + Surface + Suggested fix + Source.
...
4. On `confirm`:
   ...
   **Linear writes** (via Linear MCP):
   ...
   - Lightweight attach-Issues (for former Need-only items) are created in the same pass as the main Issues, before the Customer Needs that reference them.
...
Then run Codex worktree cleanup if dispatched (unless `--dry-run`).
...
- [`qa-triage-pulse`](../../docs/routines/qa-triage-pulse.md) — cron'd async sibling routine that pre-stages Customer Needs every Wednesday after the 10am PST Product Sync. The skill's Phase 1 step 0 resumes from those pre-stages when present, cutting interactive triage time to ~5 minutes.
- [`bug-intake`](../../docs/routines/bug-intake.md) — cron'd async sibling routine for Discord + Telegram + Drive bug-source intake (M/W/F). Shares the Linear protocol and privacy boundary. This skill is the interactive single-source counterpart for QA-sync notes specifically.
