...
| 3 | Shape | Fixed (badges), Capsule (icon button / chip), Concentric (`child_radius = parent_radius − padding`). Client buttons ride the shared `Button` (`emphasis`), `IconButton`, and `Chip`: one button corner per surface for every emphasis, 16px in the app and square on the website (DL-026, DL-028); fields 16px; heights 48 / 44 / 40 / 32 (DL-022, DL-023, Rule 19). Admin carve-out: fixed 4/8/12/16/9999 scale — the FAB is a capsule at both sizes (DL-010); `AdminButton` stays pill on the 28/32/40 compact metric (DL-011; DL-029: fields 44 on touch widths and 40 from 640px, pills 36); filled-underline is the default field anatomy. | [language.md § Shape System](./language.md) |
