...
| 2.5 | **Rounded corners scale?** Larger elements have larger radii; buttons and fields follow emphasis | | Client runtime scale (not stock Tailwind): tag `rounded-md` (8px), inner content `var(--radius-squircle)` (12px), card and field `rounded-lg` (16px), panel and sheet `rounded-xl` (20px), modal `rounded-2xl` (24px). Buttons: primary capsule, secondary and tertiary 12px, icon circle, chip capsule (DL-021) — never a radius class on a button or field. Admin: badge 8, card 12, dialog/pane 16, pill 9999 — no 24px step |
