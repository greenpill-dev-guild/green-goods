...
Before ANY work, check for a `bundle_id` in the task brief. If present, load the matching bundle from `.claude/registry/skills.json` (under the `bundles` key), including its `quality_contract`. Without a bundle, read `.claude/context/values.md#implementation-quality-contract` directly.
...
4. Apply the Implementation Quality Contract, then remove speculative abstractions, mixed abstraction levels, and narrating comments in a final simplification pass
...
- Import only from paths declared in `packages/shared/package.json#exports` — never `@green-goods/shared/src/**` or undeclared internals
...
Then surface it through an existing declared package export (root or `@green-goods/shared/hooks`, whichever matches the local boundary):
