...
- Envio v3 handlers use `indexer.onEvent`, tests use `createTestIndexer`, and dynamic contracts use `indexer.contractRegister`.
- Do not restore generated-v2 imports, `MockDb`, ReScript setup, package-local Envio skill copies, or package-local pnpm workflows.
- Validate schema or config changes with `cd packages/indexer && bun run codegen && bun run build`.
