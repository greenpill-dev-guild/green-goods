...
- Validate with `cd packages/agent && bun run test && bun run typecheck`; for security-sensitive or handler-heavy work, also run `cd packages/agent && bun run test --scope unit --coverage`.
