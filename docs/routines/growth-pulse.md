...
1. **Dedupe** against existing open Linear Customer Needs/Issues on the Product team filtered by `protocol:green-goods` + `activity:build`. Match by `## Anomaly type` + affected scope. If a duplicate exists, **append a comment** with the new numbers and refresh the date — do not create a parallel Issue.
