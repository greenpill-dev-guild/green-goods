...
**Refresh refs first.** Every later step reads `origin/main`, `origin/develop`, and the release tags, and a cloud checkout may be shallow or hold only one branch. If `git rev-parse --is-shallow-repository` prints `true`, run `git fetch --unshallow origin`. Then run `git fetch origin '+refs/heads/main:refs/remotes/origin/main' '+refs/heads/develop:refs/remotes/origin/develop' '+refs/tags/*:refs/tags/*'`; the forced tag refspec updates moved tags. Never add `--prune` to this fetch: with a tag refspec it deletes every local-only tag, which would destroy backup tags if this spec ever runs in a developer checkout. If either fetch fails, stop and log the failure; never brief from stale refs or partial history.

**Cross-run state lives in the channel.** Every post ends with a marker line (Phase 7), and each run reads the markers back before deciding: `GET https://discord.com/api/v10/channels/${DISCORD_ENGINEERING_CHANNEL_ID}/messages?limit=100` with the header `Authorization: Bot ${DISCORD_BOT_TOKEN}`, paging back with `before=<oldest message id>` until a page comes back empty or older than 30 days, and keeping only messages with a line that starts `-# release-prep ·`. Other routines post with the same bot, so match the marker, never the author. The newest `brief 2/2` marker for the current version is "the last posted brief" below: its fields give the target date, gate verdict, and `develop` SHA it saw, and the message timestamp gives when it posted. A `gate-check` marker for the current version and target date means the gate check already posted. If the channel cannot be read, post nothing unless this is a manual run, and log the failure.
...
- **Version**: take `X.Y.Z` from the release project's name. The runbook sets the bump type — minor for the monthly release, patch for a hotfix, major for a breaking change. Check the name against the latest release tag on the remote, counting only exact `vX.Y.Z` tags, so a tag deleted upstream can never count (`git ls-remote --tags --refs origin 'refs/tags/v*' | sed 's#.*refs/tags/##' | grep -E '^v[0-9]+\.[0-9]+\.[0-9]+$' | sort -V | tail -1`): the version must be the next patch, minor, or major above it, and if it is not, flag it as a decision. Without a release project, fall back to the next minor above the latest tag. Never read the version from `package.json`: `develop` lags the tags whenever a hotfix has not been merged back.
...
- **Size budget**: every message must fit Discord's 2,000-character limit, marker line included; check them all before posting any. When message 1 or the QA gate check would not fit, shorten its open list first (fewer rows, then `+N more` with the release project link), and keep message 1's other sections in the order above. When message 2 would not fit, cut release-note highlights, lowest-impact first, and keep the gardener announcement whole. Never drop the verdict, the target date, or the release project link.
- **Marker line**: end every message (each of the brief's two messages, the gate check, a delta note) with one line of Discord subtext that Phase 0 reads back, and never reformat it: `-# release-prep · v{X.Y.Z} · {brief 1/2|brief 2/2|gate-check|delta} · target {YYYY-MM-DD or none} · gate {clear|not-ready|undefined} · develop {short SHA}`. A brief counts as posted only once its `brief 2/2` message lands.
...
3. Paste a bootstrap prompt, not a copy of this spec. The routine fetches `origin/develop` and executes this file on every run, so a change takes effect as soon as it merges to `develop`. Keep the bootstrap free of phase summaries: a summary drifts from the spec and competes with it.
   ...
   1. Run `git fetch origin develop`. If it fails, stop, report the failure in the run log, and exit non-zero: an older `origin/develop` may still be readable, and running it would run a stale spec.
