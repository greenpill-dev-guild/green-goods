...
| `activity:*` | Routines apply: `activity:build` (confirmed bugs and behavioural defects, including anomaly-derived code fixes), `activity:maintenance` (operational upkeep, polish, cleanup, or anything that isn't a user-visible defect). `activity:qa` is **not** routine-applied — it marks the validation pass itself (QA sweeps, session parents, acceptance passes). The full taxonomy also includes `activity:research`, `activity:architecture`, `activity:design` — human-applied. |
