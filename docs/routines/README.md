...
| `release-prep` | GitHub (read-only), Linear (read-only) | GitHub = open PRs + commit ranges (`main..develop`, plus hotfixes on `main` not yet merged back) + existing releases/tags · Linear = the active release project: its name sets the version, its targetDate drives the self-gating window (brief posts at target − 3 days), and its `QA complete` milestone is the release gate. No Drive/PostHog — a pure readiness draft; reads the release runbook from `develop`. |
...
| `package:*` | `package:pwa` (installed app), `package:editorial` (public website), `package:client` (plumbing both client surfaces share), `package:admin`, `package:shared`, `package:contracts`, `package:indexer`, `package:agent`, `package:docs` | Affected code surface, keyed to the repo (replaces old `area:*`). Orthogonal to `protocol:*` (the product) — one package can serve several products. Apply only to code-touching work; omit if the surface is unknown or the issue isn't code (research / funding / ops). |
| `activity:*` | Routines apply: `activity:build` (confirmed bugs and behavioural defects, including anomaly-derived code fixes), `activity:maintenance` (operational upkeep, polish, cleanup, or anything that isn't a user-visible defect). `activity:qa` is **not** routine-applied — it marks the validation pass itself (QA sweeps, session parents, acceptance passes), and only the `qa-call-report` session parent carries it. The full taxonomy also includes `activity:research`, `activity:architecture`, `activity:design` — human-applied. |
...
- `qa-triage-pulse` writes **Customer Needs only** (pre-stage from Wednesday Build Sync notes, label `source:qa-triage-pulse` + `session:<YYYY-MM-DD>`). Never creates Issues. The interactive `/qa-triage` skill promotes them to Issues with human judgment in the loop.
