# Design System Creative Briefs

**Date:** 2026-04-15
**Status:** Spec
**Scope:** Root DESIGN.md, packages/client/DESIGN.md, packages/admin/DESIGN.md, COPY_PROMPT.md
...
**Workflow enabled:** Explore in Stitch/Figma AI → iterate on variants → export → polish in Claude/Antigravity → implement against existing component system.
...
```
DESIGN.md                        ← Shared foundation (empathy, color hierarchy, typography,
                                   shape, motion, material, regenerative constraints, dos/don'ts)
packages/client/DESIGN.md        ← Client: browser editorial + PWA field tool
packages/admin/DESIGN.md         ← Admin: operator cockpit, M3 strict
COPY_PROMPT.md                   ← Voice/tone + per-surface copy patterns
```

### Usage Pattern

```bash
# Exploring a client browser screen in Stitch or any AI tool:
cat DESIGN.md packages/client/DESIGN.md | pbcopy

# Exploring an admin screen:
cat DESIGN.md packages/admin/DESIGN.md | pbcopy

# Getting copy for any surface:
cat COPY_PROMPT.md | pbcopy
```

### Relationship to Existing System

These files are a **lossy export** of the design skill ecosystem — they translate deep internal design philosophy into a format external AI tools can build from. They do not replace:

- `skills/design/*` — Still the canonical design philosophy for Claude
- `packages/shared/src/styles/theme.css` — Still the canonical token implementation
- `packages/admin/src/styles/admin-m3-tokens.css` — Still the canonical M3 overrides

The DESIGN.md files use **Stitch-compatible color vocabulary** (neutral/primary/secondary/tertiary = canvas/ink/stone/accent). The codebase uses its own naming (`--color-primary` = green). Neither changes — the DESIGN.md translates between them.

| Stitch Role | Job | Internal Token | Color |
|-------------|-----|---------------|-------|
| Neutral | Canvas (80-90%) | `--bg-white-0`, `--bg-weak-50` | Warm linen white (direction: #FAF8F5) |
| Primary | Ink (8-15%) | `--text-strong-950` | Warm charcoal rgb(12,10,9) |
| Secondary | Subdued support (3-5%) | `--text-sub-600` | Earth stone rgb(87,83,78) |
| Tertiary | Accent/CTA (1-3%) | `--color-primary` / `--green-500` | Garden green #1FC16B |

**Warm canvas note:** The proposed #FAF8F5 is a creative direction — warmer than the current neutral-50 (250,250,249). Implementation requires updating `--bg-white-0` and `--bg-weak-50` in theme.css as a separate step. The existing Stone neutral scale already has warm undertones, making this a natural evolution.

## Deliverable 1: Root DESIGN.md
...
**Key aesthetic words** (for AI tool prompts):
...
**State colors:** Information (blue), Warning (orange/amber), Error (red), Success (green — shares with tertiary in success contexts).

### Typography

| Role | Font | Weight | Use |
|------|------|--------|-----|
| **Editorial headlines** | Serif display (Fraunces, Lora, or Newsreader) | 600-700 | Browser site only — garden/impact pages |
...
**Rationale:** The serif headline only appears on the public browser site where editorial weight matters — the coffee-table-book chapter heading. PWA and admin stay utilitarian with their respective sans-serif workhorses.
...
| **Capsule** | Radius = half container height (`rounded-full`) | Primary buttons, FABs, icon buttons |
...
- `rounded-full` — Primary buttons, icon buttons (capsule)
...
**Two motion schemes:**
- **Standard** — Efficient, minimal overshoot. Default for operator cockpit and data-dense views.
- **Expressive** — Bouncy, delightful, +50% duration. Activates for hero moments only.
...
**Canvas recession:** When sheets open, the main content recedes: `scale(0.97) + opacity(0.85) + blur(2px)`. No dark scrims — depth through transform + blur.
...
- Make value flows visible end-to-end (show the mycelium)
- Match complexity to garden maturity (design for succession)
...
- Use countdown timers, leaderboards, or streak mechanics (degen patterns)
...
## Deliverable 2: Client DESIGN.md (packages/client/DESIGN.md)
...
|------|-----------|-----------|----------|----------|------------|
| **Browser** | Standard browser visit | Funders, Community members | Walking through the garden | Per-route: Ambient Display (editorial pages) / Data Landscape (browse/fund pages) | SiteHeader: hamburger (mobile) + horizontal links (desktop) |
...
## Deliverable 3: Admin DESIGN.md (packages/admin/DESIGN.md)
...
**Why strict:** M3+Liquid Glass hybrid produced inconsistent "AI generic slop" UI. Strict M3 provides discipline; glass limited to where spatial depth cues actually help.
...
Existing tokens (`--ws-primary`, `--ws-on-primary` in admin-m3-tokens.css) support per-workspace color atmosphere:
...
- **Desktop profile**: On desktop, Profile redirects to Hub and opens RightSheet with profile content (no dedicated profile workspace on desktop).
...
- Garden descriptions: place-first. Lead with where and what, not the technology.
...
- Empty states: encouraging. "No work yet. Start by documenting what you see."
- Sync status: calm. "Saved locally" / "Synced" / "Syncing..."
- Errors: never blame the user. "We couldn't submit that" not "Your submission failed."

**Admin (Operators / Evaluators):**
- Utility copy only. Status language and task framing.
...
1. Create `DESIGN.md` at repo root
2. Create `packages/client/DESIGN.md`
3. Create `packages/admin/DESIGN.md`
4. Create `COPY_PROMPT.md` at repo root

### Phase 2: Token alignment (separate task)
1. Evaluate warm canvas direction (#FAF8F5) against existing neutral-50
2. If adopted, update `--bg-white-0` and `--bg-weak-50` in theme.css
3. Verify WCAG contrast ratios with warm canvas
4. Select and load serif display font for client browser mode

### Phase 3: Validation
1. Paste foundation + client DESIGN.md into Stitch — verify output matches direction
2. Paste foundation + admin DESIGN.md into Stitch — verify M3 compliance reads through
3. Test COPY_PROMPT.md with Gemini/Claude copywriting — verify voice consistency
4. Cross-check generated output against existing component library

### Out of Scope
- Changing existing CSS tokens (Phase 2 task)
- Modifying existing design skill files
- Building new components
- Stitch-specific formatting or API integration
