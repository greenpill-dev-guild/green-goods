...
Visual Reference: The Regenerative Stack Architecture Diagram should show these five horizontal layers stacked vertically with bidirectional arrows connecting adjacent layers. The Settlement Layer sits at the bottom in dark blue. Capital Formation Layer above it in green with Octant and Juicebox logos. Data and Verification Layer in teal with EAS and Hypercerts logos. Protocol Layer in light green with Green Goods logo centered. Application Layer at top in white with PWA, WhatsApp, and Dashboard icons. On the right side, show the five user personas with dotted lines to their primary interaction layers.

### 1.5 Competitive Positioning

Green Goods operates at the intersection of four market categories: Impact MRV platforms, Regenerative Finance protocols, Community coordination tools, and AI-powered environmental monitoring.

Comparison with Direct Competitors

Silvi focuses on AI-powered tree verification using satellite imagery and ground-truth photos. Their strengths include deep tree species identification expertise and partnerships with carbon credit certifiers. However, they have narrow focus on trees only, use a centralized platform rather than onchain-native, apply enterprise pricing that excludes grassroots projects, and offer no capital formation layer. Green Goods differentiates through multi-domain coverage where trees are one of five action types, onchain-native output via Hypercerts, integrated capital formation via Octant and Revnets, and community-owned infrastructure through the Fractal Gardens model. Potential partnership exists where Silvi serves as an Evaluator oracle for tree-specific verification.

GainForest uses satellite imagery and AI to monitor forest health and detect deforestation. Their strengths include advanced remote sensing and academic partnerships. However, they require satellite imagery not available for small plots, have high technical barriers for local communities, focus on conservation rather than restoration, and offer limited community governance. Green Goods differentiates through ground-level verification via photos and GPS that works at any scale, accessibility to non-technical users via WhatsApp, coverage of restoration and active land use, and Conviction Voting for community governance power.

Regen Network provides blockchain-based ecological credit creation and trading on Cosmos. Their strengths include deep ecological science foundation and proprietary soil carbon methodology. However, they use Cosmos ecosystem with limited DeFi integration, require scientific expertise to participate, focus on credit creation over community coordination, and have high barriers for smallholder farmers. Green Goods differentiates through Ethereum and Arbitrum native architecture with deep DeFi integration, accessible MDR methodology rather than scientific sampling, focus on capturing all regenerative work not just credit-eligible activities, and community coordination as a core feature.

Toucan and KlimaDAO provide carbon market infrastructure by bridging legacy credits onchain and creating demand through reserve currency. Their strengths include significant liquidity and strong brand recognition. However, they focus on existing legacy credits rather than new impact creation, face quality concerns with bridged credits, have speculative token dynamics that overshadow impact, and offer no community coordination tools. Green Goods differentiates through creating new verified impact rather than bridging existing credits, quality control through multi-layer verification, sustainable capital formation through yield rather than speculation, and focus on the work rather than the financial instrument.

Competitive Positioning Summary

On Accessibility for grassroots projects: Green Goods is High with WhatsApp and offline PWA. Silvi is Medium with enterprise pricing. GainForest is Low requiring satellite data. Regen Network is Low requiring scientific methodology.

On Onchain Integration for DeFi and governance: Green Goods is High being Ethereum native. Silvi is Low with centralized database. GainForest is Medium with limited DeFi. Regen Network is Medium on Cosmos with limited bridge.

On Capital Formation for sustainable funding: Green Goods is High with Octant and Revnets. Silvi is Low as verification only. GainForest is Medium with token incentives. Regen Network is Medium with credit sales.

On Multi-Domain Coverage beyond trees: Green Goods is High covering solar, waste, education, and mutual credit. Silvi is Low covering trees only. GainForest is Low covering forests only. Regen Network is Medium but methodology-heavy.

On Community Governance for local decisions: Green Goods is High with Fractal Gardens and Conviction Voting. Silvi has None. GainForest is Low with token voting only. Regen Network is Low with validator governance.
...
Switching Costs: Once Gardens have verified actions and minted Hypercerts, migrating means abandoning attestation history.

Data Moat: Accumulated action data creates training data for AI Agents and verification models that improves over time.

Community Ownership: Fractal Gardens model means communities have ownership stake and are incentivized to stay and grow.
...
Visual Reference: The Five-Sided Marketplace Diagram should show a pentagon with each persona at a vertex. Funder at top, Evaluator at top-right, Community Member at bottom-right, Gardener at bottom-left, Operator at top-left. Lines connect all vertices showing relationships. Funder to Operator shows deposits capital. Operator to Gardener shows approves work and triggers payments. Gardener to Community Member shows performs beneficial work. Community Member to Evaluator shows flags issues and provides ground truth. Evaluator to Funder shows certifies impact and builds trust. Center shows Green Goods Protocol logo with text Orchestrates trust and value flow.
...
Visual Reference: Action Domain Lifecycle Diagram should show six phases horizontally for each domain. Phase 1 Setup shows clipboard icon for site registration and baseline assessment. Phase 2 Capture shows camera icon for Gardener submissions via PWA or AI Agent. Phase 3 Verification shows checkmark icon for Operator review and approval. Phase 4 Aggregation shows stack icon for bundling into Impact Report. Phase 5 Certification shows stamp icon for Evaluator review and certification. Phase 6 Tokenization shows token icon for Hypercert minting. Below show domain-specific examples flowing through these phases.
...
Validation Success Criteria: Completion Rate target is at least 80% on first attempt. Time to Submit via PWA target is under 5 minutes. Time to Submit via AI Agent target is under 3 minutes. AI Parse Accuracy target is at least 85% of fields correctly extracted. Error Rate target is under 10% requiring Operator correction.

Validation Timeline: Week 1 covers Solar Domain. Week 2 covers Waste Domain. Week 3 covers Agroforestry Domain. Week 4 covers Education Domain. Week 5 covers Mutual Credit Domain. Week 6 covers synthesis and schema revisions.
...
Visual Reference: The Fractal Gardens Model Diagram should show a hub-and-spoke layout with the Protocol Revnet as a large hexagon in the center labeled Green Goods Protocol Revnet with $GG token symbol and contents listing Shared Infrastructure including Servers, Indexers, Legal, and Development in dark green. Surrounding it are 6 smaller hexagons for Garden Revnets labeled Nigeria Garden with $LAGOS, Brasil Garden with $BRAZIL, Cape Town Garden with $CPT, Uganda Garden with $UGA, Thailand Garden with $KPG, and DevConnect Garden with $DEV in light green with unique accent colors. Dashed arrows flow from each Garden toward center labeled 2.5-5% Tribute via JBSplits. Solid arrows flow from center toward Gardens labeled Infrastructure subsidies and token purchases. Bottom section shows three callout boxes for economic mechanics.
...
Visual Reference: Capital Formation Flow Diagram should show a circular flow with 6 nodes. Node 1 Funder deposits ETH into Octant Vault. Node 2 Octant Vault deploys to low-risk DeFi strategies with note Principal preserved, only yield spent. Node 3 Yield Pool accumulates yield for distribution. Node 4 Hypercert Marketplace where protocol purchases verified Hypercerts. Node 5 Garden Treasury receives ETH and backing increases. Node 6 Gardeners can redeem tokens at higher floor price. Arrows connect in a loop with Node 6 flowing back to Node 4 via Verified work. Center callout states The Octant-Backstopped Impact Floor: Each yield cycle raises the minimum price for ecological labor.
...
Visual Reference: AI-Driven Capture Flow Diagram should show horizontal swimlane diagram with four lanes for Gardener, AI Agent, Backend, and Blockchain. Gardener lane shows taking photo and sending to WhatsApp, then receiving confirmation. AI Agent lane shows receiving message, identifying object and extracting metadata, constructing action payload, and formatting confirmation. Backend lane shows validating payload, compressing and uploading to IPFS, and constructing EAS attestation. Blockchain lane shows Smart Account signing via Pimlico, attestation anchored on Arbitrum, and Envio indexer picking up event. Show clock icons between steps indicating target under 5 seconds for AI response.
...
Visual Reference: Impact Report Flow Diagram should show horizontal swimlane diagram with four lanes for Operator, Evaluator, System, and Blockchain. Operator lane shows opening dashboard, filtering and selecting actions, clicking Create Report, receiving certification notification, clicking Mint Hypercert, and seeing Hypercert in treasury. Evaluator lane shows receiving report, reviewing evidence and checking CIDS compliance, and signing certification. System lane shows aggregating metadata, calculating impact metrics, generating preview, constructing Hypercert metadata, and uploading to IPFS. Blockchain lane shows certification attestation anchored and Hypercert minted and registered with Garden Revnet.
...
Visual Reference: Governance Flow Diagram should show funnel diagram with voting inputs narrowing to allocation outputs. Top section shows multiple Community Members with GreenWill badges each with stake amounts flowing into voting pool. Middle section shows time axis with conviction formula Conviction equals Stake times Time and visual of conviction growing over time. Bottom section shows three proposals with conviction bars and percentage allocations with output arrows showing yield flowing proportionally.
...
Conclusion: The Arbitrum grant target of $12,000 TVL is a starting point but not sustainable long-term. The protocol needs to grow to approximately $100,000-$200,000 TVL for self-sustaining operations with meaningful Hypercert purchasing power.
...
Approach C Market-Driven Pricing: Let Gardens list Hypercerts at their desired price. Octant Vault purchases at market clearing price. Price discovery through supply and demand.
...
Tier 2 Languages Planned for v1.5: French West African for Ivory Coast chapter. Swahili for East African expansion including Uganda. Pidgin English for Nigeria which is critical for TAS adoption.
...
Right to Object: Users can opt out of analytics tracking. Users can delete account with limitations noted above.
...
### 16.4 Visual Asset Specifications

This section provides specifications for visual assets that should accompany the PRD. These descriptions can be handed to a designer for execution in Figma, Miro, or similar tools.

The Regenerative Stack Architecture Diagram: Show five horizontal layers stacked vertically. Settlement Layer at bottom in dark blue with Arbitrum and Ethereum logos. Capital Formation Layer above in green with Octant and Juicebox logos and text Yield generation and treasury management. Data and Verification Layer in teal with EAS and Hypercerts logos and text Atomic proof of work and tradable impact certificates. Protocol Layer in light green with Green Goods logo centered and text Coordinates Gardens, validates actions, routes fees. Application Layer at top in white with PWA, WhatsApp, and Dashboard icons and text Gardeners, Operators, and Funders interact here. Bidirectional arrows connect adjacent layers with labels describing value and data flows. On right side show five user personas with dotted lines to their primary interaction layers.

The Fractal Gardens Model Diagram: Hub-and-spoke layout with Protocol Revnet as large hexagon in center labeled Green Goods Protocol Revnet with $GG token and contents Shared Infrastructure in dark green. Six smaller hexagons around perimeter for Garden Revnets with local token symbols in light green with unique accents. Dashed arrows from Gardens to center labeled 2.5-5% Tribute via JBSplits. Solid arrows from center to Gardens labeled Infrastructure subsidies and token purchases. Three callout boxes at bottom for Issuance Price Ceiling, Redemption Price Floor, and Impact Floor Rising.

User Flow Diagrams: Horizontal swimlane format for each of four key flows. AI-Driven Capture shows Gardener, AI Agent, Backend, and Blockchain lanes with clock icons between steps targeting under 5 seconds. Impact Report Minting shows Operator, Evaluator, System, and Blockchain lanes. Capital Formation shows circular flow with 6 nodes for Funder, Vault, Yield Pool, Marketplace, Treasury, and Gardeners. Governance shows funnel from Community Members through conviction accumulation to proposal allocation.

Action Domain Lifecycle Diagram: Six phases horizontally labeled Setup, Capture, Verification, Aggregation, Certification, and Tokenization with icons for each. Domain-specific examples along bottom showing representative actions flowing through phases.

Five-Sided Marketplace Diagram: Pentagon with Funder at top, Evaluator at top-right, Community Member at bottom-right, Gardener at bottom-left, and Operator at top-left. Lines connecting all vertices with relationship labels. Green Goods Protocol logo in center with text Orchestrates trust and value flow.
