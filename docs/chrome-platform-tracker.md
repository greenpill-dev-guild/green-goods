...
| --- | --- | --- | --- | --- | --- |
| `/llms.txt` and agent-readable docs | Client and docs publish `llms.txt`; clean-room browser proof checks `/llms.txt` and route discovery in CI. | Public browser site, docs, and built client/admin route proof. | Stale or overbroad agent context can imply private app or admin access. | `CI=true bun run agentic:browser-proof` | ship |
...
| Same-document View Transitions | Client and admin already ship View Transition CSS/logic with reduced-motion coverage. | Existing route/card/dialog transitions. | Route transitions must not mask wallet, admin, sheet, or onchain state changes. | Existing component tests plus `CI=true bun run agentic:browser-proof` when public clean-room route proof is needed. | ship |
| WebMCP runtime tools | Public-safe v1 tools are registered by `packages/client/src/modules/webmcp/public-tools.ts`; clean-room DevTools MCP is project-configured in `.mcp.json` through `scripts/mcp/brave-devtools.mjs`. | Public route description and approved public route navigation. | Wallet/passkey, admin, vault, attestation, marketplace, onchain, telemetry, or private garden state would be unsafe. Native WebMCP discovery requires a Brave build with `navigator.modelContext`. | `CI=true bun run agentic:browser-proof` for injected model-context registration proof; clean-room DevTools MCP / Puppeteer WebMCP discovery must show only public-safe tools when native WebMCP is available. | ship |
| Clean-room browser proof | CI/clean-room browser proof captures screenshots, accessibility summaries, `/llms.txt`, reduced motion, console/page errors, overflow, and WebMCP discovery; the MCP wrapper launches Brave only with isolated profile, stable viewport, page-id routing, structured output, redacted network headers, localhost HTTPS support, WebMCP debug category, WebMCP testing flags, and external usage/CrUX calls disabled. The upstream package is named `chrome-devtools-mcp`, but Google Chrome, Chrome for Testing, Chromium, and Edge are forbidden Green Goods browser-proof targets. | Public browser, docs, Storybook, and non-authenticated route debugging when clean-room proof is needed. | Real-profile MCP proof can expose authenticated admin/wallet state or browser profile data. Local authenticated QA must use the authenticated Brave QA profile instead. | `CI=true bun run agentic:browser-proof`; use authenticated Brave QA for local sensitive proof. | ship |
...
- WebMCP runtime remains limited to public-safe client tools until the user explicitly approves additional `navigator.modelContext.registerTool`, `toolname`, or `tooldescription` work.
