...
| WebMCP runtime tools | Public-safe v1 tools are registered by `packages/client/src/modules/webmcp/public-tools.ts`; Brave-backed DevTools MCP is project-configured in `.mcp.json` through `scripts/mcp/brave-devtools.mjs`. | Public route description and approved public route navigation. | Wallet/passkey, admin, vault, attestation, marketplace, onchain, telemetry, or private garden state would be unsafe. Native WebMCP discovery requires a Brave build with `navigator.modelContext`. | `bun run agentic:browser-proof` for injected model-context registration proof; Brave-backed DevTools MCP / Puppeteer WebMCP discovery must show only public-safe tools when native WebMCP is available. | ship |
| Brave DevTools MCP proof | Browser proof captures screenshots, accessibility summaries, `/llms.txt`, reduced motion, console/page errors, overflow, and WebMCP discovery; the MCP wrapper launches Brave only with isolated profile, stable viewport, page-id routing, structured output, redacted network headers, localhost HTTPS support, WebMCP debug category, WebMCP testing flags, and external usage/CrUX calls disabled. The upstream package is named `chrome-devtools-mcp`, but Google Chrome, Chrome for Testing, Chromium, and Edge are forbidden Green Goods browser-proof targets. | Public browser, admin, docs, and Storybook debugging when route proof is needed. | Real-profile MCP proof can expose authenticated admin/wallet state or browser profile data. | `bun run agentic:browser-proof`; use isolated/non-default Brave profiles for sensitive proof. | ship |
