...
Client and admin source maps are handled by GitHub Actions, not Vercel. The `client.yml` and `admin.yml` workflows upload source maps to the app-specific PostHog environment on trusted `main` pushes. Vercel runs plain package builds only, and Vite emits source maps only when `GG_ENABLE_SOURCEMAPS=true` is set by the upload workflow.
