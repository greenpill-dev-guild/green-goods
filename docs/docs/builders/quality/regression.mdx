...
When a contract interface changes, the `build:abis` script regenerates the committed ABI JSON files in `packages/contracts/abis/` that the shared package consumes. Forgetting this step causes type errors in downstream hook tests -- a common regression vector.
