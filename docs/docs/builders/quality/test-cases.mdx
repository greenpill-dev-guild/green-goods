...
Product-experience test cases now have a canonical checked-in source: `scripts/data/qa-test-catalog.json` (see the admonition below). For behavior-level coverage, the source of truth remains the package test scripts, GitHub Actions workflows, Vitest configs, contracts test scripts, Playwright config, and test-quality guardrail. Use this page as an orientation map, not as proof that every listed workflow has full automated coverage.
...
:::info Canonical catalog
Product-experience test cases are now maintained as a checked-in catalog:
`scripts/data/qa-test-catalog.json` (v2) versions the scenario definitions across Public
Website, PWA, Admin Dashboard, and Docs — the installed-PWA iOS/Android tabs merged into one
PWA surface and Cross Surface dissolved into per-surface rows (the catalog itself is
the count of record), and
`bun run qa:workbook` generates per-run Excel workbooks from it. See
[Product Experience QA](/builders/quality/product-experience-qa) for the journey matrices and
the QA session process. The behavior-level categories below still need links to concrete tests,
workflows, or acceptance artifacts before being treated as enforced coverage.
:::
...
7. Steward reviews and submits WorkApproval attestation
8. WorkApprovalResolver validates: steward role, work reference, confidence/method
...
- **Role verification**: hat-based role checks for gardener, steward, evaluator actions
...
Current test infrastructure is operational, and product-experience scenarios are tracked in the checked-in catalog (`scripts/data/qa-test-catalog.json`). Known gaps:

- The behavior-level categories below are not yet linked to that catalog or generated from it.
