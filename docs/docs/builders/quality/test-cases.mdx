...
| `ADM-006` | Transaction | Certify | Certification flow does not overpromise transaction state | Screenshot |
...
| `XPLAT-007` | Data integrity | Hub Work Review | Two stewards acting on the same submission converge on one state | Screenshots from both sessions |
