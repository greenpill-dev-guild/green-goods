...
Green Goods is an **offline-first, single-chain** platform for documenting regenerative work on-chain. The system is built as a Bun monorepo with seven documented package surfaces that communicate through clearly defined boundaries.
...
| `packages/ops` | Authenticated local operations service for deploy/upgrade/script jobs |
