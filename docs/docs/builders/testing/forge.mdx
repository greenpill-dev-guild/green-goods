...
| `test` | `true` | Unit tests; separate `.generated/foundry/out/test` artifacts avoid stale cache |
...
- **Invariant tests** (`test/invariant/`) -- stateful accounting and role-hierarchy properties
- **E2E scripts** (`script/utils/run-e2e.sh`) -- named protocol workflows through package scripts
...
The adaptive build system (`bun run build`) defaults to fast mode (~2s cached) and only triggers a full build when necessary. For CI and deployments, use `bun run build:full` which compiles with `--skip test --skip script`.

```bash
bun run build             # Adaptive (fast by default)
bun run build:fast        # Force fast mode
bun run build:full        # Full build (CI/deploy)
bun run build:target -- src/registries/Action.sol  # Single target
```

### CI Integration

The `contracts.yml` workflow runs on pushes and PRs touching `packages/contracts/`, root dependency/config files, and contract audit scripts. It installs Foundry via `foundry-rs/foundry-toolchain@v1`, runs `bun run test`, lint, build, and the realism audit for PR CI. The fork-readiness job is limited to trusted pushes or manual dispatch because it requires private RPC secrets.
