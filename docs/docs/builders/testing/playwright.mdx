...
The `bun run test:e2e` wrapper starts `bun dev`, waits for the client and admin surfaces to become reachable, then runs `npx playwright test` with `SKIP_WEBSERVER=true` and `SKIP_HEALTH_CHECK=true`.
