...
graph TB
    ...
    subgraph Servers["Selected Web Servers"]
        S1["Indexer :3006\nonly when required"]
        S2["Client :3001\nPLAYWRIGHT_APP=client"]
        S3["Admin :3002\nPLAYWRIGHT_APP=admin"]
    end
...
Playwright config starts the indexer on port 3006 only when the selected specs need it;
`SKIP_INDEXER=true` disables that server. App servers are selected with `PLAYWRIGHT_APP`:
`PLAYWRIGHT_APP=client` starts the client on 3001, `PLAYWRIGHT_APP=admin` starts the admin on 3002,
and an unset value starts both apps for broader local runs. `SKIP_WEBSERVER=true` reuses externally
managed services.
...
The `bun run test:e2e` wrapper starts `bun run dev:web`, waits for the client and admin surfaces to
become reachable, then launches the checked-in Playwright CLI under system Node with
`SKIP_WEBSERVER=true` and `SKIP_HEALTH_CHECK=true`.
...
- **Timeouts**: 30s navigation, 15s actions; fork and testnet projects get 120s
