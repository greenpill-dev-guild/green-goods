...
graph TB
    ...
    A11y --> SB
    SB -->|CI| Gates["check:design-tokens\ncheck:stories\ncheck:story-quality\ntest:stories:ci\nbuild-storybook"]
```
...
The public component library deploys with the docs site at `/storybook/` on GitHub Pages. The deployed build uses the public browser dialect as its Storybook chrome and exposes a small design-tool import bundle under `/storybook/design/`.
...
Storybook is now a light regression gate. The repo CI verifies the design-token contract, required Storybook coverage for shared foundations plus curated admin surfaces and client dialect shell stories, story-quality guardrails for admin, shared Canvas, and curated client PWA/public stories, a small browser-mode Storybook test lane for `storybook-ci` stories, then builds the unified Storybook to catch compile-time issues. Keep the CI tag curated; do not tag every story by default.
...
The bundle is served at `/design/` inside Storybook. In the GitHub Pages deploy, that means:

- `/storybook/design/DESIGN.md` -- canonical Warm Earth DesignMD source
- `/storybook/design/DESIGN.browser.md` -- preferred public-browser dialect for the component library
- `/storybook/design/design-md.generated.json` -- machine-readable DesignMD tokens
- `/storybook/design/theme.css` -- runtime CSS token projection
- `/storybook/design/storybook-design-manifest.json` -- machine-readable map across design files, story roots, and import surfaces
- `/storybook/index.json` -- Storybook component and state catalog
...
The `storybook.yml` workflow triggers on PRs that modify Storybook config, story files, the curated story coverage contract, and component or view files that feed the unified Storybook build. It runs five required checks:

1. **Design-token contract** (`check:design-generated`, `check:design-tokens`) -- Confirms root DesignMD artifacts are current and the shared theme runtime projection still exposes the expected Warm Earth tokens
2. **Storybook contract** (`check:stories`) -- Ensures shared foundations, curated admin surfaces, and required client PWA/public shell stories have corresponding stories
3. **Story quality** (`check:story-quality`) -- Catches nondeterministic fixtures, unmarked mock harnesses, generic gallery exports, and dark-mode duplicate stories across the protected Storybook surface
4. **Storybook interaction smoke** (`test:stories:ci`) -- Runs browser-mode Vitest against curated `storybook-ci` stories
5. **Build verification** (`build-storybook`) -- Confirms the static Storybook site compiles without errors across shared, admin, and client stories

The built artifact is uploaded via `actions/upload-artifact@v4` for review.

Chromatic publishing is optional. The workflow publishes only when `CHROMATIC_PROJECT_TOKEN` is configured, and uses `--exit-zero-on-changes` so first-pass visual baselines do not block functional Storybook checks.

The GitHub Pages deploy is owned by `deploy-docs.yml`. It builds Docusaurus first, then builds Storybook into `docs/build/storybook`, so the docs site and component library ship as one static Pages artifact.
