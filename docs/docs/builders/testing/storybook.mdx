...
# Check admin/shared Canvas story determinism and harness conventions
...
1. **Design-token contract** (`check:design-tokens`) -- Confirms Warm Earth token expectations match the shared theme implementation
2. **Storybook contract** (`check:stories`) -- Ensures shared foundations plus curated admin surfaces have corresponding stories
3. **Story quality** (`check:story-quality`) -- Catches nondeterministic admin fixtures, unmarked mock harnesses, generic gallery exports, and dark-mode duplicate stories
