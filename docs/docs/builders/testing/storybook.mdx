...
Standalone Storybook publication is owned by Vercel, not GitHub Actions. Configure the Vercel project with the repository root as its root directory; `vercel.json` runs `bun install --frozen-lockfile`, builds with `bun run --filter @green-goods/shared build-storybook`, and serves `packages/shared/storybook-static`. Map the Vercel project to `design.greengoods.app`.
...
The `design.yml` workflow triggers on PRs that modify Storybook config, story files, the curated story coverage contract, and component or view files that feed the unified Storybook build. It runs five required checks:
...
- Storybook deploy config: `vercel.json`
