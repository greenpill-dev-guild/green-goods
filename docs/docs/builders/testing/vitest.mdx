...
```mermaid
graph TB
    CMD["bun run test"] -->|vitest projects| Runner["Vitest Runner\nNode + DOM, isolated threads"]
...
|---------|-------------|------------|-------------|
| **shared** | Node + `jsdom` projects | `src/__tests__/setupTests.ts` | Curated DOM-free Node globs; DOM owns the remainder |
| **admin** | Node + `jsdom` projects | `src/__tests__/setup.ts` | `.ts` tests in Node; `.tsx`/`.jsx` tests in DOM |
| **client** | Node + `jsdom` projects | `src/__tests__/setupTests.ts` | `.ts` tests in Node; `.tsx`/`.jsx` tests in DOM |
| **agent** | `node` | `src/__tests__/setup.ts` | Node-specific mocks |

All packages use:
- **Thread pool** (`pool: "threads"`) for parallel test execution
- **V8 coverage** provider with package-specific thresholds
- **10-second timeout** per test

The `agent` package is the exception: it runs in the `node` environment with `fileParallelism: false`, and its coverage thresholds are intentionally lower than the frontend/shared packages.

Shared, admin, and client keep `isolate: true` and split DOM-free tests into Node projects so pure
module proof does not pay for browser globals. Shared uses a curated Node allowlist because some
`.ts` tests still depend on DOM setup; admin and client route `.ts` to Node and `.tsx`/`.jsx` to DOM.
Project discovery must preserve the intended union exactly once.
...
Do not mock a test's own subject. A direct subject test imports the subject outside every mock
factory through its real relative or declared package specifier, while mocks isolate only its
dependencies. `bun run check:test-quality` enforces this rule and reconciles certified critical
seams with `scripts/data/module-seam-registry.json`.

Keep these proof types separate:

- **Direct test**: exercises the subject's interface without mocking the subject.
- **Conformance test**: proves production and local/test adapters obey the same observable contract.
- **Consumer wiring**: proves a real caller uses the declared interface.
- **Production composition**: proves defaults and runtime identity are assembled at the intended root.
- **Coverage floor**: catches broad execution regressions; it does not certify interface depth,
  adapter fidelity, consumer reachability, or production composition.
...
```bash
# All packages
bun run test                        # Run the root test script across the workspace

# Per package
cd packages/shared && bun run test  # Shared package only
cd packages/shared && bun run test:live  # Shared live RPC checks (explicit opt-in)
...
cd packages/admin && bun run test:watch
cd packages/agent && bun run test:watch

# Specific test file
cd packages/shared && bun run test src/__tests__/hooks/work/useWorkMutation.test.ts

# Coverage report
cd packages/shared && bun run coverage
cd packages/admin && bun run test:coverage
cd packages/client && bun run coverage
cd packages/agent && bun run test:coverage
...
The default shared test command is intentionally hermetic. Live RPC coverage is excluded from `bun run test` and only runs through `bun run test:live`.
