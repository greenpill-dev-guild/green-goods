...
When a steward mints a garden, GardenToken fans out to all configured modules in two phases. Phase 1 handles role setup and governance. Phase 2 handles integration wiring. Each optional module call is wrapped in `try/catch` so failures never block the mint.
...
The core user flow: a gardener submits work, a steward reviews it, and the attestation chain is anchored on EAS. The WorkResolver validates that the gardener has the correct hat and the action is active. The WorkApprovalResolver validates the steward role and optionally triggers a Karma GAP impact attestation.
...
sequenceDiagram
    ...
    Note over G,Op: Later -- steward reviews
...
| Steward role | WorkApprovalResolver | Attester must wear the Steward hat for the target garden. |
...
When gardens receive deposits, the yield accrues in an ERC-4626 vault. A steward triggers harvest, and the YieldSplitter divides the yield three ways.
...
sequenceDiagram
    ...
    participant UI as Admin Dashboard
    participant IPFS as IPFS (Pinata-backed)
    participant EAS as EAS Contract
