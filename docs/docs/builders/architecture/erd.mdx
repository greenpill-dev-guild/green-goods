...
erDiagram
    ...
    WorkApproval {
        ...
        String attester "Steward address"
        Boolean approved
    ...
    Hat {
        ...
        String role "owner | steward | evaluator | gardener | funder | community"
...
| --- | --- | --- |
| **Garden** | Envio (GardenToken events) | A regenerative project with a tokenbound account (ERC-6551 TBA), role tree, and configured domains. Created when a steward calls `mintGarden()`. |
...
| **Hat** | Envio (HatsModule events) | A role assignment from Hats Protocol. Each garden gets 6 roles: owner, steward, evaluator, gardener, funder, community. |
...
| **WorkApproval** | EAS GraphQL | A steward's review of a work submission. References the work UID with an approval boolean, confidence score, and method description. |
