...
| --- | --- | --- |
| Frontend QA | Node.js, Git, Bun from setup, root `.env`, ports 3001/3002/3003 available | `bun run dev:doctor -- --profile web` |
...
Most local web work starts with the generated defaults. Add secrets only for the workflow you are testing.
...
| `*_OP_REF` | Optional 1Password CLI reference for a local secret, such as `PINATA_JWT_OP_REF=op://vault/item/field` |
| `OP_ENVIRONMENT` / `OP_SERVICE_ACCOUNT_TOKEN` | Optional bulk-loading path for CI, service accounts, or newer 1Password CLI environment flows |
...
For normal local development, either put non-shared local values directly in `.env` or sign in to the `op` CLI and use root-only `*_OP_REF=op://...` references. For CI workflows or service-account usage, set `OP_SERVICE_ACCOUNT_TOKEN`, set `OP_ENVIRONMENT`, and enable `OP_ENABLE_ENVIRONMENT_LOAD=true`.
...
<StepFlow
  steps={[
    ...
    {title: "Configure env bootstrap", detail: "Set `APP_ENV` in root `.env`. Add only the secrets needed for your workflow. Use root `.env` only -- never add package-level env files."},
