...
| Upload-capable QA | Frontend QA plus `PINATA_JWT_OP_REF=op://...` or `VITE_PINATA_JWT` in root `.env` | `bun run dev:doctor -- --profile upload` |
...
| `VITE_PINATA_JWT` | Upload-capable Pinata JWT for the current browser-facing shared IPFS upload path. `PINATA_JWT` is generated from it as a script compatibility alias. Not required for baseline web dev. |
...
| `Pinata forbidden` or upload fails | Missing Pinata JWT | Add `PINATA_JWT_OP_REF=op://...` or `VITE_PINATA_JWT` to root `.env`, then rerun `bun run dev:doctor -- --profile upload` |
