...
For personal local-only credentials, put direct values in root `.env`. For shared Green Goods credentials (upload-capable QA, deploys, CI), edit `.env.template` to use 1Password URIs in the `op-scheme://Vault/Item/field` form, sign in to the `op` CLI, and run `bun run env:sync`.
