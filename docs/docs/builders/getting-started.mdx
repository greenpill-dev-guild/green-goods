...
| Full-stack / indexer | Frontend QA plus Docker and Envio v3 declarations from `bun run --cwd packages/indexer codegen` | `bun run dev:doctor -- --profile full` |
...
| Production local indexer mirror | Production-backed review plus Docker and generated `packages/indexer/.envio/` declarations | `bun run dev:prod:mirror:health` |
...
local account-abstraction infrastructure exists. `?mockAuth=steward` only
...
The workbench launches the canonical checkout registered in
`.dev-surfaces/surfaces.json`. From an isolated worktree, run `bun run dev`
inside that worktree so validation uses the code you are editing.
...
bun run dev:admin       # Steward dashboard
