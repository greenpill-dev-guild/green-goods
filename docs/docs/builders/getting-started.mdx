...
3. **Environment file** -- generates `.env` from `.env.schema` defaults if no `.env` exists. Prefer configuring `OP_ENVIRONMENT` for 1Password injection and keep `.env` for non-secret defaults.
...
| `OP_ENVIRONMENT` | 1Password environment name/id used by Varlock bulk secret injection |
| `OP_SERVICE_ACCOUNT_TOKEN` | 1Password service account token (required in CI/deployed environments) |
...
For CI workflows, this repo sets a single repository variable `OP_ENVIRONMENT`, then uses `varlock run` to bulk-load variables from that 1Password environment.
...
<StepFlow
  steps={[
    ...
    {title: "Configure env bootstrap", detail: "Set `APP_ENV` and `OP_ENVIRONMENT` in root `.env` (recommended). Use root `.env` only -- never add package-level env files."},
