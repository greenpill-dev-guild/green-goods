...
The admin package is the Green Goods steward canvas. This page is the single UI contract for `packages/admin`.
...
- Hub canonical deep links: `/hub/work/:workId`, `/hub/work/submit`, `/hub/assess/create`, `/hub/certify/:assessmentId`, `/hub/certify/create`, `/hub/confirm/:commitmentId`
...
- Retired `/hub/history` bookmarks return to the Hub's default Work mode, and retired `/community/pools` bookmarks open Community Coordination. Both redirects preserve the existing query string.
...
- Controlled Chrome is the admin material boundary: the NavigationBar/FAB dock is the only translucent surface — flat `rgb(var(--admin-surface-0) / 0.85)` with a 12px backdrop blur, a warm ambient shadow (`--admin-chrome-shadow`), and a 1px ink ring. The AppBar root and `MainSheet` stay transparent so content sits directly on the constant linen canvas. Route cards, forms, tables, lists, dense content, dialogs, and the account side sheet stay solid.
- `ConnectShell` is the disconnected full-screen state with a centered connect prompt and no navigation.
- Admin owns the shell forks in `packages/admin/src/components/Shell/` — `AppBar`, `NavigationBar` (with the FAB), and `MainSheet` — plus `CanvasLayout`, `AdminSideSheet`, `CommandPalette`, `AccountProfilePanel`, `AccountSettingsPanel`, `AccountSurface`, `ConnectShell`, and `PageHeader`.
...
- Shared owns `GardenChip`, `NotificationPanel`, and `SheetErrorBoundary`; `AppBar`, `NavigationBar`, and `MainSheet` are admin-owned forks in `components/Shell/`. The `LeftSheet`/`RightSheet`/`BottomSheet` renderers are deleted — every workspace overlay uses `AdminDialog` (centered on desktop, with the variant-specific mobile geometry in the dialog contract); the three global AppBar surfaces are the one `AdminSideSheet` exception.

## Preferred Primitives

- `AppBar` (admin Shell fork)
- `NavigationBar` (admin Shell fork)
- `MainSheet` (admin Shell fork)
- `GardenChip` (shared)
- `CommandPalette`
...
- Use `WorkbenchCard` (a shared Canvas primitive from `@green-goods/shared`, not an Admin* wrapper) for workbench records, action queue items, assessment queue items, and other records stewards scan, compare, and act on in a grid or list.
...
- `.canvas-route-card` is the route content frame — now transparent, so route content sits directly on the linen canvas.
...
- `--admin-surface-0` is the white card role — solid raised surfaces (cards, dialogs, the dock base) draw from it.
...
- **Chrome.** Header is a hairline-bottom bar (`px-4 py-3 sm:px-6`) with an optional inline icon, a title-large (22px/28px at weight 600) **sentence-case** title, and a `text-sm` sentence-case description. The body is the scrollable region between header and footer. The `actions` slot renders as a pinned footer bar — hairline top border on the raised white surface (the `--admin-surface-0` family), the same anatomy `SheetFooter` gives the flows. `AdminDialog` owns all of this; consumers never rebuild header or footer chrome.
...
- Admin is a steward canvas. Use utility copy and operational hierarchy over marketing language.
- Default page sequence is the `CanvasRouteFrame` + `CanvasRouteHeader` composition (`PageHeader` under the hood — views never import it directly), then the primary workspace, then any secondary inspector (`AdminDialog`, centered on desktop).
...
- Use the faint workspace wash over the constant linen canvas, transparent AppBar continuity, Controlled Chrome depth on Navigation/FAB, and high-contrast state accents to establish hierarchy without turning the UI into a promo surface.
- Use `/hub` as the reference implementation for canvas composition and interaction density.
- `bun run check:design-tokens` enforces the material boundary: admin source must not add glass, backdrop blur, or decorative gradients outside `packages/admin/src/index.css`, `admin-m3-tokens.css`, and `admin-m3-components.css`. The dock material itself is defined by the Controlled Chrome contract rules in `admin-m3-tokens.css`; the dock's geometry lives in the Shell JSX.
...
- Every primary route must answer one steward question, not three.
...
- Default page anatomy is `CanvasRouteFrame` + `CanvasRouteHeader` -> one dominant workspace canvas -> optional secondary inspector (`AdminDialog`, centered on desktop, or a single rail).
...
- Use utility copy. Titles identify the workspace. Descriptions explain scope, freshness, or what the steward can do next.
- Keep imagery subordinate to status, title, and actions unless the steward is explicitly reviewing media itself.
...
- `/hub` is the primary canvas workspace. Its primary object is the Hub workbench with Work, Assess, and Certify modes, plus the route-backed commitment confirmation flow.
...
- Read-only Hub deep links (`/hub/work/:workId`, `/hub/certify/:assessmentId`) are `AdminDialog` inspectors that render through the Hub controller and preserve shareable `gardenAddress` and `sort` query state. Full-surface creation/commit deep links (`/hub/work/submit`, `/hub/assess/create`, `/hub/certify/create`, `/hub/confirm/:commitmentId`) open the action-flow dialog (full-width bottom sheet on mobile), not inspector sheets. Hub mode belongs to the path, not a `view` query parameter.
- `Certify` stays visible for evaluator-only users as a read-only handoff state. Owner or steward authority unlocks mint actions.
- `/garden` is the situational overview of a single garden. Its primary object is health, activity, and impact state.
- `/community` is the operations workspace for members, coordination, endowment, and payouts. Governance and commitment pools share the Coordination mode; only one mode should dominate the canvas at a time.
...
- `/garden`: large summary card grids before the steward reaches the actual overview or impact workspace.
...
- `/community`: treating members, coordination, endowment, and payouts as one blended page instead of four focused operational modes, or splitting commitment pools out of Coordination into a fifth mode.
...
- List pages: `CanvasRouteFrame + CanvasRouteHeader + ListToolbar + filters/sort + loading/empty/error + results`
- Detail and workbench pages: `CanvasRouteFrame + CanvasRouteHeader` (or garden hero) plus main/rail composition
- Operational pages: `CanvasRouteFrame + CanvasRouteHeader + stage/tabs + primary workspace + optional inspector`
