...
- The shell is `TopContextBar + .workspace-canvas + MainSheet + NavigationBar`.
...
- Admin owns `CanvasLayout`, `CommandPalette`, `AccountSheet`, `AccountSettingsPanel`, `AccountSurface`, `UserAvatar`, `ConnectShell`, and `PageHeader`.
- Shared owns `TopContextBar`, `NavigationBar`, `GardenChip`, `MainSheet`, `SideSheet`, `BottomSheet`, and `SheetErrorBoundary`.
...
- `/hub` defaults to one dominant workbench surface under the stage rail. Detail, review, submit, and read-only handoff flows belong in bounded `SideSheet` or `BottomSheet` inspectors.
