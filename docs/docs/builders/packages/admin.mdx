...
- `/hub` defaults to one dominant workbench surface under the stage rail. Detail, review, and read-only handoff flows belong in bounded `LeftSheet`, `RightSheet`, or `BottomSheet` inspectors. Full-surface creation/commit flows (Submit Work, Create Assessment, Create Hypercert) open as a centered `AdminDialog` (`size="2xl" variant="flow"`) with a scrim — a bottom-sheet on mobile — hosting the shared `ActionFlowShell` grammar (pinned header + scrolling body + pinned footer), not a Hub inspector sheet. They are centered modals: never a fullscreen takeover, an edge-to-edge surface, or a non-modal route.
- Read-only Hub deep links (`/hub/work/:workId`, `/hub/certify/:assessmentId`, `/hub/history/:historyEventId`) are bounded inspector sheets that render through the Hub controller and preserve shareable `gardenAddress` and `sort` query state. Full-surface creation/commit deep links (`/hub/work/submit`, `/hub/assess/create`, `/hub/certify/create`) open the centered action-flow dialog (bottom-sheet on mobile), not inspector sheets. Hub mode belongs to the path, not a `view` query parameter.
