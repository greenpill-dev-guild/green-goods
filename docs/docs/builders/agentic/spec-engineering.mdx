...
The project keeps 11 skills -- `review`, `module-seams-review`, `ship`, `plan`, `debug`, `clean`,
`audit`, `qa-triage`, `doc-feedback`, `design`, and `resolve-pr-comments` -- reserved for procedures
and integrations with real orchestration. Domain knowledge that used to live in skills (`testing`,
`react`, `web3`, `contracts`, the PostHog question library, ...) moved to
