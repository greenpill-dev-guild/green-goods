...
The project keeps 10 skills -- `review`, `ship`, `plan`, `debug`, `clean`, `audit`, `qa-triage`,
`doc-feedback`, `design`, and `resolve-pr-comments` -- reserved for procedures and integrations with
real orchestration. Domain knowledge that used to live in skills (`testing`, `react`, `web3`,
`contracts`, the PostHog question library, ...) moved to `.claude/context/*.md` and `docs/`, so
specs reference documentation rather than skill files for patterns.
