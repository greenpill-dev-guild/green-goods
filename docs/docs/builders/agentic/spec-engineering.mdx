...
The project keeps 11 skills -- `review`, `ship`, `status`, `plan`, `debug`, `clean`, `audit`, `qa-triage`, `doc-feedback`, `posthog-questions`, and `design` -- reserved for procedures and integrations with real orchestration. Domain knowledge that used to live in skills (`testing`, `react`, `web3`, `contracts`, ...) moved to `.claude/context/*.md` and `docs/`, so specs reference documentation rather than skill files for patterns.
