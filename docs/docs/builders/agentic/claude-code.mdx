...
```
.claude/
  skills/           # 13 skills -- the filesystem + SKILL.md frontmatter is the registry
    ...
    research/       # Evidence-first grounding for planning and decision discussions (passive)
    ...
    module-seams-review/ # Deep boundary, direct-test, mock-fidelity, and validation-velocity review
    qa-triage/      # Build sync QA notes -> Linear records + QA-sheet rows
    qa-session/     # Live founder-led QA walkthrough copilot: capture -> fix-now -> revalidate
    doc-feedback/   # Google Doc review feedback processing
    design/         # Design direction + implementation (design/implementation.md)
    resolve-pr-comments/ # Validate and resolve actionable GitHub review feedback
  ...
  context/          # Per-package context files
    ...
    admin.md        # Dashboard, steward workflows
...
There is no separate skills registry, index file, or per-skill version ceremony -- each skill's
`SKILL.md` frontmatter description is what triggers it, and the filesystem is the count of record.
Six skills are user-invocable slash commands advertised in the session banner
(`/review /audit /clean /qa-triage /qa-session /doc-feedback`); other skills load from their declared triggers or
are invoked by the workflow that owns them. The curated PostHog question library moved to
`docs/routines/posthog-questions.md` -- it is a data dictionary, not a procedure. `.agents/skills`
is a symlink to `.claude/skills`, so Codex sees the same set. `bun run check:codex-guidance` keeps
Codex guidance in parity, and `scripts/quality/check-guidance-links.mjs` (run locally via
`bun run drift:check` and in CI by the supply-chain workflow's guidance job) enforces link integrity,
retired-name hygiene, and banner/frontmatter parity.
