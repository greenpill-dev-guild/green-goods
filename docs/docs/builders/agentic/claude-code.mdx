...
- **Multi-agent teams** coordinate parallel work across packages using the `agent-teams` skill and `/teams`
...
.claude/
  ...
  skills/           # 19 active skill definitions organized by domain
  ...
  hooks.json        # Tool hook policy snapshot
  settings.json     # Claude Code project settings + live hook config
```
...
Multi-agent teams use the `agent-teams` skill and `/teams` flow to spin up parallel workstreams. Each agent gets a focused task brief, and the team lead coordinates through the task system. Teams are used for large refactors, cross-package migrations, and documentation sprints.
