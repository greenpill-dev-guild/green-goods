...
### Armin Ronacher + Earendil -- Friction, Legibility, Judgment

Armin Ronacher's writing and Earendil's public material add an important counterweight to "go faster" agent discourse: some friction is not process waste, it is where engineering judgment lives. Their guidance emphasizes agent-legible codebases, simple and stable patterns, explicit review call-outs for high-cost decisions, and resisting the temptation to let output volume masquerade as architectural progress.

Green Goods applies that thinking by separating deterministic checks from judgment-heavy review, keeping criticality explicit, and treating high-risk surfaces like contracts, permissions, migrations, and shared public APIs as places where humans should slow down on purpose.
...
- **Skills** in `.claude/skills/` encode the repo's procedures (review, ship, plan, debug, clean, audit, qa-triage, …). Specialist dispatch uses the harness's built-in subagent types with task-scoped prompts rather than committed agent specs — review, migration, triage, and documentation work all route through skills and workflow prompts.
...
- **Armin Ronacher + Earendil** -- talks, RFCs, and posts on friction, review, durable systems, and agent-legible codebases
- **Green Goods `CLAUDE.md`** -- The living reference for how this project structures agent context (see root of this repository)

<NextBestAction
  title="Next best action"
  why="See how Green Goods decides which friction to automate and which friction to keep human-owned."
  actionLabel="Friction Engineering"
  actionHref="./friction-engineering"
  alternatives={[
