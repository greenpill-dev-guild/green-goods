...
Use it for local browser debugging, rendered DOM proof, console/network/performance inspection, screenshots, performance traces, and WebMCP discovery.

## Agent access model

`.mcp.json` allows `brave-devtools` for local browser debugging across client, admin, docs, and Storybook surfaces. Use isolated or non-default Brave profiles for authenticated admin, wallet-adjacent, or private-data proof.
...
- Use Brave DevTools MCP when live browser state, rendered DOM, console/network/performance data, screenshots, or WebMCP discovery are required.
