...
Skills are modular instruction sets in `.claude/skills/`. Each skill provides domain-specific patterns and constraints. The repo keeps a focused local skill set here, while plugin and system skills add more runtime coverage. Local skills cover areas like:
...
- `ui` -- Shared primitives, accessibility, Storybook, and interface composition
- `ops` -- Deployments, migrations, and repo workflow
- `review` -- Code review and audit workflows
