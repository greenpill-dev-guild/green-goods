...
A Claude guidance check validates skill frontmatter, the skills registry, command aliases, command bundles, retired guidance surfaces, and a small set of forbidden guidance patterns:

```bash
node .claude/scripts/check-skill-frontmatter.js
```

This runs in CI to catch structural drift in Claude-owned guidance surfaces. It is not a semantic proof that every guidance file agrees with every other file.
...
5. **Testable consistency** -- CI validates that guidance registries and surface lists stay aligned
