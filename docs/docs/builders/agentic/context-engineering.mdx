...
graph TB
    subgraph Layers["Context Layers (broadest → most specific)"]
        ...
        L3["Layer 3: context/*.md\nRead on demand — package and domain depth"]
        L4["Layer 4: rules/*.md\nPath-scoped — typescript, contracts, frontend"]
        L5["Layer 5: skills/*\nProcedural — 12 skills, loaded on trigger"]
    end
...
The root `CLAUDE.md` provides Claude Code harness context: commands, architecture, key patterns,
git workflow, and tool-specific routing. Keep shared policy in `AGENTS.md` or the canonical context
file it links so this always-loaded layer remains useful rather than becoming a second copy.
...
These files are canonical, read-on-demand depth for package and domain work. Package `AGENTS.md`
files and task prompts route agents to them; their presence alone does not prove every harness loads
them automatically.
...
5. **Checked drift** -- `check:codex-guidance` and the maintenance loop catch stale references and parity drift
