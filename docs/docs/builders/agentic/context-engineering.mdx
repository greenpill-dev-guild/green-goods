...
graph TB
    subgraph Layers["Context Layers (broadest → most specific)"]
        ...
        L5["Layer 5: skills/*\nPer-agent — 19 active skill sets"]
    end
...
.claude/
  ...
  skills/          # Modular instruction sets (19 active skills)
...
This repository does not currently check in a `.claude/agent-memory/` tree. For repo-level continuity, rely on `session-state.md` plus the committed context, rules, and skill files.
