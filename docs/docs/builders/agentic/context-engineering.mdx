...
```
.claude/
  agents/          # Agent specifications (4 custom agents)
  context/         # Package and domain context files
  evals/           # Runnable agent evals plus acceptance QA companion
  registry/        # Skill bundles and activation configs
  rules/           # Path-scoped coding rules
  scripts/         # Guidance consistency and workflow checks
  settings.json    # Canonical Claude Code settings, hooks, and env flags
  skills/          # Repo-local instruction packs
  specs/           # Reusable task templates
```

Live planning no longer lives under `.claude/`; use `.plans/` for active and backlog plan hubs.
...
Each agent's specification lists the skills it needs. Repo-local skills live in `.claude/skills/`; plugin and system skills load from their own surfaces at runtime. Skills provide detailed how-to instructions for specific domains.

## Session Continuity

### Local Memory

Claude-local project memory is not part of the tracked repo guidance surface. Treat any `.claude/projects/` state as local and unversioned, not as canonical documentation for future agents.
...
This enables context recovery after compaction or session handoff without relying on local memory.
...
4. **Tracked guidance is version controlled** -- Repo guidance is committed; local memory is not
