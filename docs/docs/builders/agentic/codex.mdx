...
graph TB
    ...
    Skills[".agents/skills\nSymlink to .claude/skills"] -->|skills| Sandbox

    Sandbox -->|output| PR["Draft PR\nmax 20 files"]
```

OpenAI Codex is used for automated maintenance tasks in the Green Goods monorepo. The repo-scoped
configuration is intentionally small: `AGENTS.md` is the primary context, `CLAUDE.md` is the
fallback, the local environment delegates lifecycle work to repo scripts, the workspace-write
sandbox disables network access, and hooks add repo-specific context and policy checks around tool
use. Model selection stays outside the project config.
...
- **`model_verbosity` / `model_reasoning_summary`** -- keep routine output compact while retaining a concise reasoning summary
...
- Codex-visible path: `.agents/skills` → symlink → `.claude/skills`
...
| Model | Selected by the Claude harness or user | Selected by the Codex environment or user |
