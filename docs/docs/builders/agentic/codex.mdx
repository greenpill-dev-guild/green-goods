...
# Prepend mise and Bun shims, then:
...
Codex local-environment shells are not guaranteed to be login shells; all
workspace behavior lives in `scripts/dev/setup.js` and `scripts/dev/clean.js`.
