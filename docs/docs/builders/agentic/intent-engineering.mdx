...
source_of_truth:
  ...
  - .plans/README.md
  - .plans/_templates/feature/brief.md
...
The `.claude/context/product.md` file provides product and organizational context for decisions that require business judgment:
...
This context is loaded when agents make prioritization or UX decisions, ensuring they align with organizational intent.
...
From `.claude/context/product.md` and `.claude/context/values.md`, agents use these heuristics:
...
### Scope Decisions
- When the canonical product context maps work to a funding commitment, keep the implementation
  inside that commitment's accepted deliverables.
...
<NextBestAction
  ...
  why="See how to write specifications that guide agent implementation."
  actionLabel="Spec Engineering"
  actionHref="./spec-engineering"
  alternatives={[
