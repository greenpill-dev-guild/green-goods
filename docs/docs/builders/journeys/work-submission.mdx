...
How a piece of regenerative work moves from a gardener's phone to an EAS attestation, surviving offline conditions and steward review along the way. This is the **highest-volume journey** in Green Goods and the offline-first mechanic that drives the rest of the architecture.

## Personas

- **A: Gardener** — submits the work via PWA wizard or via the agent.
- **B: Steward** — reviews queued submissions and signs the approval attestation.
...
stateDiagram-v2
    ...
    AwaitingApproval --> Approved: steward /approve OR Hub approve
    AwaitingApproval --> Rejected: steward /reject OR Hub reject
...
| Admin Hub (steward review) | `packages/admin/src/views/Hub/index.tsx` (stage `work`) | Steward opens Hub workspace, selects garden |
...
| 6a | AgentPending | A | `agent` (db) | `db.addPendingWork`, `db.getOperatorForGarden`, `notifyOperator` | DB row + Telegram message to steward with `/approve <id>` button | shipped |
...
| 9b | Approved (agent) | B | `agent` / `handlers/approve.ts` | `blockchain.submitWork` (gardener key), `blockchain.submitApproval` (steward key) | Two attestations: work uses gardener custodial key, approval uses steward key — prevents self-attestation. `auditLog("steward:approve", ...)` | shipped |
| 10 | Rejected | B | `admin` (Hub) or `agent` / `handlers/reject.ts` | `useWorkApprovalActions`, `db.removePendingWork` | Gardener notified via Telegram; no on-chain attestation. `auditLog("steward:reject", ...)` | shipped |
...
- **Steward permission denied (agent path).** `blockchain.isOperator(garden, user.address)` returns `{verified: false}`. Bot replies with reason; no DB mutation.
- **Steward approves work for gardener with no account.** `db.getUser(pendingWork.gardenerPlatform, pendingWork.gardenerPlatformId)` returns `null`. Bot replies "Gardener account not found. They may need to run /start first." This is a real failure mode in mixed-channel onboarding.
...
- Hub FAB (`buildHubFabConfig`) gates the "Submit Work" action on `canManage` — stewards can submit on behalf of a gardener via the admin Hub when needed.
