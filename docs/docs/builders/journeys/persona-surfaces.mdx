...
feature_status: Live
source_of_truth:
  - docs/docs/builders/specs/v1-0.mdx (§ 3.1 Personas)
  - packages/client/src/router.config.tsx
  - packages/admin/src/views (all workspaces)
...
| **B** | Operator (Steward) | Coordinates the garden; reviews work; mints hypercerts |
| **C** | Evaluator (Verifier) | Hat-holder who attests assessments; gated by `HatsModule.isEvaluator` |
| **D** | Funder (Capital Provider) | Deposits into vaults / cookie jars; receives Karma GAP linkage |
| **E** | Community Member (Beneficiary) | Local resident; signals priorities via conviction |

## Public PWA routes (canonical)

There are exactly four public routes in `packages/client/src/router.config.tsx` (plus `/login`, `/landing`, and authenticated `/home` / `/garden` / `/profile` paths):
...
### E: Community Member (Beneficiary)
