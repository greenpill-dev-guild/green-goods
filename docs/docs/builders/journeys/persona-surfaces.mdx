...
| **B** | Steward | Coordinates the garden; reviews work; mints hypercerts |
...
- The 4-stage Hub workspace (Work → Assess → Certify → History) is the canonical steward pipeline. Stage transitions correspond to journey steps; stage filtering is governed by `useHubWorkbenchController`.
- Cross-garden context: `useEligibleAdminGardens` + `useAdminGardenWorkspaceSelection` constrain admin views to gardens the user holds a steward/owner hat on.
...
- An evaluator who happens to also be a steward sees the same workspace as any steward; they additionally have on-chain authority to sign Assessment attestations.
...
- **Conviction signaling** is currently **steward-facing**: the conviction allocation hooks (`useAllocateHypercertSupport`, `useHypercertConviction`) and signal-pool primitives (`useGardenPools`, `useRegisteredHypercerts`) are wired into admin Community Governance / Strategies / SignalPool views, not into a public PWA surface.
...
| E: Community Member | No public-side conviction allocation surface. Hooks exist (`useAllocateHypercertSupport`, `useHypercertConviction`) but no view in `packages/client/src/views/Public/` allows a community member to allocate signal from the public browser site. Steward-facing surfaces in `packages/admin/src/views/Community/` and `Garden/{Strategies,SignalPool}.tsx` cover it for hat-holders only. | Spec § 3.1 names "Public Signal Feed and Gardens Conviction Voting" as the Persona E key interface — partial. |
