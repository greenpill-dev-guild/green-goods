...
source_of_truth:
  ...
  - packages/shared/src/hooks/admin-ui/hub/useCreateHypercertController.ts
...
stateDiagram-v2
    ...
    BaselineAssessed --> WorksAccumulating: gardeners submit + stewards approve
...
| --- | --- | --- |
| Admin Hub Certify stage | `packages/admin/src/views/Hub/index.tsx` (stage `certify`) | Steward selects garden, opens FAB "Mint Hypercert" |
| Admin Community Strategies | `packages/admin/src/views/Garden/Strategies.tsx` | Steward configures yield split + decay |
| Admin Community Payouts | `packages/admin/src/views/Community/index.tsx` (mode `payouts`) | Steward surfaces yield allocation history |
| Vault harvest | `packages/admin/src/components/Vault/PositionCard.tsx` (via `useVaultOperations` → `useHarvest`) | Steward triggers harvest of accrued yield |
...
| # | State | Persona | Surface (package + view) | Hook / Service | Side effects | Status |
| --- | --- | --- | --- | --- | --- | --- |
| 12 | Finalize Season | B | (not built) | No `useCloseSeason` orchestration hook | Spec § 1.5 "Season One" + § 6.3 reference a Season primitive but there is no admin UI to declare a Season closed and freeze the work / assessment / hypercert set | aspirational |
| 13 | Publish Season record | B | (not built) | Public Season read shape is planned; no publish mutation exists | The write path (steward declares Season N closed → Season record materialized) is not in the codebase | aspirational |
...
- **Season close attempted (aspirational).** No code path exists. Stewards currently cannot mark a Season closed; works keep accumulating.
...
- A "Season close" primitive does not exist in the contract surface today. If you scope the orchestration: define a public Season read model and a write path (steward-callable, gated by Owner hat) that snapshots the works + assessments + hypercerts set and emits a Season event the indexer can materialize.
