...
- **D: Funder (Capital Provider)** — primary subject. Either a public supporter (Public Fund page) or a steward-funder hybrid using the admin dashboard.
- **B: Steward** — administers vault parameters, cookie jar policies, yield split config; does not deposit on behalf of funders.
...
| Admin Garden / Vault | `packages/admin/src/views/Garden/Vault.tsx` | Steward-funder picks a garden, opens Vault tab |
| Admin Hub / Cookie Jar | `packages/admin/src/views/Hub/components/CookieJarDepositModal.tsx`, `CookieJarManageModal.tsx`, `CookieJarPayoutPanel.tsx` | Steward opens cookie jar modals from Hub workspace |
| Admin Garden / Strategies | `packages/admin/src/views/Garden/Strategies.tsx`, `SignalPool.tsx` | Steward configures yield split + conviction strategy |
...
| # | State | Persona | Surface (package + view) | Hook / Service | Side effects | Status |
| --- | --- | --- | --- | --- | --- | --- |
| 14 | Steward lists hypercert for yield | B | (no dedicated view yet — workflow lives in shared hook) | `useCreateListing`, `useBatchListForYield`, `useMarketplaceApprovals` | Marketplace approval + listing tx | shipped |
