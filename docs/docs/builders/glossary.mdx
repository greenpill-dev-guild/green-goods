...
Looking for community terms? See the [Community Glossary](/glossary).
...
### Celo
A carbon-negative blockchain focused on mobile-first financial tools and regenerative finance. Green Goods production runs on Arbitrum One, and GreenWill badges are issued on Arbitrum. A separate, partial Celo deployment exists (core contracts only, no GreenWill), so treat Arbitrum as the source of truth for live features.
...
### Envio
A blockchain event indexer that listens to on-chain events and exposes indexed data via GraphQL API. Green Goods uses Envio HyperIndex to track gardens, actions, roles and membership, vault activity, hypercerts, and badges. Work submissions and approvals are EAS attestations, queried directly from EAS rather than the indexer.
...
### Reown AppKit
A wallet connection SDK (formerly WalletConnect) for web3 applications. Green Goods uses Reown AppKit in the Admin dashboard to connect steward wallets like MetaMask for signing transactions.
