...
:::info Active v1 spec
Green Goods is currently working through v1. Some modules are implemented, while deployment, indexing, and UI readiness still vary by surface. Use Deployment Status and the integration pages for current chain readiness.
:::
...
The v1.0 strategy frames Green Goods as a Regenerative Compliance and Local First Impact Reporting Platform for verifiable ecological actions, Hypercert-based impact certification, Octant-style vaults, and future capital-formation surfaces.

```mermaid
flowchart TD
    Client[Client PWA - Offline-First] --> Shared[Shared Package]
    Admin[Admin Dashboard] --> Shared
    Agent[Agent Service - Telegram now; WhatsApp/SMS planned] --> Shared
...
### 1.5 Original Target Release

- **Release:** Green Goods Protocol v1 (Beta Candidate)
- **Original season assumption:** Q1 2026 (Transition from Alpha to Beta)
- **Original network assumption:** Arbitrum One and Ethereum
...
The governance goal is to implement a robust badging and reputation system built on GreenWill and Hats Protocol. These tools serve as the permission layer for the Gardens platform, ensuring that governance is driven by those who create value. By using Conviction Voting, the protocol empowers reputation holders to signal distinct priorities: deciding on hypercert yield allocation and prioritizing specific garden actions.

### 2.2 Original Success Metrics

The numeric targets below came from the original Q1 2026 beta planning assumption. They are retained as historical planning context, not as current April 25, 2026 delivery commitments. Update them from current program tracking before using this table for grant, launch, or status reporting.

| Metric | Definition | Original target |
|--------|------------|--------|
...
| **Reputation Density** | Number of active users holding a GreenWill "Steward" or "Sprout" badge | 32+ users |
| **Verification Efficiency** | Percentage of submitted actions processed (Approved/Rejected) by Evaluators/Stewards within 72 hours | >90% |
...
#### Persona B: The Garden Steward (The Steward)
...
| Impact Report Aggregation | Steward | Quarter ends and reporting is due | Aggregate all verified actions into a single Impact Report | Mint a Hypercert bundle and sell it to the Octant Vault |
...
- **PWA Impact Reporting:** A Steward uses the PWA to aggregate 500 individual tree planting actions into a "Q1 Reforestation Report." The system formats this data according to the Common Impact Data Standard (CIDS) for Karma GAP integration.
...
:::note Not the 2026 Commitment Pooling feature
"Commitment pooling" in this v1.0 domain meant a farm-verification-for-credit pilot that was deferred and never built (see the reconciliation note at the top of §3.4). The **Commitment Pooling** feature planned for 2026 is a different thing: a mutual-aid promise loop — offers, requests, evidence, and counterparty confirmation — with no credit lines; see [Design & Research](/reference/design-research) for its planned journeys. Borrow-and-repay records remain a separate design-only, evidence-gated follow-on: records-only, interest-free, and never a per-person credit score.
:::

**Primary Chapter (as planned in v1.0):** Greenpill Brasil through a farm-verification pilot.

**Key Outcome:** Credit access for rural farmers through verified productive capacity.

Rural farmers often lack access to credit due to inability to verify their productive capacity. In this v1.0 plan, Green Goods would have served as the verification tool for mutual-credit pilots in Brasil, where farmers demonstrating consistent agricultural output could unlock mutual credit lines — a pathway from verified work to financial inclusion. The pilot was deferred with the domain.
...
| Credit Enabled | Value of credit lines enabled |
...
- **Participants:** 3-5 Tech and Sun stewards from Nigeria (including Anthony Amio and Mmeri Anosike) plus university coordinators
...
#### Mutual Credit Domain Validation (deferred with Domain 5 — never run)

- **Participants:** 3-5 farmers from the planned Brasil mutual-credit pilot
...
- **Octant Vaults** — Deployment of yield-generating vaults for capital formation
...
- **GreenWill** — Reputation badge registry and claim surfaces based on user activity
- **Hats Protocol** — Management of on-chain roles (e.g., Steward, Evaluator) and permissions

**Verification:**
- **Stewards** — Tools for initial data aggregation and verification
...
- *Red Flag:* If error rate exceeds 15%, requiring Steward intervention on the majority of submissions.
...
3. **Review/Confirm** — Steward approval queue, Evaluator certification, and Hypercert minting.
...
4. **Store** — Data anchored on IPFS through the Pinata-backed upload path and stored onchain with EAS.

#### Flow 2: Impact Report Minting and Evaluation (High Trust)

1. **Aggregate** — Steward selects 20 verified "Tree Planting" actions in the PWA and clicks "Create Report."
...
3. **Mint** — The system now allows the Steward to mint the Hypercert, which references the Evaluator's certification attestation.
...
1. **Yield Allocation** — Garden members (steward, gardener, community member) vote via Conviction Voting to determine the split of yield to send to hypercerts.
2. **Action Prioritization** — Garden stewards and gardeners signal which specific actions (e.g., "Waste Cleanup") the Garden should prioritize. This signal directs the Steward's focus for the next cycle.
...
| FR-P-002 | Admin Dashboard | Comprehensive dashboard for Stewards to manage submissions and for Evaluators to review and certify Impact Reports |
...
| FR-P-007 | GreenWill Badging | GreenWill badge registry and claim surfaces based on user activity thresholds |
| FR-P-008 | Role Management | Integration with Hats Protocol to assign and manage on-chain permissions for Stewards and Evaluators |
...
| GG-FEAT-004 | Admin Dashboard v2 | High | Planned | 3 weeks | React, Envio | Steward verification queue |
| GG-FEAT-005 | Hypercerts Minting | High | Planned | 4 weeks | ERC-1155, IPFS | Aggregation + atomic mint |
| GG-FEAT-006 | Octant Vault Integration | High | Planned | 4 weeks | ERC-4626, DeFi strategies | Yield-generating deposits |
...
| GG-FEAT-009 | GreenWill Badging | Medium | Planned | 2 weeks | GreenWill registry | Reputation badges |
| GG-FEAT-010 | Hats Protocol Roles | Medium | Planned | 2 weeks | Hats Protocol | Steward/Evaluator permissions |
...
For an interactive version of these assumptions, use the [Protocol Revenue Explorer](/builders/specs/revenue-explorer) to test fee surfaces, TVL scenarios, treasury yield, and grant dependence.
...
**Original activity assumptions (per Arbitrum grant target model):**
- Active Gardeners: 150 by end of Q1 2026 original target
...
| IPFS pinning (Pinata-backed path) | 1,500 images x $0.001 | $1.50 |
...
| Photos and Media | Submissions | IPFS via Pinata-backed path | Verification evidence |
...
**Data Collected from Stewards and Evaluators:**
...
| IPFS (Pinata-backed path) | Images, metadata JSON, profile data | Immutable once pinned, global |
...
- **Mitigations:** Future AI face detection prompting user to retake; Steward review flagging PII before Hypercert minting; user guidance to avoid including faces; clear notice that IPFS photos are public.
...
- **Risk:** Data shared with Twilio, OpenAI, Pimlico, and Pinata creates exposure.
...
- **Likelihood:** Medium | **Impact:** High | **Level:** High
- **Mitigations:** Student names as optional field allowing pseudonyms; no photos of minors required; school administrator as adult Steward; parental consent process for any minor data; student ID codes instead of names.
...
- **Right to Rectification** — Users can update profile data. Action data can be flagged for Steward correction via new attestation with correction reference (original is not deleted).
...
| Data Processing Agreements | Contracts with Twilio, OpenAI, Pimlico, Pinata, Envio | Before launch |
...
**Celo Builders Fund** — funding scope not yet finalized
...
- **UAT:** Structured testing with 5 Gardeners and 3 Stewards per pilot Garden
...
|----|------|------------|--------|------------|
| R-T1 | AI Hallucination | High | Medium | Agent prompts include strict bounds; Agent stages data for Steward review, never finalizes directly |
...
| R-T4 | IPFS Pinning Failures | Medium | Medium | Pinata-backed uploads; retry logic with exponential backoff; local queue preservation |
...
| R-G3 | Garden Steward Abandonment | Medium | High | Multi-sig requirement for Garden administration; succession planning in onboarding; Protocol can appoint interim steward |
...
The due dates below are original planning dates from the v1 draft. Reconfirm owners and dates before treating them as active commitments.

| # | Question | Owner | Original due |
|---|----------|-------|--------------|
...
> Reconciled 2026-07-25 with the canonical [community glossary](/glossary) and the generated [ontology reference](/reference/ontology.generated). Entries below are annotated where the original v1.0 framing was aspirational rather than shipped.

| Term | Definition |
|------|------------|
| **Garden** | A community of gardeners rooted in a place, represented on-chain as an ERC-721 garden token whose ERC-6551 token-bound account holds the garden's treasury, role Hats, and metadata. (The original v1.0 framing as a Juicebox Revnet with its own token, treasury, and governance remains aspirational and is not deployed.) |
| **Action** | A discrete unit of ecological work (e.g., planting a tree, cleaning a beach) that is captured, verified, and attested |
| **Work** | One documented instance of an Action performed by a gardener — media, notes, and metadata submitted as an EAS Work attestation for steward review; approval records a separate Work Approval attestation. (The original "aggregate of Actions" framing inverted the cardinality and is retired.) |
| **Work Approval** | A separate Steward decision attestation that approves or rejects an already-attested Work submission; Evaluators author Assessments, not Work decisions |
| **Assessment** | An up-front baseline and strategy for a garden — domain, diagnosis, SMART outcome targets, selected Actions, reporting period — authored by an evaluator or steward before work begins, mirrored to a Karma GAP milestone when that module is active; explicitly not a review of submitted Work |
| **Report** | Not a canonical entity. The aggregated bundle of approved Work described here ships as a **Hypercert**; a standalone exportable report artifact is still planned work (GG-FEAT-013) rather than a term the ontology tracks. |
| **MDR pipeline (Monitoring–Data–Reporting)** | The protocol's internal term for the action capture-to-attestation-to-reporting pipeline. Distinct from the client submit wizard's MDR (Media–Details–Review) — see the [community glossary](/glossary#mdr-media-details-review). |
...
| **GreenWill** | Reputation badges earned through verified regenerative actions |
...
| Octant YDS Docs | [docs.v2.octant.build](https://docs.v2.octant.build/docs/developers/yield_donating_strategy/) | Yield donating strategy architecture |
