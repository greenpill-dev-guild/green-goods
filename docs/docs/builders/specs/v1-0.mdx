...
| **Work** | One documented instance of an Action performed by a gardener — media, notes, and metadata submitted as an EAS Work attestation for operator review; approval records a separate Work Approval attestation. (The original "aggregate of Actions" framing inverted the cardinality and is retired.) |
| **Work Approval** | A separate Operator decision attestation that approves or rejects an already-attested Work submission; Evaluators author Assessments, not Work decisions |
| **Assessment** | An up-front baseline and strategy for a garden — domain, diagnosis, SMART outcome targets, selected Actions, reporting period — authored by an evaluator or operator before work begins, mirrored to a Karma GAP milestone when that module is active; explicitly not a review of submitted Work |
