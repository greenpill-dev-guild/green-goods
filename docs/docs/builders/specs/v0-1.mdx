...
:::info Complete historical spec
v0.1 is complete and kept for historical context. v1 is the current active specification.
:::
...
```mermaid
flowchart LR
    User[Gardener / Steward] --> PWA[React PWA]
    ...
    TBA --> Badges[Badge and recognition surface]
```
...
**Garden Steward** -- Person managing an ecosystem, creating actions, and gathering resources to improve biodiversity.
...
As a garden steward, I want an easy way to add gardeners for my garden.
...
As a garden steward, I want to approve biodiversity work to reward work completed.
...
- **Badge System Based on Work** -- Active garden stewards and gardeners earn badges and tokens for their actions created and work completed, gaining future governance eligibility and airdrop eligibility.
...
**Smart Account:**
- Properties: Role (Gardener or Steward)
- Methods: `createAction`, `updateAction`, `uploadWork`, `approveWork`

### PWA-Driven Interface

Interface for both stewards and gardeners, fully mobile-built and accessible via the web as a PWA.

### Media and Metadata

Work submissions include media references and action metadata. This spec does not name a storage provider; provider-specific storage belongs in the current package and integration docs.
...
**Methods:**
- `registerAction(action_id, frequency, category, metadata)` -- requires sender whitelisted as steward
- `updateAction(action_id, frequency, metadata)` -- requires sender is owner

**Action Struct:** id, createdAt, frequency, category (enum), metadata with title, description, instruction, and media.

## Milestones

| Phase | Scope |
|-------|-------|
| **Designs for Garden Steward UI** | Screen designs for steward flows |
...
| **Interface for Approving Work** | Garden stewards approve work done by gardeners |
...
| Quarter | Focus |
|---------|-------|
| **Q2** | Simple MVP for gardeners and stewards to create actions and perform work, earning badges |
...
- [GitHub Repo (community-host)](https://github.com/greenpill-dev-guild/community-host) -- Historical React PWA codebase
