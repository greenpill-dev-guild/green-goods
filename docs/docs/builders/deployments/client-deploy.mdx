...
Both must be built before the client. The monorepo's `bun run build` command handles this order automatically.
...
<NextBestAction
  ...
  why="The admin dashboard shares the same build toolchain as the client but targets garden stewards with role-based access."
