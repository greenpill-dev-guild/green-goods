...
Vercel runs `bun run build` only. Source-map generation and PostHog upload run from the GitHub Actions `client.yml` lane on trusted `main` pushes, where `GG_ENABLE_SOURCEMAPS=true` enables map output for the upload job.
