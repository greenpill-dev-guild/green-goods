...
2. Run `bun run test` and `bun run build:full` in `packages/contracts` to verify tests pass and contracts compile
...
Action metadata is uploaded to IPFS via the Pinata-backed upload path during deployment:
...
The IPFS cache at `packages/contracts/.generated/runtime/ipfs-cache.json` prevents re-uploading unchanged metadata.

## Making A Deployment

### Deployment Architecture

#### CLI Layer (`script/deploy.ts` -> `script/deploy/cli.ts`)
...
- IPFS metadata upload runs through the Bun-native Pinata uploader used by the deployment scripts
...
# Pure simulation (compile + validation, no RPC calls)
...
Core deployments use CREATE2 for deterministic addresses on the supported deployment targets. This means:
