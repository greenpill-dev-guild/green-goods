...
| `PINATA_JWT` | Pinata JWT for shared IPFS uploads |
...
- `PINATA_JWT` controls authenticated uploads to the shared Pinata-backed IPFS path -- treat it like any other production secret
...
```bash
bun run dev:full  # Starts all services including agent via PM2
