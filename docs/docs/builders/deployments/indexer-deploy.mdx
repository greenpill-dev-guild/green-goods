...
The Green Goods indexer uses Envio HyperIndex `3.6.1` to index on-chain events into a
...
the repository proof gates pass, select the approved branch/version in Envio Cloud and verify its
hosted configuration and secrets before starting a full reindex. Pushing code does not authorize
deployment, reindexing, or endpoint cutover.
