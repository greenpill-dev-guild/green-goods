...
Additional admin-specific variables may include API keys for services like Hypercerts marketplace. Vercel must use `VITE_POSTHOG_ADMIN_KEY` for Admin analytics; `VITE_PUBLIC_POSTHOG_KEY` is not read by the app, and Admin intentionally does not fall back to the client `VITE_POSTHOG_KEY`.
...
Vercel runs `bun run build` only. Source-map generation and PostHog upload run from the GitHub Actions `admin.yml` lane on trusted `main` pushes, where `GG_ENABLE_SOURCEMAPS=true` enables map output for the upload job.
