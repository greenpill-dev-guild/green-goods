---
title: Integrations
sidebar_position: 1
slug: /builders/integrations/overview
audience: developer
owner: docs
last_verified: 2026-04-19
feature_status: Live
goal: "Complete Integrations with clear, role-specific next actions."
difficulty: "standard"
estimated_time: "20-45 minutes."
prereqs: "Chain and deployment context."
next_steps: "./eas"
persona_context: "Developers and maintainers building, operating, and validating Green Goods systems."
source_of_truth:
  - packages/shared/src/modules/data/eas.ts
  - packages/shared/src/modules/data/ipfs/upload.ts
  - packages/contracts/script/utils/update-action-metadata.ts
  - packages/indexer/config.yaml
  - .mcp.json
keywords:
  - integrations
  - eas
  - indexer
  - mcp
---

import {FeatureState, NextBestAction, StatusBadge} from "@site/src/components/docs";

# Integrations

<StatusBadge status="Live" />

```mermaid
graph TB
    GG["Green Goods\nProtocol"]

    GG --> EAS["EAS\nAttestations"]
    GG --> Hats["Hats Protocol\nRBAC"]
    GG --> HC["Hypercerts\nImpact Certs"]
    GG --> TB["Tokenbound\nERC-6551"]
    GG --> Octant["Octant\nYield Vaults"]
    GG --> CJ["CookieJar\nPayouts"]
    GG --> ENS["ENS\nGarden Names"]
    GG --> Envio["Envio\nIndexer"]
    GG --> Pinata["Pinata-backed\nIPFS"]
    GG --> Pimlico["Pimlico\nAccount Abstraction"]
```

## Integration surfaces

<FeatureState
  title="Indexer GraphQL"
  status="Live"
  summary="Use for gardens/actions/membership entities and chain-scoped metadata."
/>

<FeatureState
  title="EAS GraphQL"
  status="Live"
  summary="Use for work/approval/assessment attestations and cryptographic traceability."
/>

<FeatureState
  title="Pinata-backed IPFS uploads"
  status="Live"
  summary="Shared IPFS upload utilities and contract metadata utilities use the Pinata-backed upload path for media and metadata."
/>

<FeatureState
  title="v1 module integrations"
  status="Implemented (activation pending indexing)"
  summary="Several modules are deployed on Arbitrum/Sepolia, but indexer coverage and UI readiness vary by surface."
/>

## Reference

- [API Index](../packages/api-index)
- [Agent/MCP Guide](../agentic/mcp-guide)
- [Deployment Status](../deployments/status)

<NextBestAction
  title="Next best action"
  why="Use canonical endpoint and activation references before implementing or debugging integrations."
  actionLabel="Open API index"
  actionHref="../packages/api-index"
  alternatives={[
    {label: "Deployment status", href: "../deployments/status"},
  ]}
/>
