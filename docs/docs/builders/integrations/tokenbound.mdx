...
and history included, as long as transfers are open: the GardenToken contract can limit them to
the protocol's admin or lock them entirely (`transferRestriction`). Either way, no app of ours has
to exist for a garden to keep being real.
