...
2. On the **Pending** tab, a new submission may first appear as syncing or queued while it waits on the network or the queue to flush.
3. Once it reaches the chain, it stays in **Pending** while an operator reviews it.
4. Later, the same item shows up as **Approved** or **Rejected** in the Completed tab, along with any reviewer feedback.

<div style={{maxWidth: '320px', margin: '1rem auto'}}>
  <img src="/img/screenshots/client-work-dashboard.png" alt="Work Dashboard — a recent submission visible on the Pending tab with syncing or pending status" style={{width: '100%', borderRadius: '12px', border: '1px solid var(--ifm-color-emphasis-300)'}} />
</div>
