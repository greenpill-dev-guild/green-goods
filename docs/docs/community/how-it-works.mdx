...
- **Automatic smart account** — A [smart account](/glossary#smart-account-account-abstraction) is created for you using account abstraction. This is your on-chain identity.
...
sequenceDiagram
    ...
    participant PK as Passkey (Device)
    participant SA as Smart Account

    User->>App: Open app / tap "Sign In"
    App->>PK: Request biometric auth
    PK-->>App: Signed challenge
...
```mermaid
graph LR
    W["Work Attestation\n(Gardener submits)"] -->|reviewed by| WA["Work Approval\n(Operator verifies)"]
    WA -->|aggregated into| A["Assessment\n(Evaluator certifies)"]
    A -->|minted as| H["Hypercert\n(Impact certificate)"]
