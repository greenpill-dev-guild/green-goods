...
test.describe("Work Submission Flows", () => {
  // SKIP: #338 owner:afo expiry:2026-10-16 — needs real auth + gardens data
  test.skip(
    () => true,
    ...
    test("retries failed work submission", async ({ page }) => {
      ...
      if (await dashboardButton.isVisible({ timeout: 5000 })) {
        ...
        if (await retryButton.isVisible({ timeout: 5000 })) {
          ...
          // SKIP: #338 owner:afo expiry:2026-10-16 — requires a failed queued submission fixture
      ...
      if (await gardenCard.isVisible({ timeout: 5000 })) {
        ...
        if (await actionCard.isVisible({ timeout: 5000 })) {
          ...
          const requiresImage = await imageRequirement
            ...
            .isVisible({ timeout: 2000 })
            .catch(() => false);
          // SKIP: #338 owner:afo expiry:2026-10-16 — selected action must declare image evidence
