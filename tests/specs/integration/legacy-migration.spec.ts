...
test.describe("Legacy Migration - Green Goods Base Sepolia Integration", () => {
  test("should have indexer running on port 8080", async ({ request }) => {
    // Test the indexer GraphQL endpoint directly
  ...
  test("should load the homepage when client is running", async ({ page }) => {
    // Skip if client is not running
    try {
      ...
      console.log("Client not running, skipping client tests");
      test.skip();
    }
  });

  test("should have working GraphQL proxy when client is running", async ({ page }) => {
    // Skip if client is not running
    try {
      await page.goto("/", { timeout: 5000 });
      ...
      // Check if the GraphQL proxy is working by making a request through the client
      ...
      console.log("Client not running, skipping proxy tests");
      test.skip();
    }
  });
  ...
  test("should validate environment configuration", async () => {
    ...
    const chainId = process.env.VITE_CHAIN_ID || "84532";
