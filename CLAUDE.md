...
1. **Brave/Chrome MCP live DOM** — if an admin tab is open (`mcp__claude-in-chrome__tabs_context_mcp` or equivalent Brave-backed browser MCP), use `javascript_tool` or `read_page` to read `data-component` / `data-variant` / `data-region` / `data-workspace` on the rendered page. This is the preferred path because the user runs admin review in Brave first, with Chrome/Chromium-compatible fallback.
