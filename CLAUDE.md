...
> **`test` vs `test:fast`**: `bun run test` follows the root dependency-ordered script: Contracts; Shared + Docs in parallel; Indexer; then Client + Admin + Agent in parallel. `bun run test:fast` covers the same package test scripts through Turborepo, runs at most two package suites concurrently, and caches passing runs by input hash. Use it for the normal full-scope iteration loop. Cache invalidates automatically when a package's source, its workspace dependencies' source (shared/contracts), `.env`, `biome.json`, or root tsconfigs change. **Failing tests are never cached** — fix the test, not the cache. Use the exact uncached `bun run test` only when a gate or evidence claim requires it. To force a fresh cache-aware run, use `bun run test:fast:force` or `rm -rf .turbo`.
