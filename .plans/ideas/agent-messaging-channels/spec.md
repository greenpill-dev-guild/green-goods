...
**Status:** DRAFT proposal; idea stage; no implementation or provider selected.
...
Only A1–A3 are accepted user decisions. P1–P5 are recommendations awaiting selection. O1–O5 remain open; documentation completion does not close them.

| ID | State | Decision or question | Consequence |
| --- | --- | --- | --- |
| A1 | Accepted | WhatsApp first for TAS; Nigeria; English | Pilot recruitment, carrier proof and costs must reflect this setting. |
...
| P2 | Proposed | New users start with private drafts; existing users reuse their current account | Do not generate another wallet merely because someone opens WhatsApp. |
| P3 | Proposed | Owner signs each work publication in first release | Avoid a new delegated signer or validator before the core journey is proven. |
...
| O1 | Open: product/research | Is one browser passkey setup acceptable before a new user's first onchain publication? | User has not answered this question. RESR-75 explicitly asks for verification without sign-up; a browser visit with account creation is still sign-up. |
| O2 | Open: product/operations | Direct Meta, Twilio or another provider; who owns sender, billing and support? | Obtain actual Nigeria provisioning, media, template, migration and cost evidence before selection. |
...
The [messaging project](https://linear.app/greenpill-dev-guild/project/agent-messaging-channels-whatsapp-sms-71cda634fcf7) is research-gated. [RESR-75](https://linear.app/greenpill-dev-guild/issue/RESR-75) remains Todo and requires no-sign-up identity/verification, country-specific cost, comprehensible consent, fallback and total labor evidence. No research-panel acceptance was found. [RESR-21](https://linear.app/greenpill-dev-guild/issue/RESR-21) leaves same-address total-passkey-loss recovery open.
