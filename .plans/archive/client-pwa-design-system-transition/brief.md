...
classes. The global heading rule in `packages/client/src/styles/typography.css` paints all
`h1`-`h6` with primitive green tokens, which is the likely root cause of dark-mode green-on-green
drift across the PWA.
...
- Headings read as scoped accent rather than ink, weakening the hierarchy in both light and dark
  mode.
...
- Heading text reads as foreground/ink across both PWA and public browser; scoped green is
  reserved for explicit accents.
...
Stage 0 readiness, Stage 1-4 source implementation, local QA evidence, and automated validation
are complete. The protected PWA baseline is now clear; the only remaining client `legacy-runtime`
rows are deferred public/browser `Hero.tsx` rows. On 2026-04-29, the human owner waived final
installed-phone PWA signoff and real media thumbnail/video overlay proof as archive blockers.
...
- `packages/client/src/styles/typography.css` as the one explicit cross-shell carve-out: the
  global heading rule is repaired in Stage 1 and verified with a focused public-browser smoke on
  `/` and `/gardens`.
- The generated client PWA token audit and token-usage baseline only when they must be updated
  after implementation removes legacy runtime entries.
- Visual evidence under this plan hub's `handoffs/qa-evidence/` folder.
...
- Additional shared/client redesign beyond the accepted Stage 1-4 PWA source migration.

## Success Signal

The plan can close when automated design/client gates pass, stage-by-stage visual evidence is
recorded, the protected PWA baseline has no remaining in-scope rows, and any manual/device proof
gaps are either completed or explicitly waived by the human owner.
