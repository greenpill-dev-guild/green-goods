...
1. Run `node scripts/plan-hub.mjs list --agent codex --lane state_api --json`
...
6. Mark the lane in progress:
   - `node scripts/plan-hub.mjs set-lane --feature <feature-slug> --lane state_api --status in_progress --actor codex --branch codex/state-api/<feature-slug>`
...
9. Emit one run record to `.plans/_automation/runs/` with `node scripts/log-automation-run.mjs`, using the metric contract in `.plans/active/<feature-slug>/metrics.md` when present
