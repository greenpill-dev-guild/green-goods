# Garden Onboarding Call (Pass 2) — Mandate Confirmation & First Season
...
**Privacy**: contact details, wallet addresses, join identities, and statements naming people live in research-notes storage only. Never in Linear, analytics, or public artifacts. **This repository is public** — treat everything in this file as published. Our own support contact is the one exception to "never in Linear": it is deliberately recorded there, because gardens need it and it must not sit in a public repo.
...
**Protocol semantics** — *you do not need these on the call.* They are the exact contract terms behind the plain-language rules above, here so a mapped answer can be checked afterwards.
...
**Cover**
- The named support contact, how fast they answer, and when office hours run. **These are recorded in the Linear mirror, not here** — this repository is public, so support contacts are never stored in it. Read them from the mirror before the call.
- That contact is also who the survey told gardens to reach to retract their answers, so it has to be someone actually reachable.
...
- steward read-back: "Is this what you meant?"
...
- **Every focus garden becomes settlement-capable in August.** That is the whole of what a steward needs to hear here: no garden is waiting on another.

> **Facilitator context — do not say this on the call.** Internally, one garden is the first-execution hypothesis and evidence from these calls informs rollout order. Saying so turns the call into a ranking exercise and breaks the no-comparison guardrail above. The steward hears "your garden is capable in August", never "your garden is ahead of / behind another".
...
- **Early August** — Cycle 1 opens: steward-curated seeding from the July mandate artifacts, with baselines recorded first; first oracle-verified G$ settlement where relevant. (The August release milestone closes 2026-08-31.)
...
Run this over one completed survey response before the call. It drafts; the facilitator verifies. Paste the prompt, then the submission.

````
You are preparing for a Green Goods garden onboarding call. Below is one completed
survey response from a garden. Turn it into the facilitator's working pack.

Work in four phases and show each one.

PHASE 1 — EXTRACT
Quote the respondent verbatim throughout this phase. Never paraphrase, never tidy their
wording, and mark anything left blank as "not answered" rather than guessing.

  a) THE NEEDS LIST (2.1). Split it into individual needs — one per line as they wrote
     them. Keep their exact words, including anything that does not sound like a "need"
     to you. Number them N1, N2, N3...
  b) THE ONES THAT MATTER MOST (2.2). Prose covering two or three needs, each with what
     would change and when it matters. Attribute each fragment to a need from (a); if a
     need in 2.2 does not appear in 2.1, add it to the list rather than dropping it.
  c) How the needs relate to each other (2.3), if they said
  d) What is causing them (2.5), who feels them most (2.6), how the situation feels (2.7)
  e) Where this is heading in two or three years (2.8)
  f) Their first commitment (3.6), including which need it names, and its direction (3.7)
  g) Work that was not on our lists (3.5)
  h) What they cannot show yet (4.6)
  i) What we did not ask (5.8)
  j) The inputs later phases consume, quoted as given: capacity (3.8), the work grids
     3.1-3.4, what they track and where it lives (4.1, 4.4), who should record a baseline
     (4.7), the active-member band (1.6), roster inputs (1.4, 1.7, 1.8, 3.9), where G$
     should go (5.1), the consent flags (5.4, 5.5), and the redaction list (5.6)
  k) What they cannot show yet (4.6). This feeds no mandate field — it is evidence for
     PRD-719 (assessment deltas from accumulated work metrics). Keep it verbatim and route
     it there rather than into the pack.

PHASE 2 — MAP
First build a NEED RECORD for every need found in Phase 1. A need is something the
garden is trying to solve — it has no "kind" and no provider; those belong to the
commitments that answer it.
  statement       <- their words from 2.1 / 2.2
  desired outcome <- 2.2 for the two or three; "unresolved — ask on the call" for the rest
  horizon         <- 2.2 for the two or three; "ask on the call" for the rest

Then map the mandate fields for the commitment they described in 3.6:
  unit and quantity        <- 3.6, bounded by capacity in 3.8
  direction                <- 3.7: a Request (help asked for) or an Offer (help given).
                              This sets who confirms it was kept — Request creator, or
                              Offer recipient — and the provider never confirms their own.
  provider and beneficiary <- 3.6, 2.6
  linked need              <- the need named in 3.6, matched to a Need record above
  desired outcome          <- 2.2 for that need
  horizon                  <- 2.2 for that need
  domains and unit candidates <- the grids 3.1-3.4 (rows marked "Happens today" first,
                              then "Could happen with support")
  evidence                 <- 4.1 (what they already track), 4.4 (where it lives)
  baseline author          <- 4.7 top-ranked
  confirmation group scale <- 1.6
Where a field has no support in the answers, write "unresolved — ask on the call".
Never invent a value.

PHASE 3 — DRAFT
Produce these seven artifacts.

  0. THE NEEDS BOARD. Every Need record from Phase 2, in the garden's own words, with
     its outcome and horizon where known and "ask on the call" where not. This is the
     garden's portfolio and it is the point of the whole section — do not collapse it
     down to the one that became a commitment, and do not label needs as requests or
     offers: a need is a thing to solve, not a direction of help.

  1. THEORY OF CHANGE, IN THEIR WORDS. One short paragraph, assembled from answers they
     already gave — never invented, never generic. Use this shape and their vocabulary:

       "Because [cause, 2.5], [who feels it, 2.6] face [the needs, 2.1]. If [the
        commitment, 3.6] happens, then [outcome, 2.2] — and we would know because
        [indicator, 4.1 — only from the "We track this today" column, never from
        "We would like to track this"]. Over [horizon, 2.2], this builds toward
        [aspiration, 2.8]."

     If they described how the needs relate (2.3), add a sentence carrying that shape
     faithfully. Some portfolios are sequential ("X has to come first"), some are
     enabling ("X makes Y possible"), and some are reinforcing loops ("the hub and the
     teaching feed each other"). Do not turn a loop into a line because a line is easier
     to write. Where a tier is missing, write the sentence with the gap visible — "we do
     not yet know why this has persisted" — rather than filling it with something
     plausible.
  ...
  3. ONBOARDING ROSTER. From 1.4, 1.6, 1.7, 1.8, 3.9, 4.4: who becomes steward,
     gardener, or evaluator in the app; how many accounts to prepare; who needs the
     WhatsApp/SMS path instead of an app account (anyone on a feature phone or a
     shared device); what training to prepare; what existing records may need to move.

  4. READINESS ROW. Fields: Participation (confirmed / invited / not ready / unavailable
     — use "invited" unless the response states otherwise; "not ready" is a respected
     outcome and belongs here whenever the garden says it) · Need examples · Commitment rows
     (draft vs confirmed) · Domains · Evidence and confirmation · G$ rewards and where
     they should go (5.1) · Open decisions with an owner.

  5. GAP LOG. Every signal that our model did not fit their reality: anything in 2.1
     that resists being framed as something to solve, a horizon in 2.2 that does not fit
     week / month / Season / years, "Not sure yet" on 3.7 (they could not tell whether
     the commitment was help asked for or help given), work listed in 3.5, anything in
     5.8, and any place they wrote around our categories rather than inside them. Quote
     each one. A need or commitment we cannot classify is the most valuable thing here.

  6. OPEN QUESTIONS FOR THE CALL. The five most useful things to ask, ordered. Prefer
     questions that unblock a mandate row over general curiosity.

PHASE 4 — CHECK
State plainly: which mandate fields are still unresolved, whether capacity supports the
quantities drafted, and whether consent (5.4, 5.5) permits quoting this garden in
research summaries.

RULES
- Never score, rank, or compare this garden against any other, and never state a
  completeness percentage. This is one garden's picture, read on its own terms.
- Never infer readiness. Silence is not agreement.
- Keep contact details, wallet addresses, and any third-party names out of the readiness
  row and gap log — those live only in the research notes.
...
- 2026-07-19 (audit fixes) — closed defects found in a full review pass. Media paths corrected to `artifacts/visuals/`. 5.4 identified as **written-notes** consent rather than recording consent, with a stop-and-re-plan procedure when it is "No". The roster downgraded from "names and roles" to counts and roles, because no survey question collects names. Phase 1 extended to extract everything Phases 2-4 consume, and to route 4.6 to the assessment-deltas issue rather than leaving it orphaned. "Not ready" given a home in the readiness row. The theory-of-change indicator restricted to metrics the garden actually tracks. The G$ block stopped steering gardens toward circulation. Added: the retraction window, a thin-response fallback, the redaction list, output destinations, and a 07-28 last-call date.
- 2026-07-19 (legibility pass) — restructured to follow the order of use: **At a glance** summary table added; guardrails moved above the agenda, where "read before every call" actually lands; pre-call prep reordered so the synthesis prompt is step 3 and the retraction check is step 1; agenda `Cover` run-ons broken into bullets; **survey question numbers glossed inline** throughout, so the guide can be used without the survey open; media kit turned into a table mapping each asset to its block; protocol semantics separated out from facilitator language and marked "not needed on the call" (kept as plain markdown rather than a collapsed `<details>` block, so it survives the Linear mirror). Deduplicated the Celo sponsored-account sentence and the who-confirms rule, and standardised on "provider" for the latter. Timing stated honestly: the eight blocks sum to exactly 60, so book 75. **Fixed**: Appendix B artifact 2 said quantities must respect 3.7 (direction) where 3.8 (capacity) is meant; the owning-issue line repeated the current issue ID inside its own "formerly" parenthetical, where the superseded identifier belonged; `Updated` header lagged the change log.
- 2026-07-19 (public-repo pass) — this repository is public, so the support contact (a real person, their response time, and office hours) no longer has a placeholder here: block 7 now reads it from the Linear mirror, where it is recorded instead. The privacy header states the repo's public status outright and names that single deliberate exception to "never in Linear". Also closed a contradiction a review pass caught: the G$ talking points told the steward that "this call's evidence orders the rollout" and named the first-execution garden, seven lines after the guardrail forbidding cross-garden comparison. The rollout-ordering fact is now marked facilitator context and explicitly not to be said; the steward hears only that every focus garden is settlement-capable in August. (This entry deliberately avoids naming the superseded ID: Linear's mirror auto-links any bare issue identifier and resolves a renumbered one to its *current* issue, which would invert the sentence.)
