...
| # | Screen | User action | System response | State | If it fails |
|---|---|---|---|---|---|
| 1 | W3 (WF:101-119) | Picks "I need help / I can offer / I want to organize" | Maps to `NeedKind` 0/1/2 (SPEC:52; WF:122); no domain question at creation (SPEC:58) | draft (local) | — |
...
**(finding →)** Local-first transcription (Web Speech) has "es/pt offline not guaranteed" and a TAS-Android feasibility spike is *inside* the shared workstream (SPEC:231) — the flow degrades safely (audio-only) but the offline-transcription coverage for es/pt is an open field-evidence item, not a drawn state. Comprehension of Request/Offer/Initiative in es/pt without facilitation is research checkpoint #1 (JN:214) — the vocabulary is survey outcome #2's ship-or-revise subject (**→ P2-2, P2-3**).
...
```mermaid
flowchart LR
  A["W13 /gardens discovery"] -->|"filter kind/domain/horizon/status"| B["W13 need detail"]
...
| # | Screen | User action | System response | State | If it fails |
|---|---|---|---|---|---|
| 1 | W13 (WF:399-407) | Browses `/gardens`; filters kind/domain/horizon/status; order = Recent activity | Discovery lives in existing client public surfaces; **funding never a filter/sort/rank** (SPEC:279; WF:417) | public read | empty/partial honest (JN:107) |
...
**P1-3 · Fourth-garden public-naming authority is contradicted across same-class artifacts and is mid-reconciliation.** "Name it / decision #27 stands": `CP acceptance-matrix.md:49` (§3 **Public claim** matrix, authority "decision #27"), `CP status.json:93`, `CP plan.todo.md:9`, and the most-recent repo commit `dc53c66ca` ("fourth-garden naming stands (decision #27)"). "Anonymize / #27 superseded": `corrections-log.md:56` (this folder's latest same-day "Resolution": "superseding decision #27 for all instrument and external artifacts … the outreach target stays in internal decision records only") and `CP external-communications.md:128`. Both `external-communications.md` and `corrections-log.md` are **modified in the working tree** (another agent is editing). These are same audience-class (internal governance/claims artifacts), so per the review's fourth-garden rule this is a **reportable finding — not fixed either way**. **Community impact**: `survey-instrument.md:40` gates es/pt translation prep on the fourth-garden language being confirmed. **Resolve via Linear** (the cited authority is "the live Linear r4 named-cohort state") — see §21.
...
**P1-5 · External story understates September testimony and risks implying August testimony.** `CP external-brief.md:73` scopes the September app as "(view, signal, confirm)" — omitting "testify", which spec decision #8 enumerates as "view / signal / confirm / testify" (`spec.md:23`) and which the CI spec makes a first-class September action (`spec.md:259,261`; SB-6). Separately, `CP external-brief.md:23` folds "community testimony" into step 6 labelled "Planned — August/September", which CP decision #34g requires external copy must **not** imply as August (`CP plan.todo.md:83`). **Fix**: `CP-XB:73` add "testify"; `CP-XB:23` isolate testimony as September. **Lane**: docs/editorial.
...
**P2-2 · The needs vocabulary is undefined in the glossary.** `Need / Request / Offer / Initiative / signal / desired outcome / horizon` and the two-axis states are defined only in `CP external-communications.md:125` and `design-research.md:102` — **not** in `docs/docs/reference/glossary-community.md` (its Domain Entities / Term Reference registers carry no entry). Survey outcome #2 (`research-plan.md:13`) is a "ship-or-revise the needs vocabulary" gate, but the glossary (the canonical vocabulary home) has nothing to revise. **Lane**: docs.

**P2-3 · Survey outcome #2 → schema/copy loop lacks a named owner and a date before the contracts lane.** `research-plan.md:13` says outcome #2 "decides schema and creation-flow copy changes before the September community app". Schemas are append-only with immutable field order (`spec.md:41,181`), so a `NeedKind`/vocabulary change after registration is expensive. **Fix**: bind outcome #2 to a decision date **before** the `need-schemas` deploy (the contracts lane), not merely "before September". **Lane**: research/contracts.
...
4. **Fourth-garden naming unresolved** (P1-3) → survey/onboarding translation prep stalls (`survey-instrument.md:40`); public copy risk. Breaks the **research timeline** feeding schema/copy.
...
- **Fourth-garden authority** (P1-3): the cited tie-breaker is "the live Linear r4 named-cohort state" (`plan.todo.md:9`). Whether decision #27 stands or was superseded by the 07-11 owner review is a **Linear-current** fact; the repo shows both, mid-reconciliation.
...
- **Two-axis never collapsed**: enforced in copy across `spec.md:195`, `wireframes.md:169-170`, `CP external-communications.md:125`, `design-research.md:102`.
- **Retraction tombstone**: consistent across `spec.md:211` ↔ `diagrams.md:244` ↔ `wireframes.md:276-281` ↔ `journeys.md:66`.
- **No per-Need escrow / no rankings**: consistent across `spec.md:20,279` ↔ `CP acceptance-matrix.md:50-51` ↔ `CP external-brief.md:46,51` ↔ `journeys.md:108`.
