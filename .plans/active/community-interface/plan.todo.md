...
|---|---|---|---|---|
| Four Need schemas + two resolvers + refUID registration | `contracts` | [PRD-687](https://linear.app/greenpill-dev-guild/issue/PRD-687) | none (parent tracker) | ⏳ |
...
2. [ ] PRD-687 four exact schemas across two resolvers + deploy target: envelope-normalized Need/reference fields, directional signal, moderation/reopen, typed merge, funding chain/receipt contract; remains blocked only until step 1 clears
...
5. [ ] PRD-687 consolidated shared slice: envelope normalization, canonical directional signals, needs hooks, two-axis joined read, coalesced `needSignal` queue intent, `need`/`testimony`, waiting_for_hat recovery, funding verifier, voice spike
...
- [ ] Four schemas + two resolvers + need-schemas.ts per `spec.md` §3; bun wrappers only, never raw forge
...
- [ ] After shared-foundation proof: exact four-schema types/codecs, envelope normalization and parent joins, canonical latest directional-signal derivation, coalesced queue intent, two-axis joined read, funding verifier/export, and voice capture per `spec.md`; hooks stay in shared; locale keys en/es/pt
