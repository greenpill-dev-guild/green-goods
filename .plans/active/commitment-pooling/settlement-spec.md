...
creates a separate durable `settlement` job after canonical Fulfillment; the fulfillment
transaction never calls settlement synchronously. `queueDisbursement` is permissionless because
source, beneficiary, token, amount, rail, eligibility, and the one-live-disbursement invariant are
contract-derived. Garden claims settle to the registered `providerGarden` Safe without the
...
| `queueDisbursement(commitmentId)` | anyone; normal app path is the durable `settlement` job created after indexed Fulfillment | commitment `Fulfilled`; active owning-pool settlement account at `settlementAccounts[commitment.garden]`; no live disbursement; declared reward rail is exactly `CeloSettlement`, token equals configured G$, and source equals that owning-pool Celo Safe. `executorGarden = commitment.garden`. Individual beneficiary = stored provider same-address Celo AA account and requires `memberDeliveryEnabled`. Garden beneficiary = the separately active `settlementAccounts[providerGarden].account`, never the Arbitrum GardenAccount, and does not require the member-AA gate. Module derives source, beneficiary, token, and amount without caller overrides; event `DisbursementQueued`. A duplicate call against the same live commitment pointer reverts onchain; the job executor reconciles that exact existing pointer as success |
...
| Operator writes | Explicitly seed/top up a garden outside commitments; dispatch a queued command; retry the same command after transport delay; retry a stored Celo acknowledgment when fee/delivery recovers; create a new logical attempt only after authenticated execution failure. Fulfilled commitment rewards are queued by the app's durable permissionless `settlement` job, not by a second payment-approval control. | Once dispatched, timeout alone cannot cancel or requeue the payment. |
...
- **W10 commitment dialog**: `CeloSettlement` exposes automatic settlement eligibility/job status and recovery, never a second payment-approval button; `ArbitrumExternal` alone exposes "Record payout." Garden rewards show Safe-to-Safe readiness independently of member delivery, while Individual rewards show the AA gate. Batch actions remain in W21/W22.
- **Admin Operations tab funding view (deployer-gated)**: protocol-Safe inflow, explicit non-commitment "Seed garden" / top-up controls and resulting GG→garden funding hops, automatic fulfilled-commitment settlement rows, Safe balances, native ETH/CELO fee reserves, command/ack message IDs and explorer links, delivery/manual-execution guidance, Safe/Roles/cap health, and batch console.
