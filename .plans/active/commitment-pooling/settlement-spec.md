...
**Protocol-pool parity amendment (Decision Log #30, 2026-07-27)**: the protocol pool is the root
garden's ordinary commitment pool, not a separate funding machine. Fulfilled protocol-pool
commitments use the same `queueDisbursement(commitmentId)` path as every other pool. The app
may create a per-user, per-device `settlement` attempt only after canonical Fulfillment and the
complete indexed queue-readiness conjunction; the fulfillment transaction never calls settlement
synchronously. Every attempt/retry re-reads the permanent commitment pointer, which coordinates
competing apps globally. `queueDisbursement` is permissionless because
source, beneficiary, token, amount, rail, eligibility, and the one-disbursement-per-commitment
invariant are contract-derived. The commitment's disbursement pointer is permanent once set:
terminal cancellation preserves it, so neither the app nor another permissionless caller can
reopen that reward. Garden claims settle to the registered `providerGarden` Safe without the
member-AA gate; Individual claims require `memberDeliveryEnabled`. The steward-only
`queueFunding(garden, amount)` path remains in the initial version solely for discretionary,
non-commitment garden seeding/top-ups.

**What stays true from the locked register**: no bridged G$, ever. CCIP transports data only and receives no token amounts. Sarafu integration and transferable settlement vouchers stay deferred. One Celo Safe exists per garden (1:1 mapping, deployed on demand); the Green Goods protocol Safe is the direct House of Alignment receiving account; the only modeled Green Goods funding route is protocol → garden. The Celo executor is a narrowly scoped Zodiac Roles member, never a Safe owner and never an arbitrary-call bridge. Gardeners never initiate a cross-chain command in the field. If the Celo AA/paymaster spike fails, Garden-claim rewards and protocol → garden funding may continue Safe-to-Safe; only Individual reward delivery and member sends remain blocked. No broadcast is authorized by this spec, a milestone date, or a passing implementation test.
...
| 10 | `commitmentDisbursed` | `mapping(uint256 commitmentId => uint256 disbursementId)` (0 = never queued; immutable first-and-only reward-disbursement pointer once set, including after terminal cancellation) |
...
Failed commitment reward terminally closes that disbursement while preserving its permanent
`commitmentDisbursed` pointer and all command/outcome history. Cancelling a Queued batch does the
same atomically for every immutable member through `cancelBatch`; no queued member with a non-zero
`batchId` can be cancelled alone. `queueDisbursement` rejects every non-zero historical pointer,
including one whose disbursement is Cancelled, so a terminal close can never create a fresh
disbursement ID. A delay,
...
| `setMemberDeliveryEnabled(bool)` | module owner | enabling requires the Celo AA/paymaster exit evidence recorded in the settlement handoff; disabling blocks new Individual commitment-reward queues and member sends but never blocks Garden-claim rewards to registered Garden Safes or the non-commitment funding route |
| `queueDisbursement(commitmentId)` | anyone; normal app path is a signed-in app's per-user, per-device `settlement` attempt after indexed queue readiness | module unpaused; commitment `Fulfilled`; active owning-pool settlement account at `settlementAccounts[commitment.garden]`; `commitmentDisbursed[commitmentId] == 0`, meaning no reward disbursement has ever been queued; declared reward rail is exactly `CeloSettlement`, token equals configured G$, and source equals that owning-pool Celo Safe. `executorGarden = commitment.garden`. Individual beneficiary = stored provider same-address Celo AA account and requires `memberDeliveryEnabled`. Garden beneficiary = the separately active `settlementAccounts[providerGarden].account`, never the Arbitrum GardenAccount, and does not require the member-AA gate. Module derives source, beneficiary, token, and amount without caller overrides; event `DisbursementQueued`. Every duplicate call reverts with the permanent existing pointer, including after Cancelled. Before every send/retry, the app re-reads eligibility and that pointer; an exact winning pointer reconciles its local attempt, while any terminal pointer suppresses it permanently |
| `queueFunding(garden, amount)` | protocol steward or module owner | explicit non-commitment treasury seed/top-up only; never the normal payout path for a fulfilled protocol-pool commitment. The single modeled route is ProtocolToGarden, recorded on the disbursement's immutable `fundingRoute` fact; target garden must differ from `protocolGarden`; executorGarden is snapshotted as protocolGarden; source, recipient, and canonical G$ derive from funding config + active settlement accounts; no arbitrary addresses/tokens; event `DisbursementQueued(kind=Funding)` |
...
| `cancelDisbursement(id, reasonCID)` | steward | unbatched Queued (`batchId == 0`) or Failed only; records `cancelledFromState`, preserves failed attempt/failure history, creates no new execution key, and preserves the commitment's permanent disbursement pointer so the reward can never be queued again. A Dispatched subject cannot be cancelled merely because delivery or acknowledgment is late; event `DisbursementCancelled` |
| `cancelBatch(batchId, reasonCID)` | resolved batch steward | batch must be Queued; atomically marks the immutable batch and every member Cancelled-from-Queued, preserves the member list, and preserves each commitment's permanent disbursement pointer. No partial queued-batch cancellation and no cancelled member can be queued again; event `BatchCancelled` |
...
**Commitment reward binding.** Rail, source, token, and amount come from `commitment.reward`; callers supply no source/recipient/token/amount override. The rail must be `CeloSettlement`, which makes the core module's `recordRewardPaid` path unavailable for the same commitment. The declared source must equal the owning pool's active registered Celo Safe: `executorGarden = commitment.garden`. For the protocol pool this is the GG protocol Safe. Individual claims preserve the unit-provider beneficiary: Offer → creator, Request → accepted counterparty, using the same-address Celo AA route and requiring `memberDeliveryEnabled`. Garden claims resolve beneficiary to the separately active registered Celo Safe for `commitment.providerGarden`; the Arbitrum GardenAccount is attribution only and is never a Celo G$ recipient, and the Garden-Safe path does not depend on member AA. Token must equal configured `gDollarToken`. Canonical Fulfillment is the economic authorization for this fully derived payment; anyone may call `queueDisbursement`. The indexer exposes queue readiness only after every source/beneficiary precondition is true and no historical pointer exists. A signed-in app may then create at most one IndexedDB attempt keyed by `(chainId, commitmentId, userAddress)`. Each send or retry re-reads readiness and the permanent pointer; duplicate-race losers refetch and reconcile to the winner, terminal pointers suppress all later attempts, and local exhaustion is never a global reward state. Confirmation and dispute resolution never call settlement synchronously, so settlement pause, route, fee, account failure, or device failure cannot roll back Fulfillment. A permissionless caller, including an agent or keeper, gains no privileged or value-selection authority because every payment fact is contract-derived. Funding top-ups remain explicit non-commitment disbursements.

**Funding-route binding.** `queueFunding` is the initial-version treasury exception for seeding or topping up a garden outside a commitment; it is not the protocol pool's earned-reward path. It never accepts source, recipient, or token. The single modeled route `ProtocolToGarden` stores source = the protocol settlement account, recipient = the target garden settlement account, garden = target garden, and immutable `executorGarden = protocolGarden`. `protocolGarden` and `gDollarToken` are write-once initializer facts with no setter, so queued and future funding commands cannot drift from the Celo executor's immutable canonical token. Both accounts must be active, amount must be non-zero, and token is always `gDollarToken`. HoA → protocol Safe is recorded in external treasury reporting, not fabricated as a module action Green Goods did not authorize. Because this path has no commitment-level fulfillment authorization, initiation stays protocol-steward/module-owner only; no agent or keeper receives queue or value discretion.
...
7. Queued.
8. No historical disbursement pointer and any canonical queue prerequisite is false → blocked availability state with a bounded reason and no local attempt/retry. `memberDeliveryEnabled == false` is the Individual-only gate reason; it never applies to Garden rewards.
9. No historical disbursement pointer, indexed queue readiness true, and this device's `settlement` attempt queued/retrying → “arranging settlement.”
10. No historical disbursement pointer, indexed queue readiness still true, and this device's attempt exhausts → local arrangement failure with explicit retry; Fulfilled remains unchanged. Retry returns through pointer + eligibility checks.

Changing the delivery gate never hides an already queued or historical result. The gate controls
new Individual commitment-reward queues and member wallet sends. It never blocks Garden rewards
to registered Garden Safes or the non-commitment funding route. Every existing settlement renders
from its own canonical or derived state.
...
- Gating tests: `queueDisbursement` succeeds for any caller only when all derived reward facts pass
  and no historical commitment pointer exists; `queueFunding` rejects non-steward/non-owner
  callers; dispatch and command retry reject callers outside the steward/module-owner/dispatcher
  set; source pause blocks queue/batch/dispatch/
  ...
  disabling member delivery blocks only Individual commitment-reward queues and member sends,
  never Garden-claim rewards to registered Garden Safes or the funding route.
...
- **Failure behavior**: if the spike fails, `memberDeliveryEnabled` remains false. Garden-claim commitment rewards may still settle Safe-to-Safe, and discretionary ProtocolToGarden seeding/top-ups may continue. Individual commitment-reward queueing, automated member delivery, and member G$ sends remain blocked. There is no alternate member-delivery path.
...
| Operator writes | Explicitly seed/top up a garden outside commitments; dispatch a queued command; retry the same command after transport delay; retry a stored Celo acknowledgment when fee/delivery recovers; create a new logical attempt only after authenticated execution failure. Fulfilled commitment rewards are queued permissionlessly by a signed-in app's per-device `settlement` attempt, not by a second payment-approval control or operator action. | Once dispatched, timeout alone cannot cancel or requeue the payment. |
...
- **W21 Garden Pool tab settlement section (delta to W7)**: garden-scoped settlement account card (Safe address, active, cap snapshot, plus read-only member-delivery status) + detail/recovery queue. The garden-scoped CCIP command/ack console is **W22** and distinguishes retrying the same command from retrying a stored acknowledgment or creating a new attempt. W24 remains the primary all-garden execution home.
- **W10 commitment dialog**: `CeloSettlement` exposes indexed queue readiness, no-retry blockers, this device's attempt status, and recovery, never a second payment-approval button; `ArbitrumExternal` alone exposes "Record payout." Garden rewards show Safe-to-Safe readiness independently of member delivery, while Individual rewards show the AA gate. Batch actions remain in W21/W22.
- **Admin Operations tab funding view (capability-gated)**: `showOperations` follows deployer, funding, or settlement-operation capability, while the funding submit itself requires current protocol-steward or SettlementModule-owner authority and never treats deployer as sufficient. The view shows protocol-Safe inflow, explicit non-commitment "Seed garden" / top-up controls and resulting typed Funding rows with no commitment ID, app-submitted fulfilled-commitment settlement rows, Safe balances, native ETH/CELO fee reserves, command/ack message IDs and explorer links, delivery/manual-execution guidance, Safe/Roles/cap health, and batch console.
...
7. **Denominator risk from `memberDeliveryEnabled`.** Individual member delivery is gated on the Celo AA/paymaster spike; if it fails, `memberDeliveryEnabled` stays false and new **Individual** commitment-reward queueing plus member G$ sends are blocked. Garden-claim rewards to active registered Garden Safes and non-commitment `ProtocolToGarden` funding continue Safe-to-Safe. Therefore the aggregate "total G$ paid out" denominator is not necessarily near-empty, but it may contain no Individual-member payouts and cannot support a member-circulation claim. Report beneficiary cohorts separately; if the required cohort denominator is absent, that metric is **Unavailable** rather than inferred from Garden funding.
