...
**Updated**: 2026-07-21 (all 12 SVGs use document-scale type, contrast-safe Warm Earth roles, and accessible metadata; crowded role/funding cards are reflowed; the ownership map matches the six-tab external document; Follow On / Hardening and the Community/evidence checkpoint close separately in parallel on September 30)
...
| `external-brief-money-map.png` | Arbitrum proof states above Celo G$ movement, with Reported separated from Oracle-verified and an explicit rule that G$ never bridges to Arbitrum. |
...
| `rollout-settlement-states.png` | Settlement flow from Queued to Executing to Reported, then Oracle-verified or Failed after a Chainlink Functions receipt check, with no human verification path. |
