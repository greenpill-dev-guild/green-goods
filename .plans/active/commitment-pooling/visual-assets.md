...
**Updated**: 2026-07-27 (open-questions findings pass — the Reference tab's seven questions were audited against the frozen specs, the Linear decision record, and the shipped code, and the panel now carries a verdict + grounded finding + citations per question; wireframes.md was fully reconciled to the hi-fi registry (21 of 25 frames corrected, per-frame `#screens/SCREEN@state` deep links, blanket "pending sync" badge retired in favour of six explicit wireframe-only tags); the Functions-era "verification attempts" phrase in the coverage matrix was corrected. Prior pass 2026-07-25: architecture coherency — Architecture tab reconciled to the frozen specs, densest diagrams split, five undrawn surfaces added.)
