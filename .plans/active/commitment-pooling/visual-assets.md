...
**Updated**: 2026-07-28 (PRD-759 protocol-pool parity merge-review clarification — commitment-earned Celo rewards become queue-ready only from the complete indexed prerequisite set; a signed-in app may create one per-user/per-device `settlement` attempt, while the permanent onchain pointer coordinates every caller. `queueFunding` is retained only for explicit non-commitment garden seeds/top-ups and requires current protocol-steward or SettlementModule-owner authority; deployer alone cannot submit. The Reference tab records this as answered, while wireframes and D12 distinguish the two paths. Earlier 2026-07-27 pass: the seven open questions were audited; wireframes.md was reconciled to the hi-fi registry.)
**Gallery**: [Commitment Pooling — Visual Asset Gallery](https://claude.ai/code/artifact/007ef090-9e26-4b1d-898c-615155304d9d), built by `visual-assets-artifact.build.ts` in this folder. The story tab covers all 12 hand-drawn assets — loop, roles, the Use Cases journey strip, money map, settlement states, funding rails, flywheel, three tiers, circular G$, GE protocol functions, rollout timeline, and ownership map. The Architecture tab is generated wholly from `diagrams.md` (**34 Mermaid blocks across 23 named D-sections**) and the Screens tab from `wireframes.md`. A fourth **Reference** tab (added 2026-07-27) carries the deep material that used to open the Architecture tab — the `diagrams.md` preamble, the 29-asset coverage matrix, and the ship-time appendix — plus the **Open questions** panel, which since the same day's findings pass records a verdict (Answered / Answered · one open gap / Decision needed), a grounded finding, citations, and — where the call is genuinely open — a recommendation or the competing postures for each of the seven questions. Rebuild and validate locally before republishing to the same URL; an unavailable artifact publisher is a publication blocker, not proof that the live URL changed.
...
1. `visual-assets-artifact.build.ts` writes the two build targets — four audience tabs: story; **23 named Architecture sections D1–D16** (including D1b, D7b, D7c, D7d, D10b, D11b, D13b) rendered as **34 Architecture Mermaid blocks** behind a hand-written opener; the canonical CP screen states, reconciled to the hi-fi registry with per-frame deep links (six frames carry an *includes wireframe-only detail* tag); and Reference with the audited Open-questions findings panel.
