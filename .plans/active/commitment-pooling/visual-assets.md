...
**Updated**: 2026-07-28 (group-commitment amendment — D17, W2b, repeatable requirements, contribution-derived recognition, explicit garden retention, and contributor child payouts are now in the generated gallery; wireframes.md has 26 frames with per-frame hi-fi deep links. The Reference panel also records the settled Needs architecture: four schemas across `NeedsResolver` and `FundingAttributionResolver`, EAS recipient/refUID normalization, and directional `bool support` signals.)
**Gallery**: [Commitment Pooling — Visual Asset Gallery](https://claude.ai/code/artifact/007ef090-9e26-4b1d-898c-615155304d9d), built by `visual-assets-artifact.build.ts` in this folder. The story tab covers all 12 hand-drawn assets — loop, roles, the Use Cases journey strip, money map, settlement states, funding rails, flywheel, three tiers, circular G$, GE protocol functions, rollout timeline, and ownership map. The Architecture tab is generated wholly from `diagrams.md` (**36 Mermaid blocks across 25 named D-sections, D1–D18**, each footnoted with its own colour key; every ERD carries a cardinality key) and the Screens tab from `wireframes.md`. A fourth **Reference** tab carries the deep material that used to open the Architecture tab — the `diagrams.md` preamble, the 32-asset coverage matrix, the D13b permission table, and the ship-time appendix — plus the audited **Open questions** panel, including the resolved Needs schema/resolver decision. Rebuild and validate locally before republishing to the same URL; an unavailable artifact publisher is a publication blocker, not proof that the live URL changed.
...
1. `visual-assets-artifact.build.ts` writes the two build targets — four audience tabs: story (narrative opener + 12 assets); **25 named Architecture sections D1–D18** (including D1b, D7b, D7c, D7d, D10b, D11b, D17b) rendered as **36 Architecture Mermaid blocks** behind a hand-written opener (D10b is a stored-state strip + derivation table; D16.1/D16.2 are tables; every diagram carries an auto-derived colour key and every ERD a cardinality key); the canonical CP screen states, reconciled to the hi-fi registry with per-frame deep links (six frames carry an *includes wireframe-only detail* tag); and Reference with the coverage matrix, the D13b permission table, and the audited Open-questions findings panel.
