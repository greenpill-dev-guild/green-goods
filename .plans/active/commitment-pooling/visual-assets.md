...
**Updated**: 2026-07-27 (Needs architecture decision converged: the Reference panel now records four schemas across `CommunityNeedsResolver` + `FundingAttributionResolver`, EAS recipient/refUID normalization, and directional `bool support` signals as Answered; the seven-question summary is now five answered, one answered with an open gap, and one decision. Same-day prior pass: all questions were audited against the frozen specs, Linear decision record, and shipped code; wireframes were reconciled to the hi-fi registry.)
**Gallery**: [Commitment Pooling — Visual Asset Gallery](https://claude.ai/code/artifact/007ef090-9e26-4b1d-898c-615155304d9d), built by `visual-assets-artifact.build.ts` in this folder. The story tab covers all 12 hand-drawn assets — loop, roles, the Use Cases journey strip, money map, settlement states, funding rails, flywheel, three tiers, circular G$, GE protocol functions, rollout timeline, and ownership map. The Architecture tab is generated wholly from `diagrams.md` (**35 Mermaid blocks across 24 named D-sections**) and the Screens tab from `wireframes.md`. A fourth **Reference** tab carries the preamble, coverage matrix, ship-time appendix, and seven-question Open questions panel. The panel now resolves the Needs architecture as Answered and retains only the genuinely open calls as decisions. Rebuild and validate locally before republishing to the same URL; an unavailable artifact publisher is a publication blocker, not proof that the live URL changed.
...
1. `visual-assets-artifact.build.ts` writes the two build targets — four audience tabs: story; **24 named Architecture sections D1–D16** (including D1b, D3a, D7b, D7c, D7d, D10b, D11b, D13b) rendered as **35 Architecture Mermaid blocks** behind a hand-written opener; the canonical CP screen states, reconciled to the hi-fi registry with per-frame deep links (six frames carry an *includes wireframe-only detail* tag); and Reference with the audited Open-questions findings panel.
