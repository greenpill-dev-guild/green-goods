...
**Gallery**: [Commitment Pooling — Visual Asset Gallery](https://claude.ai/code/artifact/007ef090-9e26-4b1d-898c-615155304d9d), built by `visual-assets-artifact.build.ts` in this folder. The story tab covers all 13 hand-drawn assets — loop, roles, the Use Cases journey strip, money map, settlement states, funding rails, flywheel, three tiers, circular G$, GE protocol functions, the three-layer framing, rollout timeline, and ownership map. The Architecture tab is generated wholly from `diagrams.md` (**38 Mermaid blocks across 26 named D-sections, D1–D19**, including the new D19 bilateral-exchange sequence; each is footnoted with its own colour key and every ERD carries a cardinality key) and the Screens tab from `wireframes.md`. A fourth **Reference** tab carries the deep material that used to open the Architecture tab — the `diagrams.md` preamble, the 33-asset coverage matrix, the D13b permission table, and the ship-time appendix — plus the audited **Open questions** panel, including the resolved Needs schema/resolver decision. Rebuild and validate locally before republishing to the same URL; an unavailable artifact publisher is a publication blocker, not proof that the live URL changed.
...
| `artifacts/visuals/synthesis-ge-protocol.svg/.png` | **02 GE Learnings & Full Flywheel** | at §1.1 | Six functions with per-function v1 status: curation/limitation/repair full; valuation recorded; exchange reference + bilateral paired acceptance shipped; route partial; multilateral/transferable execution reserved |
| `diagrams.md` D19 (Mermaid gallery source) | **Visual Asset Gallery — Architecture** | after D18 | Bilateral exchange creation reference → atomic paired acceptance → two independent fulfillment/confirmation lanes, with the no-coupling boundary explicit |
...
- ✅ **`synthesis-ge-protocol` redrawn 2026-07-20; recalibrated 2026-08-01.** The 07-20 redraw retired the four-part quadrant and rendered six equally visible functions. The 08-01 recalibration (Grassroots Economics review response and exchange wave, plan registers #73–#75) adds explicit August/current-roadmap/later-roadmap status, the paired-start exchange rung, and more card padding, so the asset no longer implies all six are equally implemented or clips its subtitle. The stale "shown equally" PNG must not be re-uploaded; regenerate before any manual upload.
- ✅ **`synthesis-three-layers` added 2026-08-01.** NET-NEW three-layer framing presents one Commitment Pooling system: August coordination foundation / outcome-linked support / later reciprocal routing and exchange. It carries borrow-and-repay as a gated August companion rather than a deferred follow-on. Needs its first manual PNG upload to GDoc tab 02.
...
1. `visual-assets-artifact.build.ts` writes the two build targets — four audience tabs: story (narrative opener + 12 assets); **27 named Architecture sections D1–D19** rendered as **39 Architecture Mermaid blocks** behind a hand-written opener (D16 splits creation/lifecycle from settlement/offline recovery; D19 is the bilateral exchange sequence; every diagram carries an auto-derived colour key and every ERD a cardinality key); the canonical CP screen states, reconciled to the hi-fi registry with per-frame deep links (six frames carry an *includes wireframe-only detail* tag); and Reference with the coverage matrix, the D13b permission table, and the audited Open-questions findings panel.
