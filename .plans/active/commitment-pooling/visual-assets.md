...
**Updated**: 2026-07-27 (PRD-759 protocol-pool parity decision — commitment-earned Celo rewards now use an automatic durable `settlement` job and permissionless derived `queueDisbursement`; manual `queueFunding` is retained only for explicit non-commitment garden seeds/top-ups. The Reference tab records this as answered, while wireframes and D12 distinguish the two paths. Earlier same-day pass: the seven open questions were audited; wireframes.md was reconciled to the hi-fi registry.)
