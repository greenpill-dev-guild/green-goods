...
1. `visual-assets-artifact.build.ts` writes the two build targets — four audience tabs: story; **23 named Architecture sections D1–D17** (including D1b, D7b, D7c, D7d, D10b, D11b) rendered as **33 Architecture Mermaid blocks** behind a hand-written opener (D10b is a stored-state strip + derivation table; D16.1/D16.2 are tables); the canonical CP screen states, reconciled to the hi-fi registry with per-frame deep links (six frames carry an *includes wireframe-only detail* tag); and Reference with the coverage matrix, the D13b permission table, and the audited Open-questions findings panel.
