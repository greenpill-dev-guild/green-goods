...
|---|---|---|---|---|
| 1 | Maria chooses A from eligible same-pool Offers: A is still Offered, uses Individual claim type, has reserved capacity, and belongs to a creator other than Maria | creation draft stores A as B's `counterCommitmentId`; W28 excludes accepted, lapsed, self-owned, non-Individual, and capacity-inconsistent rows | W28 selected | clear the selection or choose another Offer |
| 2 | Maria submits B | immediately before enqueue/broadcast, the executor re-reads A and repeats every Offered×Offered eligibility predicate; only then may `CommitmentCreated(B, counterCommitmentId=A)` occur | W29 Proposed, or W28 selection-invalid with no B created | if A changed, stop before creation and require Maria to clear or replace the selection |
