...
When a Fulfilled commitment has a G$ reward, the planned SettlementModule records control state on Arbitrum while canonical G$ remains on Celo. The House of Alignment funds the Dev Guild working-capital wallet upstream. Green Goods models only WorkingCapitalToProtocol and ProtocolToGarden downstream funding routes. A reported transaction becomes oracle-verified only through the mandatory Chainlink Functions callback; there is no manual verification path. If the Celo account-abstraction/paymaster gate fails, protocol and garden funding may continue, but automated member delivery remains blocked and no garden-held member-claim path is introduced.
...
- Say House of Alignment funds reach the Dev Guild working-capital wallet. Green Goods models only WorkingCapitalToProtocol and ProtocolToGarden onward; do not describe an unconfirmed automatic allocation. When provenance is stated, state it precisely: GIP-26 failed its DAO vote (June 2026), so the Good Labs Foundation funds the pilot directly — roughly $800/month in G$ per member via a Flow Splitter on Flow State, first evaluation 2026-09-30.
...
- The fourth pilot garden, Barichara Regenerativa, was selected 2026-07-11 as the mature MRV-adoption anchor (decision #27). Selection is not participation: RESR-62 records confirmed/incomplete/unavailable per garden, and no external claim states Barichara participates until its corrected mandate is confirmed.
