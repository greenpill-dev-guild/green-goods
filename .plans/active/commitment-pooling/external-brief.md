...
**Last aligned with canonical synthesis**: v5 (2026-07-05) — [Commitment Pooling × Green Goods — Grassroots Economics Learnings, and the Full Flywheel We're Building](https://linear.app/greenpill-dev-guild/document/commitment-pooling-green-goods-grassroots-economics-learnings-and-the-d3939e890b14). **Corrections applied since v5**: the fund topology (House of Alignment streams directly to the Green Goods protocol Safe on Celo — the earlier working-capital hop is retired) and per-action required-work counts, both from the 2026-07-18 audit response (`corrections-log.md` §9–10).
...
- **Nobody holds your money.** The pooling module never takes custody of rewards. In August, G$ payments are executed by Garden Stewards from garden-owned accounts; automating that execution is a stretch goal (otherwise September).
...
Green Goods is one of four members of GoodDollar’s **House of Alignment** ecosystem-alignment program, alongside Gardens, ReFi DAO, and Textile. [GIP-26](https://discourse.gooddollar.org/t/gip-26-begin-distributions-to-house-of-alignment/8890) — the proposal to fund the program from GoodDollar's reserve distributions — **failed its community vote**. The **Good Labs Foundation stepped in to fund the initial season directly**: roughly **$800/month in G$** per member via a Flow Splitter on Flow State, with a **first evaluation on 2026-09-30**, when each member reports on how its G$ was used and what community impact followed. Funds stream directly into the Green Goods protocol treasury on Celo. From there, Green Goods models exactly one route: protocol treasury to garden accounts. Every focus garden is settlement-capable — each gets its own garden-owned Celo account on demand — with Tech and Sun Hub planned as the first execution. Each garden pairs settlement with a local way to spend G$ back into the pool, so value circulates instead of draining out.

## Timeline

| Date | Checkpoint | What ships | What gardens experience | Proof we hold ourselves to | Money and settlement |
|---|---|---|---|---|---|
| 2026-07-31 | Dry run | Confirmed garden mandates, facilitation kit, and a readiness matrix published with explicit gaps | Research and co-design sessions with Garden Stewards; mandate read-back and correction; today’s built rails only | Steward-confirmed mandate artifacts — or an explicit incomplete/unavailable status; no launch claim | Rewards via existing community-fund rails only; no G$ moves |
| 2026-08-31 | Release | Pooling module and non-transferable register live on Arbitrum; promise, claim, and confirmation surfaces; steward-executed G$ settlement path | First commitments seeded from confirmed mandates; promises taken up, evidenced, and confirmed in the app | One bounded end-to-end proof with every external gate passing; settlement claims labeled *Reported* or *Oracle-verified* | Every focus garden settlement-capable (one Celo account each); Tech and Sun Hub planned first |
| 2026-09-30 | Community app | Independent community app (view, signal, confirm); steward triage in admin; funder discovery on existing public pages | Members name Requests, Offers, and Initiatives themselves and confirm promises; gathering usability sessions | Observed task completion and recovery paths in real sessions | First House of Alignment evaluation |
