...
**Authority**: plan Decision Log #42 / register #76
...
- a transferable settlement-voucher wrap around fulfilled commitment classes;
...
3. **Multilateral transferable exchange, later scope.** A separately authorized voucher layer may
   wrap eligible fulfilled classes, quote relative values, apply holding and class limits, and
   exchange through either a Green Goods venue or an approved external venue.
...
The reserved `settlementAdapter` points to a separate token contract. It reads
`fulfilledOf(account, classId)` and `committedOf(account, classId)` from
`CommitmentRegistry`, while the registry remains the authoritative non-transferable record of
promises and exposes no transfer or approval functions.
...
The current `classId == commitmentId` convention remains valid. The `uint256` class space can add
class kinds later, including per-lead-provider and exact-unit-label groupings, without migrating
the base registry. The wrapper must preserve `poolId` continuity so a voucher can always be traced
to the pool and fulfilled class that authorized it.
