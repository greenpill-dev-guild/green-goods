...
**What stays true from the locked architecture**: the module **never custodies funds** (`contract-spec.md` §3 decision 8); the register exposes no transfer surface (`contract-spec.md` §6.2 Interface); and the `SettlementModule` is a records-only control plane (`settlement-spec.md` §2–3). `CreditRegister` is the same shape: it **records and gates, it never holds capital**. Disbursement rides the existing rails. Interest-free. No revolving corpus. No per-borrower credit score, ever.
...
Credit stats use **numerator/denominator only** (no floats, no leaderboards; same discipline as `promiseKeptRate`, `contract-spec.md` §8.4), on a sibling `CreditPoolStats`: `creditIssued`, `creditRepaid`, `creditOutstanding`, `repaymentRate`, and `defaultRate`. `newOutstanding` rides every mutating event so handlers never re-sum. `bun codegen` follows schema/config edits.

## 7. Surface impact (deltas to `uiux-spec.md`; wireframes remain an explicit post-unblock deliverable)
...
- **Editorial / community:** aggregates only (`creditIssued`/`repaymentRate` at pool/garden level); **never** per-borrower listings (decision #21; `uiux-spec.md` §7.2 privacy rules).
...
Runs **after** the pooling interface freezes (`contract-spec.md` §10, packages/contracts PR chain 2) and alongside/after the `SettlementModule` for the G$ leg. `status.json` tracks this as a **blocked follow-on lane**: no implementation starts without an explicit unblock. **No re-upgrade of the pooling module or register** — exactly how settlement was added (`settlement-spec.md`, plan decision #14).
