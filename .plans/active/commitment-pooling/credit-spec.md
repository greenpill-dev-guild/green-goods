...
- **Repayment (up, no rail):** **record-only on Arbitrum** — `CreditRegister.recordRepayment(loanId, amount, celoTxHash)`, exactly the shape of `recordSettled`: an operator records an already-executed Celo tx by its hash; the module never moves G$ and never calls Celo. The member sends G$ back to the garden Safe on Celo as an explicit online wallet action (`transfer`, `settlement-spec.md:130`); `executionRef` ties the record to the Celo tx for auditors. **No bridge value authority is introduced** — consistent with the locked "no bridge until operator burden binds" rule (`settlement-spec.md:9,114`).
...
Runs **after** the pooling interface freezes (PR chain 2, `contract-spec.md:1260`) and alongside/after the `SettlementModule` (chain 2.5) for the G$ leg. `status.json` tracks this as a **blocked follow-on lane**: no implementation starts without an explicit unblock. **No re-upgrade of the pooling module or register** — exactly how settlement was added (`settlement-spec.md`, plan decision #14).
