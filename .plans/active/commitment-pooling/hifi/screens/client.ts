...
// The pool's charter, in the garden's own words. The client-side twin of the
// steward's "What this pool is for" field, which the console has shown every day
// while the member's own pool tab said nothing: the one sentence that explains
// what any of this is lived on W1@empty-open alone, which is the state almost
// nobody sees twice.
//
// It reads as the commitments list's subtitle, not as a card. Round 8 removed a
// 236px summary card from exactly this gap for putting a summary between a
// member and the commitments they came for, and that decision stands. This is
// one line under a section header.
const W1_CHARTER =
  `<div class="t-meta">Neighbours here offer help and ask for it: rides, tools, workshops and garden work. What you commit to is kept in the open, and the person you did it for confirms it.</div>`;
  ...
  switch (state) {
    ...
    case "paused":
      content = pagepad(
        ...
        offerCard({
          readOnly: true,
          readOnlyNote: "New participation is paused. Proof, linked work, and recovery remain available inside the commitment.",
    ...
    case "campaign-tools":
      content = pagepad(
        banner("Tool library is under review. Proof and confirmations stay available.", "stone", "eye-line"),
    ...
    case "empty-open":
      content = pagepad(
        ...
        barPair(hot("w1.offer", btn("Offer", { kind: "pri" })), hot("w1.request", btn("Request", { kind: "sec" }))),
      );
      break;
    case "funded-offer":
    ...
    case "reviewing":
      content = pagepad(
        banner("Your stewards are reviewing this season. You can still add proof and confirm commitments.", "stone", "eye-line"),
        seasonCard({ stage: "Reviewing" }),
        offerCard({
          readOnly: true,
          readOnlyNote: "This commitment is ready for your confirmation. Other commitments still accept proof while the season is reviewed.",
...
const W1_HOTS: HifiDef["hots"] = {
  ...
  "w1.campaign-tools": { l: "Open Tool library campaign", to: "screen:W1@campaign-tools", info: "A Reviewing campaign stays independently browseable and keeps proof and confirmation available (UX:74,127)." },
  ...
  if (state === "people" || state === "people-ended")
    return phoneFrame(
      `${head}${w1cTabs("people", ended)}${pagepad(
        ...
        // they played on this season, which is public membership. A per-person
        ...
        banner("Shared memory of this season, for context rather than as a score. Each person's own record stays between them and their stewards.", "stone", "information-line"),
      )}`,
      { appBar: false },
    );
  ...
  if (state === "insights" || state === "insights-ended")
    return phoneFrame(
      `${head}${w1cTabs("insights", ended)}${pagepad(
        ...
        sectionCard("How credit is shared", `${detailRow("Shared equally", "35% among everyone who took part")}${detailRow("By verified contribution", "65% from approved work and proof")}`),
...
const W1C_HOTS: HifiDef["hots"] = {
  "w1c.tab-commitments": { l: "Commitments tab", to: "screen:W1C@season", info: "Every commitment belonging to this season, whatever state it reached — a kept commitment is part of the season's record rather than something that vanishes from it." },
  "w1c.tab-commitments-ended": { l: "Commitments tab", to: "screen:W1C@season-ended", info: "The finished season's commitments, read-only." },
  "w1c.tab-people": { l: "People tab", to: "screen:W1C@people", info: "Who is taking part and the role they play on this season. D.3 permits the roster and the role; a per-person record — kept, lapsed, received — stays between that member and their stewards and is never a percentage, a grade, or a comparison." },
  ...
  "w1c.filters": { l: "Filter row", info: "The SAME control the pool tab carries (2026-08-17, Afo: \"we have a filter selection on the title level we should use that\"): direction chips plus the personal Mine toggle, where All means everything in this season — open and finished alike. A cycle's list is a scope over the same records, so it takes the same filter rather than a second axis of its own." },
...
const W2_STATES = [
  ...
  ["captured-evidence-queued", "Recorded — proof queued"],
  ["captured-evidence-submitted", "Recorded — proof in"],
  ...
  ["request-withdraw-confirm", "Withdraw your request — confirm"], ["request-withdrawn", "Request withdrawn"],
  ...
  ["campaign-request-evidence-queued", "Campaign request — proof queued"],
  ["campaign-request-evidence-submitted", "Campaign request — proof in"],
  ...
  ["request-evidence-queued", "Request — proof queued"], ["request-evidence-submitted", "Request — proof in"],
...
const w2StateChip: Record<W2ChipState, string> = {
  ...
  "active-waiting": "Active", contributor: "Active", "send-confirm": "Proof in",
  ...
  "captured-evidence-queued": "Active", "captured-evidence-submitted": "Proof in",
  "captured-ready-pending": "Proof in", "captured-ready-confirmer": "Ready to confirm",
  ...
  "request-withdraw-confirm": "Requested", "request-withdrawn": "Withdrawn",
  ...
  "campaign-request-evidence-queued": "Active", "campaign-request-evidence-submitted": "Proof in",
  "campaign-request-ready-pending": "Proof in", "campaign-request-ready-confirmer": "Ready to confirm",
...
const W2_REQUEST = new Set<W2State>([
  ...
  "request-withdraw-confirm", "request-withdrawn",
]);
...
const W2_PHASE_GROUPS: [CommitmentLifecycle, W2State[]][] = [
  ...
  ["Requested", ["requested", "browse-requested", "browse-requested-gated", "request-withdraw-confirm"]],
  ...
  ["Cancelled", ["cancelled", "withdrawn", "request-withdrawn", "support-cancelled"]],
  ["Expired", ["expired"]],
...
const w2NeverClaimed = (state: W2State) => w2PreAcceptance(state) || state === "withdrawn" || state === "request-withdrawn";
...
const W2_SEAT_GROUPS: [Seat, W2State[]][] = [
  ...
  ["confirmer", [
    ...
    "requested", "request-withdraw-confirm", "request-withdrawn",
    "active-waiting",
...
function w2Moments(state: W2State, overrideNote: boolean): Moment[] {
  ...
  if (state === "request-withdrawn")
    return [
      { label: "Requested", meta: "you · Jul 2" },
      { label: "Withdrawn", meta: "you · Jul 9", note: "\u201csorted another way. Luz drove me on Saturday\u201d", warn: true, open: true },
    ];
  if (state === "requested" || state === "request-withdraw-confirm" || state === "browse-requested-gated")
  ...
  if (W2_CAMPAIGN_REQUEST.has(state)) {
    ...
    if (state === "campaign-request-evidence-queued")
      return [...campaignAsked, { label: "Proof queued", meta: "saved on this device · waiting to send", open: true }];
    if (state === "campaign-request-evidence-submitted")
      return [...campaignAsked, { label: "Proof in", meta: "photo from the market · Jul 6", open: true }];
    if (state === "campaign-request-ready-pending")
      return [...campaignAsked, { label: "Proof in", meta: "photo from the market · Jul 6" }, { label: "Readiness queued", meta: "saved on this device · waiting to send", open: true }];
    if (state === "campaign-request-ready-confirmer")
      return [...campaignAsked, { label: "Proof in", meta: "photo from the market · Jul 6" }, { label: "Ready to confirm", meta: "waiting on Ana", open: true }];
    if (state === "campaign-request-confirmation-pending")
      return [...campaignAsked, { label: "Proof in", meta: "photo from the market · Jul 6" }, { label: "Confirmation queued", meta: "Ana · saved on this device", open: true }];
    ...
    return [...campaignAsked, { label: "Proof in", meta: "photo from the market · Jul 6" }, { label: "Commitment kept", meta: "confirmed by Ana · Jul 6", open: true }];
  }
  if (W2_REQUEST_WORK.has(state)) {
    ...
    return [...askedWork, { label: "Work approved", meta: "2 of 2 · Jul 14" }, { label: "Ready to confirm", meta: "waiting on Ana, who asked for this", open: true }];
  }
  if (W2_REQUEST.has(state)) {
    ...
    if (state === "request-evidence-queued")
      return [...asked, { label: "Proof queued", meta: "saved on this device · waiting to send", open: true }];
    if (state === "request-evidence-submitted")
      return [...asked, { label: "Proof in", meta: "photo from the market · Jul 6", open: true }];
    if (state === "request-ready-pending")
      return [...asked, { label: "Proof in", meta: "photo from the market · Jul 6" }, { label: "Readiness queued", meta: "saved on this device · waiting to send", open: true }];
    if (state === "request-ready-confirmer")
      return [...asked, { label: "Proof in", meta: "photo from the market · Jul 6" }, { label: "Ready to confirm", meta: "waiting on Ana", open: true }];
    if (state === "request-confirmation-pending")
      return [...asked, { label: "Proof in", meta: "photo from the market · Jul 6" }, { label: "Confirmation queued", meta: "Ana · saved on this device", open: true }];
    if (state === "request-disputed")
      return [...asked, { label: "Under steward review", meta: "Ana recorded what still needs doing", open: true }];
    return [...asked, { label: "Proof in", meta: "photo from the market · Jul 6" }, { label: "Commitment kept", meta: "confirmed by Ana · Jul 6", open: true }];
  }
  if (W2_SUPPORT.has(state)) {
    ...
    if (state === "support-evidence-queued")
      return [...offered, { label: "Proof queued", meta: "saved on this device · waiting to send", open: true }];
    if (state === "support-evidence-submitted")
      return [...offered, { label: "Proof in", meta: "photo of repaired handles · Jul 6", open: true }];
    if (state === "support-ready-pending")
      return [...offered, { label: "Proof in", meta: "photo of repaired handles · Jul 6" }, { label: "Readiness queued", meta: "saved on this device · waiting to send", open: true }];
    if (state === "support-ready-confirmer")
      return [...offered, { label: "Proof in", meta: "photo of repaired handles · Jul 6" }, { label: "Ready to confirm", meta: "waiting on João", open: true }];
    if (state === "support-confirmation-pending")
      return [...offered, { label: "Proof in", meta: "photo of repaired handles · Jul 6" }, { label: "Confirmation queued", meta: "João · saved on this device", open: true }];
    ...
    return [...offered, { label: "Proof in", meta: "photo of repaired handles · Jul 6" }, { label: "Commitment kept", meta: "confirmed by João · Jul 6", open: true }];
  }
  if (W2_CAPTURED.has(state)) {
    ...
    if (state === "captured-evidence-queued")
      return [...recorded, { label: "Proof queued", meta: "saved on this device · waiting to send", open: true }];
    if (state === "captured-evidence-submitted")
      return [...recorded, { label: "Proof in", meta: "workshop photo · Jul 6", open: true }];
    if (state === "captured-ready-pending")
      return [...recorded, { label: "Proof in", meta: "workshop photo · Jul 6" }, { label: "Readiness queued", meta: "saved on this device · waiting to send", open: true }];
    if (state === "captured-ready-confirmer")
      return [...recorded, { label: "Proof in", meta: "workshop photo · Jul 6" }, { label: "Ready to confirm", meta: "waiting on João", open: true }];
    if (state === "captured-confirmation-pending")
      return [...recorded, { label: "Proof in", meta: "workshop photo · Jul 6" }, { label: "Confirmation queued", meta: "Ana · saved on this device", open: true }];
    if (state === "captured-disputed")
      return [...recorded, { label: "Under steward review", meta: "Ana recorded what still needs doing", open: true }];
    return [...recorded, { label: "Proof in", meta: "workshop photo · Jul 6" }, { label: "Commitment kept", meta: "confirmed by João · Jul 6", open: true }];
  }
  ...
  if (state === "garden-support-arrived")
    return [
      ...
      { label: "Proof in", meta: "survey sheet · Jul 10" },
  ...
  if (state === "evidence-queued")
    return [...opened, { label: "Proof queued", meta: "saved on this device · waiting to send", open: true }];
  if (state === "withdrawn")
    return [
  ...
  // The offer cast's tail asserted `opened → worked → ready` for every state that
  // reached it, so a commitment somebody had just taken up showed "Work linked ·
  // Jul 8" and "Ready to confirm · waiting on João" — while the Work section on
  // the same screen said that pruning session was NOT linked, and the band said
  // to add proof. Four points in time on one screen. These return early instead.
  if (state === "accepted" || state === "accepted-joinable" || state === "contributor" || state === "active-waiting")
    return [...opened, { label: "Accepted", meta: "João took this up · Jul 3", open: true }];
  if (state === "active" || state === "evidence-submitted" || state === "evidence-preview")
    return [...opened, { label: "Accepted", meta: "João took this up · Jul 3" }, { label: "Proof in", meta: "north beds · Jul 8", open: true }];
  if (state === "partially-approved")
    return [...opened, { label: "Accepted", meta: "João took this up · Jul 3" }, { label: "Work approved", meta: "1 of 2 counted", open: true }];
...
const w2Disclosures = (state: W2State, opts: { work?: boolean; overrideNote?: boolean; reward?: string } = {}) => {
  ...
  return (
    ...
    // Proof, where proof exists. Gated on preAcceptance alone, this drew photos
    // and a voice note on a commitment somebody had just taken up.
    (W2_HAS_PROOF.has(state) || evidenceQueued ? sectionCard("Media", mediaFor(), { flush: true }) : "") +
    sectionCard(
      "Details",
...
const W2_BAND_ICON: Record<string, string> = {
  ...
  "Ready to confirm": "time-line", "Proof in": "image-line",
...
// The settled band, per state. See the note at its call site in w2().
const W2_SETTLED_BAND: Partial<Record<W2State, { t: string; m: string; ic: string }>> = {
  "reward-released": { t: "Kept. Your steward recorded the reward", m: "It is a reference only. The value moves outside the app.", ic: "seedling-line" },
  "support-queued": { t: "Kept. The support is on its way", m: "It has joined the queue. You will see it here when it arrives.", ic: "time-line" },
  "support-en-route": { t: "Kept. The support is on its way", m: "It has left. You will see it here when it arrives.", ic: "send-plane-line" },
  "support-executed": { t: "Kept. The support is on its way", m: "It has been sent and is waiting to be acknowledged.", ic: "send-plane-line" },
  "support-confirming": { t: "Kept. The support is on its way", m: "The last step is being acknowledged. Nothing is needed from you.", ic: "send-plane-line" },
  "support-delayed": { t: "Kept. The support is taking longer", m: "Delivery is delayed. Your commitment stays recorded exactly as it is.", ic: "time-line" },
  "support-arrived": { t: "Kept, and the support arrived", m: "The reference is in Support below.", ic: "checkbox-circle-fill" },
  "support-failed": { t: "Kept. The support is being rearranged", m: "Your commitment is recorded and stays kept while that is sorted out.", ic: "error-warning-line" },
  "support-cancelled-queued": { t: "Kept. The support was withdrawn before it was sent", m: "Your commitment and its record stay intact.", ic: "close-line" },
  "support-cancelled-failed": { t: "Kept. The support was closed after delivery could not complete", m: "Your commitment and its record stay intact.", ic: "close-line" },
};
...
const W2_BARS: Partial<Record<W2State, BarSpec>> = {
  ...
  requested: { seat: "confirmer", html: hot("w2.withdraw-request", btn("Withdraw This Request…", { kind: "danger", full: true })) },
...
function w2(stateIn: W2State): string {
  ...
  if (state === "not-found")
    return readWrap(pagepad(emptyState("search-line", "Commitment not found", "We couldn't find this commitment. It may have been withdrawn, or it hasn't reached this phone yet.", hot("w2.retry", btn("Try Again", { kind: "sec", icon: "refresh-line" })))));
  if (state === "read-error")
  ...
  const progress =
    !claimed || W2_SETTLED.has(state)
      ...
      : work
        ? progressBlock({
            // Nothing has been approved at Accepted, so the rows carry the target
            // and a zero rather than a completion the commitment has not reached.
            rows: (cast === "request-work"
              ? [{ label: "Weed", done: 2, of: 2 }, { label: "Mulch", done: 3, of: 4 }]
              : [{ label: "Prune", done: 2, of: 2 }, { label: "Plant", done: 8, of: 12 }]
            ).map((r) => (w2InPhase(state, "Accepted") ? { ...r, done: 0 } : r)),
  ...
  // where the support is. Offering "Add proof" here contradicted both the
  // state chip and the reward row, and §5.3 gates evidence attach to
  // Active / EvidenceSubmitted / PartiallyApproved anyway.
  // Settlement-bearing states are Fulfilled with the money still moving, so the
  // live question is where the G$ is. One band said "Commitment kept · the
  // season's count already grew" on all ten of them: the third statement of a
  // fact the state chip and the timeline's last moment already carry, and a
  // second firing of a celebration this person had days ago. "Kept" survives as
  // the first word, because someone arriving from a payment notification still
  // needs it; the rest says where the support has got to. Sourced beside
  // w2RewardRow so the band and the Support row below it cannot disagree.
  const settledBand = W2_SETTLED_BAND[state];
  if (settledBand)
    band = bandCard(`<div class="t-title">${settledBand.t}</div><div class="t-meta">${settledBand.m}</div>`, settledBand.ic);
  else switch (state) {
    // Your offer found a person. This is the beat the audit called the costliest
    // missing one: it reached the creator as a row inside a folded timeline,
    // under a band that opened with an instruction. The instruction is still
    // here, second, where it belongs once you know who is waiting.
    case "accepted":
      band = bandCard(
        `<div class="t-title">João took this up</div><div class="t-meta">Someone is waiting on this now. Add proof as you go, link the work that fulfills it, and João confirms it when it's done.</div>`,
        "hand-heart-line",
      );
      break;
    case "request-withdraw-confirm":
      band = bandCard(
        `<div class="t-title">Your request is live</div><div class="t-meta">Stewards review who takes this up. You can withdraw it until it's accepted.</div>`,
        "hand-heart-line",
      );
      break;
    case "request-withdrawn":
      band = bandCard(
        `<div class="t-title">You withdrew this request</div><div class="t-meta">It has left the pool. The record and the reason you gave stay in the timeline below. Asking again is a fresh request, never a retry of this one.</div>`,
        "close-line",
      );
      break;
    ...
    case "evidence-queued":
      band = bandCard(
        `<div class="t-title">Proof saved on this device</div><div class="t-meta">It will send when connected. Who you credited stays attached to it, and nothing counts toward the commitment until it sends.</div>`,
      );
      break;
    case "request-active":
      band = bandCard(
    ...
    case "request-work-fulfilled":
      band = hero("The drainage channel is clear", "Your approved work met Ana's ask. She confirmed it on Jul 15.", "checkbox-circle-fill");
      break;
    ...
    case "campaign-request-evidence-queued":
      band = bandCard(
        `<div class="t-title">Campaign proof saved on this device</div><div class="t-meta">It will send when connected. The Market rides Campaign remains the scope.</div>`,
      );
      break;
    case "campaign-request-evidence-submitted":
      band = bandCard(
        `<div class="t-title">Proof attached: 1 · no work required</div><div class="t-meta">Ana confirms this Market rides Campaign request.</div>`,
      );
      break;
    case "campaign-request-ready-pending":
      band = bandCard(
        `<div class="t-title">Campaign readiness saved on this device</div><div class="t-meta">The commitment stays Proof in until this sends. Its Campaign binding does not change.</div>`,
      );
      break;
    ...
    case "campaign-request-fulfilled":
      band = hero("The market ride happened", "Ana confirmed it on Jul 6, for the Market rides campaign.", "checkbox-circle-fill");
      break;
    case "request-evidence-queued":
      band = bandCard(
        `<div class="t-title">Proof saved on this device</div><div class="t-meta">It will send when connected. Readiness stays unavailable until the proof reaches Ana's request.</div>`,
      );
      break;
    case "request-evidence-submitted":
      band = bandCard(
        `<div class="t-title">Not sent yet. This is waiting on you</div><div class="t-meta">Your proof is attached, but Ana cannot confirm the help arrived until you send it. Sending also fixes the team and stops further proof counting.</div>`,
      );
      break;
    case "request-ready-pending":
      band = bandCard(
        `<div class="t-title">Readiness saved on this device</div><div class="t-meta">The request remains Proof in until this sends. Ana cannot confirm it twice or before the readiness transition lands.</div>`,
      );
      break;
    ...
    case "request-fulfilled":
      band = hero("You got Ana to the market", "Ana confirmed the ride on Jul 6.", "checkbox-circle-fill");
      break;
    ...
    case "support-evidence-queued":
      band = bandCard(
        `<div class="t-title">Proof saved on this device</div><div class="t-meta">It will send when connected. Confirmation stays unavailable until the proof reaches the commitment.</div>${listRow({
          icon: "image-line",
    ...
    case "support-evidence-submitted":
      band = bandCard(
        `<div class="t-title">Not sent yet. This is waiting on you</div><div class="t-meta">Your proof is attached, but João cannot confirm the repair until you send it. Sending also fixes the team and stops further proof counting.</div>`,
      );
      break;
    case "support-ready-pending":
      band = bandCard(
        `<div class="t-title">Readiness saved on this device</div><div class="t-meta">The service remains Proof in until this sends. João's confirmation opens after the readiness transition lands.</div>`,
      );
      break;
    ...
    case "support-fulfilled":
      band = hero("The tool handles are repaired", "You did the repair. João confirmed it on Jul 6.", "checkbox-circle-fill");
      break;
    case "captured":
      band = bandCard(
        `<div class="t-title">Workshop underway</div><div class="t-meta">This recorded-for-Kwame commitment is proof-only. Add proof without introducing a garden-work approval requirement.</div>`,
      );
      break;
    case "captured-evidence-queued":
      band = bandCard(
        `<div class="t-title">Proof saved on this device</div><div class="t-meta">It will send when connected. The recorded commitment keeps its StewardCaptured path.</div>`,
      );
      break;
    case "captured-evidence-submitted":
      band = bandCard(
        `<div class="t-title">Proof attached: 1 · no work required</div><div class="t-meta">The named counterparty confirms this recorded commitment.</div>`,
      );
      break;
    case "captured-ready-pending":
      band = bandCard(
        `<div class="t-title">Readiness saved on this device</div><div class="t-meta">The record remains Proof in until this sends; confirmation stays unavailable meanwhile.</div>`,
      );
      break;
    ...
    case "captured-fulfilled":
      band = hero("The compost workshop happened", "Recorded for you, and confirmed by João on Jul 6.", "checkbox-circle-fill");
      break;
    case "garden-provider":
      band = bandCard(
        `<div class="t-title">Your garden is providing this</div><div class="t-meta">Add proof as Awka gardeners run the survey. The named confirmer, Sofia, confirms it when it is done.</div>`,
      );
      break;
    case "garden-support-arrived":
      band = hero("Awka Hub did the survey", "Sofia confirmed it on Jul 12, and the support reached the garden's own account.", "checkbox-circle-fill");
      break;
    case "withdrawn":
      band = bandCard(
    ...
    case "browse-requested":
      band =
        card(`<div class="t-title">Open to help</div><div class="t-meta">Anyone here can help. You provide, submit proof, and Ana, who asked, confirms it was kept.</div>`);
      break;
    ...
    case "evidence-submitted":
      band = bandCard(
        `<div class="t-title">Proof attached: 1</div><div class="t-meta">This is garden work, so approved linked work and its assessment move the commitment toward confirmation.</div>`,
      );
      break;
    ...
    // keeps the full celebration, because that is when it happens. Coming back
    // to a finished commitment days later gets a compact kept row instead —
    // re-firing a hero on every visit stops it meaning anything, and measured
    // at 390px the band sat 1318px down, below a ~1046px identity card, so the
    // celebration was two screens past the fold anyway.
    case "fulfilled":
      band = hero("The north beds are pruned", "You did the work. João confirmed it on Jul 12.", "checkbox-circle-fill");
      break;
    case "fulfilled-pool-fallback":
      band = bandCard(`<div class="t-title">Commitment kept</div><div class="t-meta">Confirmed by garden steward, fallback · the reason is in the timeline</div>`);
      break;
    case "fulfilled-protocol-fallback":
      band = bandCard(`<div class="t-title">Commitment kept</div><div class="t-meta">Confirmed by Green Goods team, fallback · the reason is in the timeline</div>`);
      break;
    case "expired":
      band = bandCard(
  ...
  // Status sits ABOVE the identity card (2026-08-17 round 43, Afo). The band is
  // scan-layer status — where this commitment is right now — and it was landing
  // at 1318px on a 390px frame, under a ~1046px identity card, so on every
  // state it was two screens past the fold. Nothing about a status message
  // wants to be read after the people and the progress bars.
  const status = pagepad(
    capturedChip,
    ...
    band.replace(
      ...
      `<span class="fic">${icon(W2_SETTLED_BAND[state]?.ic ?? W2_BAND_ICON[w2StateChip[state]] ?? "information-line")}</span>`,
    ),
  );
  ...
  if (state === "request-withdraw-confirm")
    return phoneFrame(
      sheetOver(
        `${head}${status}${meta}${content}`,
        "Withdraw this request?",
        `${banner("Nobody has taken this up yet. Withdrawing removes it from the pool; the record and the reason you give stay in the timeline.", "stone")}
${reasonChips(["Sorted another way", "No longer needed", "Asked by mistake"])}
${field("Reason (required)", input("sorted another way. Luz drove me on Saturday"))}
${hot("w2.withdraw-request-send", btn("Withdraw This Request", { kind: "danger", full: true }))}${hot("w2.withdraw-request-keep", btn("Keep it Open", { kind: "ghost", full: true }))}`,
      ),
      { appBar: false },
    );
...
const W2_HOTS: HifiDef["hots"] = {
  ...
  "w2.add-evidence": { l: "Add proof", to: "screen:W2a@media", info: "W2a attach sheet: photo / link / note → one proof job per submit; fully offline (UX:164)." },
  ...
  "w2.send-confirmation": { l: "Send for confirmation", to: "screen:W2@send-confirm", info: "Opens the confirmation step rather than sending straight away (2026-08-17, Afo): submitForConfirmation freezes the team and credit accounting, so the act is named before it happens. Drawn once here; the request, Campaign-request and recorded casts take the same step." },
  "w2.send-cancel": { l: "Not yet", to: "screen:W2@support-evidence-submitted", info: "Returns without sending. The proof stays attached and the team stays open." },
  "w2.send-confirm-go": { l: "Send it", to: "screen:W2@support-ready-pending", info: "Queues the proof-only readiness transition — submitForConfirmation, which freezes the roster and credit accounting and emits ContributorRosterFrozen before CommitmentReadyForConfirmation. DomainImpact is rejected on-chain (CS:138b); submitting is not confirming, and the lead stays blocked from every confirmation path.", calls: ["submitForConfirmation"], pendingSync: true },
  ...
  "w2.withdraw-request": { l: "Withdraw (pre-acceptance)", to: "screen:W2@request-withdraw-confirm", info: "The requester's own pre-acceptance withdraw, the mirror of w2.withdraw on an offer. The button was drawn and the sheet was not, so it opened the offer-worded one: \u201cWithdraw this offer?\u201d over a request. Steward cancellation stays a separate recorded act with its own outcome." },
  "w2.withdraw-request-send": { l: "Withdraw this request", to: "screen:W2@request-withdrawn", info: "cancelCommitment(commitmentId, reasonCID). Nobody has taken it up, so no units release; the reason is stored and the member timeline names the requester as the actor.", calls: ["cancelCommitment"], facts: { commitment: "Requested", kind: "SupportService" } },
  "w2.withdraw-request-keep": { l: "Keep it open", to: "screen:W2@requested", info: "Dismisses the sheet with the request untouched and still open to the garden." },
...
const W2B_STATES = [
  ...
  ["frozen", "Team fixed"], ["recognition", "How credit is shared"],
] as const;
...
const W2B_POLICY = sectionCard(
  ...
  `${detailRow("Shared equally", "35% among everyone on the team")}${detailRow("By verified contribution", "65% from approved work and proof")}${hot("w2b.preview", btn("What Counts as Verified", { kind: "ghost", sm: true, icon: "information-line" }))}`,
);
  ...
  switch (state) {
    ...
    case "setup":
      body =
        ...
        sectionCard(
          "Invited so far",
          `${memberRow({ name: "João", sub: "joao.eth", joined: "Joined Jan 2025" })}${memberRow({ name: "Luz", sub: "luz.eth", joined: "Joined Feb 2025" })}`,
          { flush: true },
        ) +
        banner("Inviting someone does not give them credit. Approved work and proof do.", "stone", "information-line");
      // "Save and Go Back" named the navigation rather than the act.
      actions = hot("w2b.setup-done", btn("Save Team Settings", { kind: "pri", full: true }));
      secondary = `<div class="fbrow">${hot("w2b.add", btn("Invite People", { kind: "ghost", sm: true, icon: "add-line" }))}</div>`;
      break;
    ...
    case "remove-contributor":
      body =
        ...
        banner("Lead-managed teams only. On an Open team people leave themselves, before they have credit and before the team is fixed.", "stone", "information-line");
      actions = barPair(hot("w2b.remove-cancel", btn("Keep Them", { kind: "ghost" })), hot("w2b.remove-confirm", btn("Remove Kwame", { kind: "danger" })));
      break;
    ...
    case "assign-requirement":
      ...
      actions = barPair(hot("w2b.assign-cancel", btn("Cancel", { kind: "ghost" })), hot("w2b.assign-confirm", btn("Save", { kind: "pri" })));
      break;
    case "open-eligible":
      body =
    ...
    case "join-submitted":
      body =
        formInfo("loader4-line", "Joining…", "Sent. Waiting for the garden's list of people to catch up") +
        sectionCard("Where this is", `${detailRow("You", "Not on the team until the list updates")}${detailRow("Credit", "None. Joining on its own never earns credit")}`) +
        banner("You will show as on the team once the list updates. If it does not, this screen stays as it is and nothing was lost.", "stone", "time-line");
    ...
    case "open-member":
      body =
        ...
        W2B_POLICY +
        banner("You can leave before you have any credit and before the team is fixed.", "stone", "information-line");
      actions = hot("w2b.leave", btn("Leave This Commitment", { kind: "ghost", full: true }));
      break;
    case "frozen":
      body =
        formInfo("shield-check-line", "The team", "Team fixed · 3 on the team") +
        sectionCard("On the team", W2B_ROSTER({ frozen: true }), { flush: true }) +
        W2B_POLICY +
        banner("The team was fixed when this became ready to confirm. It cannot change now.", "stone", "shield-check-line");
      break;
    case "recognition":
      body =
        ...
        sectionCard(
          ...
          `${detailRow("Approved work", "Work a steward approved, counted against this commitment")}${detailRow("Proof", "Photos, notes and follow-through attached to it")}${detailRow("Coordination", "Carrying the commitment as its accountable lead")}`,
        ) +
        banner("Each kept commitment receives an equal budget; this season's policy shares 35% equally and 65% by verified contribution. A garden may later correct amounts with a recorded reason.", "stone", "information-line");
      // An explainer with no way out but the back chevron; the persistence sheet
      // already sets the pattern of dismissing itself (round 43).
      actions = hot("w2b.recognition-done", btn("Got It", { kind: "pri", full: true }));
      break;
    ...
    case "forming-service":
      body =
        ...
        sectionCard(
          "On the team",
          `${memberRow({ name: "Maria", sub: "maria.eth", joined: "Joined Mar 2025", badge: "Lead" })}${memberRow({ name: "Tomás", sub: "tomas.eth", joined: "Joined Jul 2025", badge: "Planned" })}`,
          { flush: true },
        ) +
        W2B_POLICY +
      ...
      // No Assign Work here: a service names no actions to point at, so the bar
      // carries one act and needs no second row.
      actions = hot("w2b.add", btn("Add People", { kind: "pri", full: true }));
      secondary = `<div class="fbrow">${hot("w2b.remove", btn("Remove Someone", { kind: "ghost", sm: true }))}</div>`;
      break;
    default: // forming
      body =
        ...
        sectionCard("On the team", W2B_ROSTER(), { flush: true }) +
        W2B_POLICY;
      // Every act is in the bar, none embedded in the card (2026-08-17 round 43,
      // Afo: "we're having actions be embedded in the view of a team when they
      // should be in the action bar and we're repeating things like add people
      // shows up in two places"). Add People had been rendered twice from the
      // SAME hotspot — secondary inside the roster and primary in the bar —
      // which also drifted back from round 19's every-adder-in-the-bar rule.
      // The bar stacks: primary on its own row, the two rarer acts beneath it,
      // which is round 31's .fbrow shape rather than three buttons on one line.
      actions = hot("w2b.add", btn("Add People", { kind: "pri", full: true }));
      secondary = `<div class="fbrow">${hot("w2b.assign", btn("Assign Work", { kind: "ghost", sm: true }))}${hot("w2b.remove", btn("Remove Someone", { kind: "ghost", sm: true }))}</div>`;
  }
  return phoneFrame(head + pagepad(body), { appBar: actions ? actionBar(actions, secondary || undefined) : undefined });
}
...
const W2B_HOTS: HifiDef["hots"] = {
  ...
  "w2b.recognition-done": { l: "Got it", to: "screen:W2b@forming", info: "Dismisses the explainer. It had no way out but the back chevron (round 43)." },
  "w2b.assign": { l: "Assign work", to: "screen:W2b@assign-requirement", info: "The ONE action that stays garden-work-only: requirement rows exist nowhere else, so a service has no named action to point someone at. Planning is not credit. It rides the bar's secondary row rather than sitting inside the roster card (round 43)." },
...
const w2aOutcomeHead = `<div class="hdr fixed"><button type="button" class="hback" aria-label="Back, preview only" disabled>${icon("arrow-left-line", "l")}</button><h1>Add proof</h1></div>`;
...
// Proof is an MDR VARIANT (iteration 2, Afo direction): the same Submit
...
// The creation, proof and confirmation flows opened with a bare
...
// written note are all proof items. Evidence used to split them, keeping
...
const w2aStepCard = (state: W2aState): string => {
  if (state === "media") return stepCard("camera-line", "Add media", "Photos, video, a voice note or a link, as proof of what happened");
  if (state === "details") return stepCard("group-line", "Who helped", "Credit the teammates who share this proof");
  if (state.startsWith("review")) return stepCard("check-line", "Review proof", "Check if the information is correct");
  return "";
};
...
function w2a(state: W2aState): string {
  let head = flowHeader("Add proof", 0, 3);
  let content: string;
  let actions: string;
  let secondary = "";
  let overlay = "";
  switch (state) {
    ...
    case "details":
      head = flowHeader("Add proof", 1, 3);
      content = pagepad(
        pickRow([{ label: "Maria", on: true }, { label: "Ana", on: true }, { label: "Kwame" }]),
        `<div class="t-meta">Tap the teammates who share this proof, no typing, just the people already on it.</div>`,
    ...
    case "review-captured": {
      ...
      // screen said WHICH commitment the proof belonged to. It is now the
      ...
      content = pagepad(
        ...
        banner("Saved on this device until it sends. Proof works fully offline.", "stone", "wifi-off-line"),
      );
      actions = hot(attachHot, btn("Submit Proof", { kind: "pri", full: true }));
      break;
    }
    ...
    case "failed":
      ...
      content = pagepad(
        ...
        banner("Your proof is held on this device and nothing is dropped. Retry the one that didn't send whenever you're ready.", "stone", "wifi-off-line"),
      );
      actions = hot("w2a.done", btn("Done", { kind: "ghost", full: true }));
      break;
    // The proof list keeps one list for everything attached, and the photo
    // rows carry the picture (2026-08-17, Afo). Proof is the surface where
...
const W2A_HOTS: HifiDef["hots"] = {
  "w2a.add-link": { l: "Add a link", info: "A link is an proof item like any other (2026-08-17): it joins the list beside photos and voice notes instead of sitting in its own form field on the next step." },
  "w2a.add-note": { l: "Write a note", info: "A written note is an proof item like any other (2026-08-17) — the same adder creation uses, so everything a member attaches composes into one list." },
  ...
  "w2a.preview": { l: "Open the photo", to: "screen:W2a@media-preview", info: "The proof thumbnail opens ImagePreviewDialog — the same dialog Submit Work opens from its media grid (Media.tsx:820). Confirmers decide on what they can see, so a photo has to be legible before it is sent, not only after." },
  ...
  "w2a.attach": { l: "Submit proof", to: "screen:W2@evidence-queued", info: "Enqueues media plus the explicit creditedContributors vector; after upload the executor calls attachEvidence with those same addresses (CS §6.1).", calls: ["attachEvidence"], pendingSync: true },
  "w2a.attach-request": { l: "Attach request evidence", to: "screen:W2@request-evidence-queued", info: "Queues proof and its explicit credited-contributor vector for Ana's request without changing its direction, provider, or confirmer.", calls: ["attachEvidence"], pendingSync: true },
  "w2a.attach-campaign-request": { l: "Attach campaign-request evidence", to: "screen:W2@campaign-request-evidence-queued", info: "Queues proof and its explicit credited-contributor vector without losing the Market rides Campaign binding.", calls: ["attachEvidence"], pendingSync: true },
  ...
  "w2a.attach-captured": { l: "Attach captured-commitment evidence", to: "screen:W2@captured-evidence-queued", info: "Queues proof and its explicit credited-contributor vector while preserving StewardCaptured and the member's source identity.", calls: ["attachEvidence"], pendingSync: true },
  "w2a.retry-row": { l: "Retry this upload", to: "screen:W2a@queued", info: "Per-row retry — a failed proof job stays visible with a retry (up to MAX_RETRIES=5); media is never silently dropped (UX:218)." },
  "w2a.done": { l: "Done", to: "screen:W2@evidence-submitted", info: "Returns to the commitment with the queued or sent proof row still visible." },
};
...
const w3TeamSection = (members: W3Member[]) =>
  ...
  `<div class="t-meta">Anyone you add can add proof and submit work.</div>` +
  memberTrail([
...
// The five capture adders ride their own row (tokens.ts `.fbar:has(.fbrow)`),
// which frees the primary to be full-width and LABELLED. Step 3 was the only
// step in the composer whose next action had no word on it — an unlabelled
// arrow — and it is the optional step, so a person with nothing to add had the
// least clear way out. The icon-only decision was a real measurement: five `sm`
// adders plus a labelled primary left about 78px. This gives it the whole row.
const W3_CAPTURE_BAR = `<div class="fbrow">${captureBar("w3")}</div>`;
...
function w3(state: W3State): string {
  ...
  switch (state) {
    case "step-howmuch":
      ...
      content = pagepad(
        ...
        w3Proof({
          ...
          note: "The garden actions this commitment includes. The amount above is what you are putting in; these are what stewards approve before it counts as kept.",
          cells: [
      ...
      content = pagepad(
        w3Review({
          ...
          details: [
            ...
            { icon: "shield-check-line", label: "What has to be approved", value: "Prune × 2 · Plant × 12, in AGRO." },
    ...
    case "step-advanced":
      ...
      content = pagepad(
        sectionTitle("Who confirms"),
        card(`${kv("Ordinary confirmation", "Offer recipient confirms")}${kv("Limit", "Choose up to the current confirmer limit, read from MAX_CONFIRMERS on the deployed module rather than a number drawn here")}`),
        ...
        field("Needs an assessment", radio([{ label: "No", meta: "proof and confirmation carry the proof", on: true }, { label: "Yes", meta: "an evaluator attaches a qualifying assessment" }], { interactive: true, name: "requires-assessment" })),
      );
    ...
    case "advanced-work-ask":
      ...
      content = pagepad(
        sectionTitle("Who confirms"),
        card(`${kv("Ordinary confirmation", "You, because it was your request")}${kv("Limit", "Choose up to the current confirmer limit, read from MAX_CONFIRMERS on the deployed module rather than a number drawn here")}`),
        ...
        hot("w3.contributor-policy", field("Contributor policy", radio([{ label: "Open team", meta: "eligible garden members may join", on: true }, { label: "Lead-managed team", meta: "the lead or a steward manages who is on it" }], { interactive: true, name: "contributor-policy-work-ask" }))),
    ...
    case "step-advanced-no-protocol":
      ...
      content = pagepad(
        ...
        sectionTitle("Team options"),
        hot("w3.contributor-policy", field("Contributor policy", radio([{ label: "Open team", meta: "eligible garden members may join", on: true }, { label: "Lead-managed team", meta: "the lead or a steward manages who is on it" }], { interactive: true, name: "contributor-policy-noproto" }))),
      );
      actions = hot("w3.advanced-done", btn("Back to Review", { kind: "pri", full: true }));
      break;
    case "step-confirmers":
      ...
      content = pagepad(
        ...
        `<div class="t-meta">Any one of the people you name may confirm. People on the team never appear here. Nobody confirms a commitment they worked on.</div>`,
        card(
        ...
        field("How many must confirm", radio([{ label: "Any one of them", meta: "whoever confirms first closes it", on: true }, { label: "Both of them", meta: "it needs both confirmations" }], { interactive: true, name: "confirmer-threshold" })),
        banner("Maria and Ana are on the team, so they are not listed. Nobody confirms a commitment they worked on.", "stone", "shield-check-line"),
      );
    ...
    case "step-confirmers-work":
      ...
      content = pagepad(
        ...
        field("How many must confirm", radio([{ label: "Any one of them", meta: "whoever confirms first closes it", on: true }, { label: "Both of them", meta: "it needs both confirmations" }], { interactive: true, name: "confirmer-threshold-work" })),
        banner("Whoever takes up the ask joins the team and leaves this list. A contributor can never confirm.", "stone", "shield-check-line"),
      );
    ...
    case "support-review":
      ...
      content = pagepad(
        w3Review({
          ...
          advanced: { hot: "w3.advanced", label: "Who confirms", value: "Whoever takes it up confirms it. A service names no garden actions, so proof and that person carry the proof." },
        }),
        `<div class="t-meta">Service offers name no garden actions. Proof and the person you help carry the proof.</div>`,
      );
    ...
    case "support-howmuch":
      ...
      content = pagepad(
        ...
        banner("A service offer names no garden actions. Proof and the person you help carry the proof.", "stone", "shield-check-line"),
      );
    ...
    case "request-what":
      ...
      content = pagepad(
        ...
        kindCards([
          { icon: "hand-heart-line", label: "Help or a service", meta: "rides, meals, repairs, confirmed by proof", on: true },
    ...
    case "request-work-what":
      ...
      content = pagepad(
        sectionTitle("What kind?"),
        kindCards([
          { icon: "hand-heart-line", label: "Help or a service", meta: "rides, meals, repairs, confirmed by proof", hot: "w3.request-choose-service" },
    ...
    case "request-work-howmuch":
      ...
      content = pagepad(
        ...
        w3Proof({
          title: "What has to be approved",
          note: "The garden actions this ask includes. The amount above is what you are asking for; these are what stewards approve before it counts as met.",
          cells: [
      ...
      content = pagepad(
        w3Review({
          ...
          details: [
            ...
            { icon: "calendar-line", label: "Due", value: "Aug 30, running with the season" },
            { icon: "seedling-line", label: "Where it runs", value: "Season of First Rains" },
            { icon: "shield-check-line", label: "What has to be approved", value: "Weed × 2 · Mulch × 4, in AGRO." },
    ...
    case "request-review":
      ...
      content = pagepad(
        w3Review({
          ...
          advanced: { hot: "w3.advanced", label: "Who confirms", value: "You, because it was your request. Proof and you carry the proof." },
        }),
        `<div class="t-meta">A service request names no garden actions, proof and you, the requester, carry the proof.</div>`,
      );
    ...
    case "saved-offer-edit":
      ...
      content = pagepad(
        ...
        field("Where it runs", input("Season of First Rains", { select: true })),
    ...
    case "saved-offer-review":
      ...
      content = pagepad(
        card(`${kv("Direction", "Offer")}${kv("Kind", "Support / service")}${kv("Garden", "Rocinha Community Garden")}${kv("Where it runs", "Season of First Rains")}${kv("Title", "Hosting climate workshops")}${kv("What people receive", "A two-hour session on local climate work")}${kv("How much", "1 workshop session")}${kv("Who confirms", "Recipient")}`),
      ...
      actions = barPair(hot("w3.edit-saved-offer", btn("Edit", { kind: "ghost" })), hot("w3.submit-saved-offer", btn("Make This Offer", { kind: "pri" })));
      break;
      ...
      content = pagepad(
        w3Review({
          ...
          details: [
            ...
            { icon: "time-line", label: "When this season ends", value: "Ask me whether to keep offering it" },
    ...
    case "repeat-noticed":
      ...
      content = pagepad(
        ...
        kindCards([
          ...
          { icon: "hand-heart-line", label: "A service", meta: "rides, meals, repairs, confirmed by proof", on: true },
        ]),
        ...
        card(
          `<div class="t-title">You have offered this three times</div><div class="t-meta">Made ongoing, what you give from now counts together, and neighbours can see it has been running. The three you already made stay as they are; they cannot be gathered up.</div>` +
            barPair(hot("w3.repeat-make-ongoing", btn("Make it Ongoing", { kind: "pri" })), hot("w3.repeat-keep-once", btn("Keep it a One-off", { kind: "ghost" }))),
          { cls: "inset" },
        ),
        w3Scope("Season of First Rains"),
      );
    ...
    default:
      ...
      content = pagepad(
        ...
        kindCards([
          { icon: "leaf-line", label: "Garden work", meta: "counts toward the garden's actions", on: true },
          { icon: "hand-heart-line", label: "A service", meta: "rides, meals, repairs, confirmed by proof", hot: "w3.choose-support" },
        ]),
        w3HowOften(),
        w3Scope("Season of First Rains"),
...
const W3_HOTS: HifiDef["hots"] = {
  ...
  "w3.choose-support": { l: "Choose Support / service", to: "screen:W3@support-howmuch", info: "Chooses the proof-only SupportService offer path. It keeps the amount step and skips only DomainImpact action anchors (UX:156)." },
  ...
  "w3.continue-request-details": { l: "Continue to review", to: "screen:W3@request-review", info: "The ask's details step → its review. An ask has nothing done yet, so what attaches here is context for whoever takes it up — the proof arrives with the work." },
...
function w4(state: W4State): string {
  ...
  const exclusion = hot(
    ...
    requestWork
      ...
      : banner("Maria, Ana, and Kwame are on the frozen contributor team, so none of them can confirm, not even through the steward fallback.", "stone", "shield-check-line"),
  );

  let inner: string;
  // The acts live in the sheet's fixed footer, never inline at the end of the
  // scrolling content (2026-08-17 round 43, Afo). Stacking a full-width primary
  // over a full-width secondary is the shape the one-row rule forbids inside an
  // actionBar; being outside one is how it escaped the check. Two acts are equal
  // halves (C.38), and the sheet asks a question, so the two buttons are its two
  // answers: Confirm, or Not Yet. "Tell the Stewards Why" named the next screen
  // rather than the act, and three of five states already said "Not Yet" — which
  // is also the admin Hub's own word for this decision (uiux §6.9).
  let bar = "";
  let title = "Commitment kept?";
  switch (state) {
    case "confirm-request":
      title = "Did the help arrive?";
      inner = `${summary}${listRow({ icon: "image-line", primary: "Proof", meta: "1 item · photo from the market" })}${meter(0, { left: "confirmations", right: "0 of 1" })}${exclusion}`;
      bar = barPair(hot("w4.confirm-request", btn("Confirm Help Arrived", { kind: "pri" })), hot("w4.not-yet-request", btn("Not Yet", { kind: "ghost" })));
      break;
    case "confirm-request-work":
      title = "Was the work you asked for done?";
      inner = `${summary}${listRow({ icon: "check-line", primary: "Approved work", meta: "Weed × 2 · Mulch × 4, all approved by the stewards" })}${meter(0, { left: "confirmations", right: "0 of 1" })}${exclusion}`;
      bar = barPair(hot("w4.confirm-request-work", btn("Confirm the Work Was Done", { kind: "pri" })), hot("w4.not-yet-request-work", btn("Not Yet", { kind: "ghost" })));
      break;
    case "confirm-campaign-request":
      title = "Did the Campaign help arrive?";
      inner = `${summary}${listRow({ icon: "image-line", primary: "Proof", meta: "1 item · photo from the market" })}${meter(0, { left: "confirmations", right: "0 of 1" })}${exclusion}`;
      bar = barPair(hot("w4.confirm-campaign-request", btn("Confirm Help Arrived", { kind: "pri" })), hot("w4.not-yet-campaign-request", btn("Not Yet", { kind: "ghost" })));
      break;
    case "confirm-support":
      inner = `${summary}${listRow({ icon: "image-line", primary: "Proof", meta: "1 item · repaired handles" })}${meter(0, { left: "confirmations", right: "0 of 1" })}${exclusion}`;
      bar = barPair(hot("w4.confirm-support", btn("Confirm Commitment Kept", { kind: "pri" })), hot("w4.not-yet-support", btn("Not Yet", { kind: "ghost" })));
      break;
    case "confirm-captured":
      inner = `${summary}${listRow({ icon: "image-line", primary: "Proof", meta: "1 item · workshop photo" })}${meter(0, { left: "confirmations", right: "0 of 1" })}${exclusion}`;
      bar = barPair(hot("w4.confirm-captured", btn("Confirm Commitment Kept", { kind: "pri" })), hot("w4.not-yet-captured", btn("Not Yet", { kind: "ghost" })));
      break;
    case "not-yet":
    ...
    case "not-yet-captured":
      ...
      inner = `${reasonChips(
        ...
        { placeholder: false },
      ))}${banner("This never cancels the commitment. Stewards review it, and every outcome shows its reason.", "stone")}`;
      bar = hot(
    ...
    case "confirmed-pending-captured":
      ...
      inner = `${meter(100, { left: "including this device", right: evidenceOnly ? "1 of 1 saved" : "3 of 3 saved" })}${listRow({ icon: "time-line", primary: "Your confirmation", chipHtml: chip("Waiting to send", "queued") })}${banner("Your confirmation is saved on this phone. It sends as soon as you're online, and the commitment counts as kept from that moment.", "stone", "wifi-off-line")}`;
      bar = hot(
    ...
    case "confirmed-captured":
      ...
      inner = `${hero(
        "You've confirmed it",
        request || campaignRequest || requestWork
          ? "João's help is recorded as kept."
          : captured
            ? "Kwame's record now shows it as kept."
            : support
              ? "Maria's repair is recorded as kept."
              : "Maria's work is recorded as kept.",
        "checkbox-circle-fill",
      )}`;
      bar = hot(
    ...
    case "not-yet-failed-captured":
      ...
      inner = `${field("What still needs doing?", input(
        ...
        : "The far bed is still overgrown…",
        { placeholder: false },
      ))}${banner("Couldn't reach the stewards just now. Your note is kept and this commitment stays ready to confirm. Try again when you are back online.", "amber", "error-warning-line")}`;
      bar = hot(
    ...
    case "provider-view":
      ...
      break;
    default:
      inner = `${summary}${listRow({ icon: "check-line", primary: "Linked work", meta: "1 approved · proof: 2 items" })}${confirmMeter}${exclusion}`;
      bar = barPair(hot("w4.confirm", btn("Confirm Commitment Kept", { kind: "pri" })), hot("w4.not-yet", btn("Not Yet", { kind: "ghost" })));
  }
        ...
        : state === "provider-view"
          ...
          : state.includes("pending")
            ...
            : state.startsWith("confirmed")
              ? { ic: "checkbox-circle-fill", info: "Confirmed. Whoever did the work sees it as kept now." }
              : { ic: "hand-heart-line", info: "Only the person it was made to can confirm it. Everyone who contributed is excluded." };
  // handle:false makes this a fixed-height drawer like the commitments sheet,
  // so confirming something never resizes the surface it opened from (round 43).
  return phoneFrame(sheetOver(w4Behind(cast), title, inner, { ...sheetInfo, handle: false, close: true, footer: bar || undefined }), {
    appBar: false,
  });
}
...
const W4_HOTS: HifiDef["hots"] = {
  ...
  "w4.confirm-support": { l: "Confirm — commitment kept", to: "screen:W4@confirmed-pending-support", info: "The recipient confirms the proof-only service commitment; fulfillment appears only after sync.", calls: ["confirmFulfillment"], pendingSync: true },
  ...
  "w4.confirm-captured": { l: "Confirm — recorded commitment kept", to: "screen:W4@confirmed-pending-captured", info: "The named counterparty confirms the proof-only StewardCaptured commitment.", calls: ["confirmFulfillment"], pendingSync: true },
  ...
  "w4.provider-note": { l: "Provider exclusion", info: "Nobody confirms their own commitment, anywhere, including steward fallback (UX:32)." },
  "w4.done": { l: "Back to the pool", to: "screen:W1", info: "Returns to the pool tab, which is what the button says. It used to land on the commitment's own fulfilled view — the provider's screen, shown to the confirmer, which is why that screen could not carry the celebration it was always meant to (registry note, UX:169,201,204)." },
  "w4.done-support": { l: "Back to the pool", to: "screen:W1", info: "Returns to the pool tab. The service commitment's own fulfilled view belongs to Maria, who did the repair." },
  "w4.done-request": { l: "Back to the pool", to: "screen:W1", info: "Returns to the pool tab. The request's fulfilled view belongs to João, who provided the help." },
  "w4.done-request-work": { l: "Back to the pool", to: "screen:W1", info: "Returns to the pool tab. The garden-work ask's fulfilled view belongs to whoever did the work (register #97a)." },
  ...
  "w4.done-campaign-request": { l: "Back to the pool", to: "screen:W1", info: "Returns to the pool tab, with the Campaign binding unchanged on the commitment itself." },
  "w4.done-captured": { l: "Back to the pool", to: "screen:W1", info: "Returns to the pool tab. The recorded commitment's fulfilled view stays Kwame's." },
  ...
  switch (state) {
      ...
      body = sheetOver(
        ...
        `<div class="t-meta">How would you like to offer this?</div>` +
          ...
          hot("w32.offer-over-time", card(
            `<div class="t-title">Offer it over time</div><div class="t-meta">Keep offering it in one garden, season after season. Its history stays together.</div>`,
            { cls: "flat" },
          )) +
    ...
    case "save-failed":
      body = `${head}${pagepad(
        ...
        barPair(hot("w32.retry-save", btn("Try Saving Again", { kind: "pri", icon: "refresh-line" })), hot("w32.keep-editing", btn("Keep Editing", { kind: "ghost" }))),
      )}`;
      break;
    case "offline-local":
      body = `${head}${pagepad(
        ...
        barPair(hot("w32.retry-save-online", btn("Try When Connected", { kind: "sec", icon: "refresh-line" })), hot("w32.keep-editing", btn("Keep Editing", { kind: "ghost" }))),
      )}`;
      break;
    case "version-conflict":
      body = `${head}${pagepad(
        ...
        barPair(hot("w32.reload-remote", btn("Use Saved Version", { kind: "pri" })), hot("w32.keep-local-copy", btn("Keep a Local Copy", { kind: "ghost" }))),
...
const w34Standard = () =>
  ...
  sectionCard(
    "Details",
    `${detailRow("Unit", "workshop sessions")}${detailRow("Kind", "A service")}${detailRow("Who confirms", "Whoever takes it up")}${detailRow("When this season ends", "Ask me whether to keep offering it")}`,
  ) +
  sectionCard(
    "People",
  ...
  switch (state) {
    ...
    case "places-partial-failed":
      body = `${w34Head()}${pagepad(
        ...
        barPair(hot("w34.retry-failed-place", btn("Try the Failed One Again", { kind: "pri", icon: "refresh-line" })), hot("w34.discard-failed-place", btn("Discard the Failed One", { kind: "ghost" }))),
      )}`;
      break;
    case "story":
      body = `${w34Head()}${pagepad(
        ...
        `<div class="t-meta">Reported participants come from what people wrote in their proof, not from the record itself.</div>`,
      )}`;
      break;
    case "participation":
      body = `${w34Head()}${pagepad(
    ...
    case "stop-confirm":
      body = sheetOver(
        ...
        `<div class="t-meta">Nothing new will open. You can start it again whenever you want to.</div>` +
          ...
          barPair(hot("w34.stop-confirm", btn("Stop Offering", { kind: "pri" })), hot("w34.stop-cancel", btn("Keep Offering", { kind: "ghost" }))),
      );
      break;
    case "edit-stopped":
...
const W34_HOTS: HifiDef["hots"] = {
  ...
  "w34.story-row": { l: "Open one kept commitment", to: "screen:W2@fulfilled", info: "Every story row is an ordinary immutable Commitment with its own proof and confirmation." },
  ...
  switch (state) {
    ...
    case "mixed-failed":
      body = `${head}${pagepad(
        ...
        barPair(hot("w35.retry-failed", btn("Try the Failed One Again", { kind: "pri", icon: "refresh-line" })), hot("w35.discard-failed", btn("Discard the Failed One", { kind: "ghost" }))),
