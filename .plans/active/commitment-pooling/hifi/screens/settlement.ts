...
const W21_HOTS: HifiDef["hots"] = {
  ...
  "w21.dispatch-funding": { l: "Dispatch queued garden funding", to: "screen:W22@garden-command", info: "Dispatches the typed Funding/ProtocolToGarden row created by queueFunding; no commitment or payout-plan identity is attached.", calls: ["dispatchDisbursement"] },
...
${hot("w24.queue-funding", `<div class="arow"><div class="grow">GG protocol Safe → garden Safes</div><span class="t-meta num">discretionary treasury funding</span>${btn("Seed / top up", { kind: "sec", sm: true })}</div>`)}
...
${banner("Commitment-earned support follows the provider garden's payout-plan actions. Seed / top up is a separate protocol-steward/module-owner treasury action. Inflow is a Celo balance read — no upstream hop is recorded.", "stone")}`,
      );
      break;
    case "funding":
      inner = acard(
...
const W24_HOTS: HifiDef["hots"] = {
  ...
  "w24.execute-funding": { l: "Dispatch queued funding", to: "screen:W22@garden-command", info: "Opens the command for an already-emitted Funding/ProtocolToGarden row with no commitment identity." },
