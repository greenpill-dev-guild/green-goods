...
// commitments are ordinary independent commitments and no drawing here may imply
...
function w29(state: W29State): string {
  ...
  const pairLine = state === "matched"
    ? "Both commitments started together"
    : state === "counterpart-lapsed"
      ? "The other commitment ended. This commitment keeps its own state."
  ...
  const feed = card(
    `<div class="t-sec">Pool exchange feed</div>` +
      listRow({ icon: "checkbox-circle-fill", primary: state === "proposed" ? "Offered in exchange for" : "Both commitments started", meta: state === "proposed" ? "Maria and Ana · Apr 2" : "Ana and Maria · Apr 4" }) +
  ...
  const pairAction = state === "proposed"
    ? hot("w29.accept-cta", btn("Start both commitments…", { kind: "pri", full: true }))
    : hot("w29.open-other", btn("Open the other commitment", { kind: "sec", full: true }));
  return phoneFrame(`${hdr("Repair the shared water pump", { back: true })}${body}`, { appBar: actionBar(pairAction) });
}

const W29_HOTS: HifiDef["hots"] = {
  "w29.accept-cta": { l: "Start both commitments…", to: "screen:W30", info: "Visible only to the counterpart Offer's creator (Ana). Opens the accept-exchange confirmation sheet — nothing is committed until she confirms there (WF:1274)." },
  "w29.open-other": { l: "Open the other commitment", to: "screen:W2@support-accepted", info: "After acceptance the pair is two ordinary commitments; the other side opens as its own detail with its own recovery paths." },
};
...
const W30_STATES = [
  ["confirm", "Start both commitments?"], ["submitting", "Submitting"], ["contract-error", "Named error"],
] as const;
...
<div class="t-meta">Both commitments start together; each is kept on its own.</div>
${hot("w30.start", btn("Start both commitments", { kind: "pri", full: true }))}${hot("w30.not-now", btn("Not now", { kind: "ghost", full: true }))}`,
    ),
    { appBar: false },
  );
}
...
function w31(_state: W31State): string {
  ...
  const body = pagepad(
    ...
    card(
      row("w31.rotation", "refresh-line", "Rotation", "Each member takes a turn receiving the pool's help.") +
        ...
        row("w31.harvest-share", "seedling-line", "Harvest share", "People commitment part of a harvest and how it arrives.") +
  ...
  // commitment" — it is a prefill layer reached from the composer's step 1.
...
const W31_HOTS: HifiDef["hots"] = {
  ...
  "w31.harvest-share": { l: "Harvest share template", to: "screen:W3@step-what", info: "Prefills existing fields only (a harvest-portion commitment with its delivery note) and lands in the editable creation flow (Appendix E.2)." },
...
export const EXCHANGE_DEFS: HifiDef[] = [
  ...
  { screen: { id: "W30", title: "W30 · Start both commitments", surface: "client", frame: "phone", group: "Client PWA", states: W30_STATES.map(([id, label]) => ({ id, label, facts: { commitment: "Offered", kind: "SupportService" }, html: w30(id) })) }, hots: W30_HOTS },
