...
const CLIENT_ENTRIES: Entry[] = [
  ...
  {
    id: "member-tile", title: "Member tile", family: "cards", covers: ["memberTile", "memberTrail"],
    kit: `memberTrail([memberTile({name, sub, hotId})])`,
    ship: "packages/client/src/components/Features/Garden/Gardeners.tsx:74",
    drift: "The compact form of the shipped Gardeners row — avatar over name rather than beside it — for the details step's added-team carousel. Same name-then-address rule.",
    rule: "Added team members ride a carousel so the roster never pushes the media list off the step; the add tile closes the rail rather than sitting apart from it.",
    usedIn: /class="mtrail/,
    specs: [
      { label: "a team of two, with the add tile", html: kit.memberTrail([kit.memberTile({ name: "João", sub: "joao.eth" }), kit.memberTile({ name: "Luz", sub: "luz.eth" }), `<div class="mtile addtile">+<div class="mtn">Add</div></div>`]) },
      { label: "no name on file", html: kit.memberTrail([kit.memberTile({ name: "0x74…c2" })]) },
    ],
  },
  {
