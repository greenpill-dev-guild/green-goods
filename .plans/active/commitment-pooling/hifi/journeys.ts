...
// sitting and ends when the promise is queued; the taking-up (sb55) and the
...
{ id: "sb4a", n: 8, title: "Prove it with work", desc: "A gardener submits work from the Garden tab and points it at the promise it fulfills; each approval counts toward one named requirement.", persona: "Gardener (provider)", scen: "S4 · AGRO+EDU", reviewVisible: true, reviewGroup: "client", chapter: "keep", roles: ["gardener"], steps: [
  ...
  { f: "WFLOW@details-linked", hot: { h: "wflow.details-continue" }, st: "3 · Details", ev: "the chosen promise sits in the details like any other field — plain work that skips the picker never carries one" },
  ...
  { f: "W2@active", hot: null, st: "Active", ev: "approvals land on the promise as visible progress per requirement", br: [{ l: "Link work you already submitted", to: "screen:WFLOW@link-picker" }, { l: "The steward's approvals", to: "sb4b:0" }] },
  ...
  { f: "W2@ready-confirmer", hot: null, st: "Ready to confirm", ev: "requirements met — the named confirmation path is ready", br: [{ l: "Confirm a promise kept", to: "sb42:0" }, { l: "The assessment side", to: "sb50:0" }] },
]},
...
{ id: "sb42", n: 9, title: "Confirm a promise kept", desc: "João finds what is waiting on him in the wallet and confirms the promise was kept — a positive-only act that queues offline.", persona: "Recipient (João)", scen: "S1 · confirmation", reviewVisible: true, reviewGroup: "client", chapter: "confirm", roles: ["gardener"], steps: [
  { f: "W5", hot: { h: "w5.inbox-row", l: "Review" }, st: "Wallet", ev: "everything waiting on you, across gardens — the drawer's inbox opens the same sheet the promise does" },
  { f: "W4", hot: { h: "w4.confirm", l: "Confirm — promise kept" }, alts: [{ h: "w4.not-yet", l: "Not yet", to: "sb5:2" }], st: "ReadyForConfirmation", ev: "the sheet names the eligibility path before the act; cycle-state banners never block confirmation" },
  { f: "W4@confirmed-pending", hot: null, st: "Queued", ev: "the positive-only confirmation queues offline like everything else" },
  { f: "W2@fulfilled", hot: null, st: "Fulfilled", ev: "kept, once — the promise carries who confirmed and how; no duplicate full-screen repeat", br: [{ l: "Request, campaign, service, and recorded-promise casts", to: "screen:W4@confirm-request" }] },
]},
...
{ id: "sb5", n: 10, title: "Say “not yet”", desc: "Saying not yet is its own act with its own reason — never a negative confirmation; the promise then goes quiet under steward review.", persona: "Recipient (confirmer)", scen: "S5", reviewVisible: true, reviewGroup: "client", chapter: "confirm", roles: ["gardener"], steps: [
...
{ id: "sb45", n: 11, title: "The team behind a promise", desc: "João opens a promise whose team is still forming and joins it himself — membership shows only once the indexed roster confirms it.", persona: "Contributor (João)", scen: "S1 · group commitment", reviewVisible: true, reviewGroup: "client", chapter: "team", roles: ["gardener"], steps: [
  { f: "W1", hot: { h: "w1.open-team-offer", l: "Open the team promise" }, st: "Pool open", ev: "a forming team is visible right on the card — and opening it lands on the PROMISE, not a bare team screen", cite: "iteration 2 E8" },
  { f: "W2", hot: { h: "w2.team-strip", l: "Open team — join in" }, st: "Accepted", ev: "the people are above the fold: creator, counterparty, and the team strip in the promise's own context" },
  { f: "W2b@open-eligible", hot: { h: "w2b.join", l: "Join this promise" }, st: "Open team", ev: "eligible open-team members join on their own" },
...
{ id: "sb54", n: 18, title: "Add people to your team", desc: "Maria leads a group promise and adds a contributor from the promise detail — planning responsibility, never recognition credit.", persona: "Lead (Maria)", scen: "S1 · group commitment · lead side", reviewVisible: true, reviewGroup: "client", chapter: "team", roles: ["gardener"], steps: [
  { f: "W1", hot: null, st: "Pool open", ev: "your own promise, its team forming — adding people is the lead's act" },
  { f: "W2", hot: { h: "w2.open-team-forming", l: "Manage the team" }, st: "Accepted", ev: "the lead manages the roster from the promise detail" },
  { f: "W2b@forming", hot: { h: "w2b.add", l: "Add contributor" }, st: "Forming", ev: "the roster checklist: garden members with faces, tap to select", cite: "iteration 2 E8 — the flow your review found missing" },
  { f: "W2b@add-contributor", hot: { h: "w2b.add-confirm", l: "Add to the team" }, st: "Picker", ev: "addContributor is an online roster action; wallet rejection keeps the selection for retry" },
...
{ id: "sb11", n: 13, title: "Watch G$ support arrive", desc: "A gardener follows declared support from queued to on its way to arrived — and only an authenticated success ever says arrived.", persona: "Gardener", scen: "S8/S9 · TAS", reviewVisible: true, reviewGroup: "client", chapter: "money", roles: ["gardener"], steps: [
  { f: "W5", hot: null, st: "Wallet", ev: "support states live on the promise itself; the wallet is where you notice them" },
  { f: "W2@support-queued", hot: null, st: "Queued", ev: "the fulfilled promise's declared support enters the settlement queue" },
...
{ id: "sb18", n: 15, title: "Find everything waiting on you", desc: "One cross-garden home: what waits on you first, then your own promises by garden, then what you keep ready to offer.", persona: "Gardener across gardens", scen: "S6 · wallet drawer", reviewVisible: true, reviewGroup: "client", chapter: "money", roles: ["gardener"], steps: [
  ...
  { f: "W4", hot: null, st: "ReadyForConfirmation", ev: "the inbox row opens the same confirmation sheet the promise itself opens", br: [{ l: "Confirm a promise kept", to: "sb42:1" }] },
  { f: "W5@queued", hot: null, st: "Queued", ev: "queued promises ride at the top of their garden group with the offline banner" },
  { f: "W5@waiting-membership", hot: null, st: "Waiting", ev: "membership-gated sends wait without spending attempts" },
]},
{ id: "sb13", n: 16, title: "Claim a protocol promise for your garden", desc: "Leila claims a Green Goods promise for her garden and follows the whole arc — acceptance, the work, the support arriving.", persona: "Garden steward (Leila)", scen: "S14", reviewVisible: true, reviewGroup: "client", chapter: "money", roles: ["steward"], steps: [
  { f: "W1", hot: null, st: "Pool open", ev: "the protocol pool seeds promises gardens can claim; they surface in your garden's pool tab" },
  { f: "W25", hot: { h: "w25.ask", l: "Ask to take this up" }, st: "Protocol promise", ev: "the claim card names what the garden would provide and what support is declared" },
  ...
  { f: "W25@accepted", hot: { h: "w25.open-promise", l: "Open the promise" }, st: "Accepted", ev: "the garden is the provider; the promise opens like any other" },
...
{ id: "sb6a", n: 17, title: "Offer it again", desc: "A promise lapsed with the season; offering it again is one tap into a prefilled composer, and the old record stays honest.", persona: "Gardener (Maria)", scen: "S1/S5 edge", reviewVisible: true, reviewGroup: "client", chapter: "change", roles: ["gardener"], steps: [
  { f: "W1", hot: null, st: "Pool open", ev: "the season moved on and a promise lapsed — offering again is one tap" },
...
{ id: "sb58", n: 28, title: "Fund a priced Offer", desc: "Maria asks to fund Ben's 40 G$ Offer, deposits only after the garden records the pledge, then sees the funded claim.", persona: "Gardener (Maria)", scen: "Member-funded priced Offer", reviewVisible: true, reviewGroup: "client", chapter: "money", roles: ["gardener"], steps: [
  ...
  { f: "W36@funded", hot: null, st: "Funded", ev: "the accepted promise carries Maria's 40 G$ funding fact; Ben now follows the ordinary evidence and confirmation path", br: [{ l: "If the promise ends without delivery", to: "screen:W36@refund-queued" }, { l: "The steward checkpoint and refund", to: "sb59:0" }] },
]},
...
{ id: "sb59", n: 29, title: "Accept a funded claim and return it if needed", desc: "A Garden Steward records Maria's deposit, accepts the funded claim, then queues the same amount back when the promise ends without delivery.", persona: "Garden Steward", scen: "Member-funded priced Offer · settlement", reviewVisible: true, reviewGroup: "admin", chapter: "settlement", roles: ["steward"], steps: [
  ...
  { f: "W37@consumed", hot: null, st: "Consumed", ev: "the funding fact remains attached while Ben's promise follows the ordinary evidence and confirmation path" },
...
{ id: "sb14", n: 14, title: "Turn a neighbor's need into a seeded promise", desc: "Kwame describes a need in his own words, neighbors support it, and a steward turns the thread into a seeded promise.", persona: "Neighbour (Kwame) + steward", scen: "S10 · September", reviewVisible: false, reviewGroup: "admin", chapter: "promises", roles: ["steward"], steps: [
