...
{ id: "sb11", n: 11, title: "Watch G$ support arrive, then send it on", persona: "Gardener", scen: "S8/S9 · TAS", reviewVisible: true, reviewGroup: "client", steps: [
  { f: "W2@support-arrangement-blocked", hot: null, marks: ["w2.reward-row"], st: "Fulfilled · not queue-ready", ev: "Individual delivery disabled or another canonical blocker creates no attempt and offers no retry; the promise stays fulfilled", cite: "SS §5 · AM §1", note: "Blocked-eligibility variant. The next scene rewinds to an eligible reward." },
  { f: "W2@support-arranging", hot: null, marks: ["w2.reward-row"], st: "Variant rewind · Fulfilled · device attempt queued", ev: "reward row: 'arranging settlement' while this signed-in app's queue-ready attempt has not created an onchain disbursement", cite: "SS §5 · AM §1" },
  { f: "W2@support-arrangement-failed", hot: { h: "w2.retry-settlement", l: "Try arrangement again" }, marks: ["w2.reward-row"], st: "Fulfilled · local attempt exhausted", ev: "this device's transient submission failures can exhaust and retry without changing Fulfilled or becoming an authenticated settlement failure", cite: "SS §5 · AM §1" },
  { f: "W2@support-arranging", hot: null, marks: ["w2.reward-row"], st: "Fulfilled · local attempt retrying", ev: "retry re-checks readiness + the permanent pointer, then returns only this device's attempt to work; no second reward approval exists", cite: "SS §5" },
  ...
  { f: "W23", hot: { h: "w23.send" }, marks: ["w23.arrived-row"], ev: "online transfer — sponsored gas, never enters the offline queue", cite: "UX:219 · SS §5" },
  ...
  { f: "W2@support-failed", hot: null, st: "Failed (disbursement)", ev: "'still arranging support — your promise is recorded' — the commitment stays Fulfilled", cite: "SS §7 · DG:666", br: [{ l: "Steward recovery", to: "sb12:6" }] },
  ...
  { f: "W23@delivery-blocked", hot: null, st: "delivery blocked", ev: "AA gate failed → no Individual reward delivery, balance, or send; Garden-claim rewards and non-commitment seeding continue Safe-to-Safe · register #34f makes the gate legible admin-side", cite: "SS §5 · Decision Log #30" },
]},
{ id: "sb12", n: 12, title: "Dispatch queued support and close the loop", persona: "Steward + protocol executor", scen: "S8/S9 · pre-broadcast proof", reviewVisible: true, reviewGroup: "admin", steps: [
  ...
  { f: "W10@queue-settlement-blocked", hot: null, st: "Fulfilled · not queue-ready", ev: "Individual delivery disabled or another canonical source blocker creates no local attempt and exposes no retry; Garden rewards and explicit garden funding remain independent", cite: "SS §3.1.3 · SS §5", note: "Blocked-eligibility variant. The next scene rewinds to an eligible reward whose device-local submission failed transiently." },
  { f: "W10@queue-settlement-failed", hot: { h: "w10.retry-settlement", l: "Retry arrangement" }, st: "Variant rewind · Fulfilled · local attempt exhausted", ev: "this device's eligible submission failed independently; retry first re-reads eligibility and the permanent commitment pointer, and cannot reconfirm the promise", cite: "SS §3.1.3 · SS §5" },
  { f: "W10@queue-settlement-pending", hot: null, st: "Fulfilled · local attempt retrying", ev: "the signed-in app retries only its device-local arrangement attempt; no onchain disbursement exists yet", cite: "SS §5" },
  { f: "W10@queue-settlement", hot: { h: "w10.queue-settlement-confirm", l: "View queued reward" }, ev: "indexed Fulfillment exposed queue-ready facts; the signed-in app submitted permissionless queueDisbursement, and the onchain commitment pointer coordinates competing devices globally without a second payout approval", cite: "SS §3 · Decision Log #30" },
...
{ id: "sb19", n: 19, title: "Send a garden's G$ to its own Safe", persona: "Protocol steward", scen: "S14 · settlement", reviewVisible: true, reviewGroup: "admin", steps: [
  ...
  { f: "W10@garden-fulfilled", hot: { h: "w10.queue-settlement-garden", l: "View arrangement" }, st: "Fulfilled", ev: "Fulfillment is the economic approval; complete indexed prerequisites make the reward queue-ready for a signed-in app attempt, while Record payout remains unavailable", cite: "AM §2 · Decision Log #30" },
  { f: "W10@queue-settlement-garden-pending", hot: null, st: "Fulfilled · device attempt queued", ev: "this signed-in app is submitting the queue-ready protocol-Safe to provider-garden-Safe route; no onchain disbursement exists yet", cite: "SS §3.1.3 · SS §5" },
  { f: "W10@queue-settlement-garden-failed", hot: { h: "w10.retry-settlement-garden", l: "Retry arrangement" }, st: "Fulfilled · local attempt exhausted", ev: "transient RPC submission failed after the Garden route was already queue-ready; only this device's attempt exhausts, while Fulfilled and the member-AA gate stay untouched", cite: "SS §5" },
  { f: "W10@queue-settlement-garden-pending", hot: null, st: "Fulfilled · local attempt retrying", ev: "retry re-checks Garden-route readiness + the permanent pointer, then returns only this device's Safe-to-Safe attempt to work", cite: "SS §5" },
  { f: "W2@garden-support-arranging", hot: null, surface: "pwa", echo: true, marks: ["w2.reward-row"], st: "Fulfilled · arranging", ev: "garden members see the same pre-disbursement arranging state without a personal-wallet claim", cite: "SS §5" },
  { f: "W2@garden-support-arrangement-failed", hot: { h: "w2.retry-garden-settlement", l: "Try arrangement again" }, surface: "pwa", echo: true, marks: ["w2.reward-row"], st: "Fulfilled · job exhausted", ev: "the member-facing retry restarts only the Safe-to-Safe job and never turns the Garden reward into Individual delivery", cite: "SS §5" },
  { f: "W2@garden-support-arranging", hot: null, surface: "pwa", echo: true, marks: ["w2.reward-row"], st: "Fulfilled · job retrying", ev: "retry returns only the arrangement job to work; the garden reward remains Safe-to-Safe", cite: "SS §5" },
  { f: "W10@queue-settlement-garden", hot: { h: "w10.queue-settlement-garden-confirm", l: "Open queued reward" }, ev: "permissionless queueDisbursement derives payer = the GG protocol Safe (the owning pool) and beneficiary = Awka Hub's registered Celo Safe, never its Arbitrum account and without the member-AA gate", cite: "AM:43 · SS §3.1.3" },
  ...
  { f: "W24@flows", hot: null, marks: ["w24.queue-funding"], st: "Confirmed", ev: "the cross-garden board shows the shared corridor: this earned delivery is confirmed, while the explicit queueFunding control is reserved for a separate non-commitment seed or top-up", cite: "SS §3.1.3 · Decision Log #30" },
...
{ id: "sb23", n: 23, title: "Register an existing garden settlement account", persona: "Settlement steward", scen: "S8 · account setup", reviewVisible: true, reviewGroup: "admin", steps: [
  { f: "W21@unregistered", hot: { h: "w21.setup", l: "Register existing account" }, st: "Unregistered", ev: "the empty settlement section leads with the one available next action", cite: "SS §3.1.3" },
  { f: "W21@register-account", hot: { h: "w21.register-confirm", l: "Register account" }, st: "Verified route", ev: "registerSettlementAccount stores the already-deployed Celo Safe after governance verification", cite: "SS §3.1.3" },
  { f: "W21@registered", hot: { h: "w21.open-queue", l: "Open disbursement queue" }, st: "Registered", ev: "the result names the Safe, recovery policy, and scoped executor role", cite: "SS §3.1.3" },
...
{ id: "sb25", n: 25, title: "Recover or cancel one settlement delivery", persona: "Settlement steward", scen: "S8 · member recovery", reviewVisible: true, reviewGroup: "admin", steps: [
  { f: "W22@outcome", hot: { h: "w22.requeue-member", l: "Source follow-up" }, st: "Authenticated failure", ev: "only a bounded failure acknowledgment unlocks a new logical attempt", cite: "SS §3.1.3" },
  { f: "W21@requeue-confirm", hot: { h: "w21.requeue-confirm", l: "Requeue attempt" }, st: "Failed", ev: "the confirmation names the preserved attempt, cleared batch, incremented attempt, and that the new key waits for dispatch", cite: "SS §3.1.3" },
  { f: "W21@requeued", hot: { h: "w21.open-queue", l: "Back to queue" }, st: "Queued · attempt 2", ev: "the new attempt is visible beside its immutable failed predecessor; dispatch, not requeue, creates its execution key", cite: "SS §3.1.3" },
...
{ id: "sb33", n: 33, title: "Seed or top up a garden outside a commitment", persona: "Protocol steward / module owner", scen: "S9 · discretionary treasury support", reviewVisible: true, reviewGroup: "admin", steps: [
  { f: "W24@flows", hot: { h: "w24.queue-funding", l: "Seed / top up" }, ev: "the funds board distinguishes permissionless app-submitted fulfilled-commitment rewards from an explicitly authorized non-commitment treasury action", cite: "Decision Log #30 · SS §3.1.3" },
