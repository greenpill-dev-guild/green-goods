...
| 15 | Fund topology: HoA stream → Dev Guild working-capital wallet/Safe → GG protocol Safe → garden Celo Safes → members. One deterministic Safe mapping per garden, deployed on demand. Pilot recovery is exactly 2-of-3: protocol multisig, Dev Guild/working-capital multisig, and named garden recovery delegate; no owner is an executor. Green Goods queues only the two downstream derived funding routes; HoA → working capital remains upstream context. | User-confirmed working-capital flow and settlement trust review through 2026-07-10. |
| 16 | Members receive G$ at same-address smart accounts on Celo only after the AA/bundler/paymaster gate passes. If it fails, automated member delivery and member sends remain blocked while WorkingCapitalToProtocol and ProtocolToGarden may continue; no alternate member-claim path ships. | User decision 2026-07-10; settlement-spec §5. |
...
33. External alignment: the latest full synthesis is canonical. `external-communications.md` defines derivative audience materials and rollout; GoodDollar settlement is on Celo, no G$ bridges to Arbitrum, and House of Alignment funds flow to the Dev Guild working-capital wallet.
...
Focus cohort (decision #23, named 2026-07-10; readiness stays evidence-gated per RESR-62): Tech and Sun Hub (Awka, Nigeria), Greenpill Cape Town (Muizenberg / Deep South Circles), AgroforestDAO / Redemption Hill (Bias Fortes, Brazil), and a fourth garden — in outreach, named only once participation is confirmed (mature MRV-adoption slot; decision #26).
