...
| `reports/client-loop-pr-feedback-2026-08-22.md` | Ledger and disposition for the 50 PR #749 review threads across five rounds: all 50 addressed — 49 fixed with root cause and location, 1 stale — plus the correction that dev mock auth does have a real signer | Dated review evidence — the deferred rows are the remaining work on this branch |
...
| `commitment-walk-artifact.build.ts` | Generator for the Commitment Loop Walk: three tabs, one per surface (client PWA, editorial website, admin console), from a WebP frame tree it does not own. `--prepare-admin` encodes fresh Storybook captures into that tree. Asserts per-tab and whole-document byte budgets, anchor uniqueness, and nav/body parity | Generator — the walk's build-time gate |
| `commitment-walk-frames.json` | Section prose and per-frame captions for all three walk tabs. The pixels live outside the repo (the client frames carry real local garden records and `.plans/` is world-readable); the words are version controlled here | Caption source of truth for the walk |
| `commitment-walk-admin-stories.json` | Storybook story id → frame name for the Admin console tab; consumed by `--prepare-admin` and by `capture-admin-stories.mjs --ids-file` | Curation list — a renamed story fails the prepare step |
...
- [Commitment Loop Walk](https://claude.ai/code/artifact/5351a22a-771f-446b-ba1c-d86c5fcd5380) — what the feature actually looks like as built, three tabs: Client PWA (42 frames from the running app) · Editorial website (19 states, two widths) · Admin console (38 frames). Each tab's closing panels record how it was captured and what it omits (`commitment-walk-artifact.build.ts`)
...
| 62 | **Protocol Safe release readiness depends only on a live threshold >= 2.** Owner count and membership are operationally managed and do not block planning, dry runs, broadcasts, receipt checkpoints, or safe-phase verification. The manifest may retain owner inventories as informational evidence. Garden settlement recovery remains its separate policy. Production indexer deployment remains in PRD-722. | Afo decision amended 2026-08-25. Supersedes the 2026-08-11 exact-owner-set and minimum-owner-count requirements. No transaction is authorized merely by this policy change. |
| 63 | **The Arbitrum ownership handover is one operator command backed by eight independently verified onchain boundaries.** The command prompts once, resumes from the ownership checkpoint, refreshes the deployer nonce before every boundary, and stops on the first failure. | Afo selected the one-command sequential ceremony on 2026-08-25. This avoids unsafe EOA delegation while removing repeated command and password entry. |
...
1. **App-wide Operator → Steward rename**: glossary (generated from `packages/shared/src/ontology/green-goods-ontology.json` → `docs/docs/reference/glossary.generated.mdx`), docs site, i18n keys ×3 locales, admin/client UI copy, vocab-lint update. CP specs/visuals already use steward (mapping note: steward = operator/owner Hats).
