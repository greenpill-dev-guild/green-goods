...
⚠️ **This hub has two independent decision lists, both numbered from 1.** A bare `#N` is therefore ambiguous in the range 1–30, and this has caused real mis-resolutions. Until a full renumber lands:

| List | Range | What it is |
|---|---|---|
| **Decision Log** (the table below) | 1–30 | Curated current-state decisions spanning the whole feature, newest last |
| **Full decision register** (further below) | 1–61 | The 2026-07-03 alignment session verbatim, plus dated addenda 28–61 |

- **`#31`–`#61` are unambiguous** — the Decision Log stops at 30, so those are always the register (`#34` alone is cited 71×).
- **`#29` and `#30` are ambiguous** — the Decision Log entries are respectively “fourth garden not selected” and “protocol-pool settlement parity,” while the register entries are different decisions. Always name the list for these numbers.
...
| 15 | Fund topology: HoA stream → GG protocol Safe (Celo) → garden Celo Safes → members. One deterministic Safe mapping per garden, deployed on demand. Pilot recovery is exactly 2-of-3: protocol multisig, Dev Guild recovery multisig, and named garden recovery delegate; no owner is an executor. Fulfilled protocol-pool commitments settle from the protocol Safe through the same commitment-reward path as every other pool; the separate ProtocolToGarden funding route remains for discretionary, non-commitment garden seeding/top-ups. HoA → protocol Safe remains upstream context. | User-confirmed topology and settlement trust review through 2026-07-10; working-capital hop removed by user decision 2026-07-18; commitment/top-up split clarified by Decision Log #30. |
| 16 | Members receive G$ at same-address smart accounts on Celo only after the AA/bundler/paymaster gate passes. If it fails, Individual commitment rewards and member sends remain blocked; Garden-claim rewards to registered Garden Safes and discretionary ProtocolToGarden seeding/top-ups may continue. No garden-custody member-claim fallback ships. | User decision 2026-07-10; beneficiary-specific gate clarified by Decision Log #30. |
...
| 22 | G$ reward commands derive garden/recipients/amounts from the fulfilled commitment. After canonical Fulfillment, the app durably enqueues the permissionless, idempotent `queueDisbursement(commitmentId)` follow-up; confirmation itself never synchronously calls settlement. A commitment may have exactly one reward disbursement ever: its pointer remains after terminal cancellation, so neither the app nor another permissionless caller can reopen the reward. Garden claims derive the registered `providerGarden` Safe and do not depend on member AA, while Individual claims require `memberDeliveryEnabled`. The separate funding route is exactly ProtocolToGarden and remains steward-initiated for non-commitment garden seeding/top-ups; token, Safe, target, and calldata are never caller supplied. The versioned command/ack tuples are frozen in `settlement-spec.md`; same-key retries are idempotent; acknowledgment retry is independent; only an authenticated success acknowledgment for the subject's current key/attempt produces `Confirmed`. Batch membership is immutable, the compile-time ceiling is 24, and the production limit remains measurement-gated; failed members reconcile individually. | User re-freeze 2026-07-23 plus Decision Log #30; closes arbitrary routes/calls, duplicate-payment retries, redundant post-fulfillment approval, terminal-cancellation reopening, member-AA coupling for Safe-to-Safe rewards, manual assertions, unbounded batches, and unrecoverable batch membership. |
...
| 30 | **Protocol-pool settlement parity with a separate treasury-seeding exception.** The protocol pool is the root garden's normal commitment pool: Garden and Individual claimants use the same claim → accept → work/evidence → confirmation → Fulfilled lifecycle as every garden pool. A Fulfilled `CeloSettlement` commitment becomes the economic authorization for its reward. Once the indexer proves every queue prerequisite and no pointer, any signed-in app may create at most one per-user/per-device `settlement` attempt keyed by `(chainId, commitmentId, userAddress)` and permissionlessly call the fully derived, one-disbursement-ever `queueDisbursement(commitmentId)`. Every send/retry re-reads readiness plus the permanent pointer; competing callers reconcile to the winner, blocked readiness creates no attempt/retry, and local exhaustion is never global. The fulfillment transaction never calls settlement synchronously. Garden claims settle from the protocol Safe to the active registered `providerGarden` Safe without the member-AA gate; Individual claims settle to the provider's Celo AA only when `memberDeliveryEnabled`. `queueFunding(garden, amount)` remains in the initial version solely for discretionary non-commitment G$ garden seeding/top-ups, authorized only to a current protocol steward or SettlementModule owner. Operations route access is capability-gated and deployer alone cannot submit funding. The resulting row is typed Funding with no commitment ID, and no agent or keeper receives value authority. | User scope lock 2026-07-27 in PRD-759 discussion; terminality, multi-device coordination, eligibility completeness, and W24 authority clarified during merge review on 2026-07-28. Reuses one pool and settlement primitive, removes redundant payment approval after fulfillment, preserves failure isolation between confirmation and settlement, and keeps an explicit treasury path for bootstrapping garden liquidity. |

### Full decision register (2026-07-03 alignment session, entries 1–27; dated addenda 28–61)
...
10. Admin placement: garden-pool flows in the Garden workspace; the protocol pool console lives under `/community`; Hub gains a confirmation queue. **Amended by Decision Log #28, register #39, and Decision Log #30's 2026-07-28 authority clarification: `/community/pools` shows Protocol plus the current garden only; all-garden oversight moved to capability-gated Operations, with each write independently authorized and deployer alone insufficient for `queueFunding`.** No top-level `/pools` workspace exists.
...
- [x] Human judgment points surfaced and decided: 30 curated Decision Log entries through the protocol-pool settlement-parity decision (2026-07-27), approved Linear change set (2026-07-04), all 22 readiness findings scope-locked (2026-07-10), and the final four August UI placements locked by register #51 (2026-07-23)
...
- [ ] Hooks, stores, query keys, five member-created field queue kinds (`commitment`, `claim`, `evidence`, `workLink`, `confirmation`) plus a per-user/per-device `settlement` attempt and an online-only wallet `transfer` capability that remains disabled unless the AA gate passes; only the five field kinds use membership waiting. Settlement starts only from the full indexed queue-ready conjunction, is keyed by chain/commitment/user, re-checks readiness + pointer before every send/retry, reconciles competing callers, and creates no attempt/retry for blocked or terminal state; hooks stay in shared
