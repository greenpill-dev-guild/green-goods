...
| 22 | G$ reward commands derive garden/recipients/amounts from the fulfilled commitment. The funding route is exactly ProtocolToGarden; token, Safe, target, and calldata are never caller supplied. The versioned command/ack tuples are frozen in `settlement-spec.md`; same-key retries are idempotent; acknowledgment retry is independent; only an authenticated success acknowledgment for the subject's current key/attempt produces `Confirmed`. Batch membership is immutable, the compile-time ceiling is 24, and the production limit remains measurement-gated; failed members reconcile individually. | User re-freeze 2026-07-23; closes arbitrary routes/calls, duplicate-payment retries, manual assertions, unbounded batches, and unrecoverable batch membership. |
