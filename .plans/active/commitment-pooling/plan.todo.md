...
| 4 | EAS bridge: WorkApprovalResolver try/catch hook (GAP precedent) + steward `syncWorkDecisions` fallback; catch-up must prove the greatest supplied sequence equals the resolver's current maximum before mutation, apply only the current decision, and evaluate Ready after the batch; inactive rejected Work may be unlinked despite historical approval delivery | Automatic state without blocking decision attestations; stale partial catch-up cannot freeze credit, and reversible pre-freeze credit preserves the live Work correction model. |
