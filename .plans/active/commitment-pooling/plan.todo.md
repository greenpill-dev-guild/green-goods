...
| 30 | **Protocol-pool settlement parity with a separate treasury-seeding exception.** The protocol pool is the root garden's normal commitment pool: Garden and Individual claimants use the same claim → accept → work/evidence → confirmation → Fulfilled lifecycle as every garden pool. A Fulfilled `CeloSettlement` commitment becomes the economic authorization for its reward; the app creates a separate durable `settlement` job that permissionlessly calls the fully derived, one-live-disbursement `queueDisbursement(commitmentId)`. The fulfillment transaction never calls settlement synchronously. Garden claims settle from the protocol Safe to the registered `providerGarden` Safe without the member-AA gate; Individual claims settle to the provider's Celo AA only when `memberDeliveryEnabled`. `queueFunding(garden, amount)` remains in the initial version as a protocol-steward/module-owner action only for discretionary, non-commitment G$ garden seeding/top-ups. It is not the normal protocol-pool payout path and grants no agent or keeper value authority. | User scope lock 2026-07-27 in PRD-759 discussion. Reuses one pool and settlement primitive, removes redundant payment approval after fulfillment, preserves failure isolation between confirmation and settlement, and keeps an explicit treasury path for bootstrapping garden liquidity. |
...
- [ ] Hooks, stores, query keys, five member-created field queue kinds (`commitment`, `claim`, `evidence`, `workLink`, `confirmation`) plus a system-created `settlement` follow-up and an online-only wallet `transfer` capability that remains disabled unless the AA gate passes; only the five field kinds use membership waiting, while `settlement` is driven by indexed eligible Fulfillment; hooks stay in shared
