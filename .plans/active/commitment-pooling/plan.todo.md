...
| 31 | Gardener-class Hypercert recognition is allocated equally across fulfilled commitments by default, then within each commitment uses a fixed 20% equal-participation floor plus 80% verified-contribution weight. Eligible contributors need approved linked Work or confirmed evidence attribution. The cycle policy is fixed at open; cycle-close review is read-only except for a reasoned correction. | User architecture amendment 2026-07-28. Cross-commitment raw work units are heterogeneous, while the within-commitment split must still reward people who carried more of the work. |
...
| 33 | Contributor payout plans are explicitly finalized before any child disbursement dispatch. Finalization verifies the recognition and amount-derived payment vectors plus exact conservation, then makes the plan immutable. A zero-child all-retained plan completes at finalization without a self-transfer. Recipients must be frozen eligible contributors and their Celo accounts are derived, never typed as arbitrary addresses. Overall plan status is derived from finalization and child states; partial success is visible, failed contributors requeue independently, and child cancellation never clears the one-plan-per-commitment pointer. | Review closure 2026-07-28. It preserves the bounded settlement state machine while closing the all-retained and stale-child audit gaps. |
...
64. Recognition amendment (2026-07-28, Afo authorization): the cycle-open allocation policy now
    ...
    contribution weights from approved linked Work plus confirmed evidence attribution.
...
65. Payout-plan amendment (2026-07-28, Afo authorization): a fulfilled CeloSettlement commitment
    ...
    Hypercert recognition weights. Plan creation verifies the complete sorted recognition vector
    against its snapshot hash. A steward may atomically edit the complete amount vector before
    finalization; payment weights derive from amounts and any recognition divergence requires a
    visible reason. The plan carries an explicit `gardenRetainedAmount`; declared reward must
...
66. Settlement reuse amendment (2026-07-28, Afo authorization): the provider garden Safe is the
    ...
    Pending/Partial/Complete/Failed view is derived from child states and never acknowledged as a
    separate cross-chain subject. Explicit finalization freezes the plan before dispatch; a
    zero-child all-retained plan completes without CCIP. Child and batch cancellation never clear
