...
| 15 | Fund topology: HoA stream → Dev Guild Working Capital Safe → GG protocol Safe (settlement account of the protocol pool) → two garden Celo Safes at launch → members; funding hops recorded as Funding disbursements; `registerSettlementAccount` is the canonical garden↔Safe mapping | User-confirmed HoA disbursement path, 2026-07-04. |
...
9. [ ] PRD-686 settlement (PR chain 2.5): starts after PRD-672's interface freeze (the module only reads the pooling module), runs parallel with PRD-673/674. Week-1 gates: Celo AA spike + GoodDollar written confirmations. Celo Safe ops (two garden Safes, Zodiac Roles + Allowance config, executor keys) run alongside; SettlementModule deploy + settlement-account registration join the gated broadcast step
