...
| Lead provider | Offer → creator; Individual Request → accepted counterparty; Garden-claimed Request → accepted pending claim's authenticated `requestedBy` operator/owner while `providerGarden = gardenContext` | One accountable human lead; lead alone consumes the register slot; GardenAccount remains provider scope |
| Contributor roster | lead auto-added at acceptance; Open permits eligible self-join/zero-credit self-leave; LeadManaged permits lead/steward changes before freeze; lead or credited contributor removal is rejected; measured `MAX_CONTRIBUTORS_PER_COMMITMENT` bounds the end-to-end vectors | Solo is one contributor; provisional cap 32 is a transaction-safety bound selected by 8/16/24/32 proof, not an arbitrary four-person product rule; every frozen contributor is excluded from confirmation |
| Work linker | active contributor, lead provider, or steward; Work attester must be active; DomainImpact link names the exact requirement index; commitment remains Accepted and unfrozen | One Work UID can increment one contributor and exactly one bound requirement once; a first approval after freeze is observed but cannot change credit, requirements, units, or recognition |
| Evidence attribution | exact CID de-duplicated per commitment; queued payload persists a non-empty, unique, bounded active-roster address vector through retry; attachment must land while Accepted and unfrozen | Credits record before readiness; a late queued attachment fails without partial writes; recognition eligibility additionally requires the commitment to be Fulfilled |
| Cycle terminal transitions | `closeCycle` and `cancelCycle` require `liveCommitmentCount == 0`; creation increments and the first Fulfilled/Cancelled/Expired transition decrements once | Ready and Disputed remain live; reconciliation/cancellation cannot strand a commitment or admit later recognition changes |
...
| Commitment-reward source | `ArbitrumExternal` declares a non-zero payer; `CeloSettlement` declares source zero, then plan creation derives/stores the registered Celo Safe for accepted `providerGarden` | Garden pays its contributors; protocol commitment may use ProtocolToGarden top-up first; never send to Arbitrum GardenAccount |
| Recognition authority | CommitmentPooling recomputes the complete sorted vector/hash from frozen eligible count, on-chain credits, and immutable cycle policy | Hypercert composition and Settlement use the same validator; a steward-supplied self-consistent split is insufficient |
| Payout authority | create/edit/finalize/prepare resolve operator/owner stewardship from immutable `providerGarden`; dispatch executes only the frozen approved plan | Root-pool steward cannot authorize spending from a claimant garden Safe |
...
| Hypercert gardener recognition | gardener class divides equally across fulfilled commitments; within each, opened cycle policy defaults 20% equal eligible participation + 80% verified contribution; cycle-less uses the immutable protocol 20/80 policy | Ready/direct dispute fulfillment uses pre-freeze `totalVerifiedCredits > 0`; recognition eligibility additionally requires Fulfilled with a frozen roster and approved Work or evidence credit; each Work UID counts once; W26 blocks inconsistent legacy/indexed state with no lead or metadata-only fallback |
