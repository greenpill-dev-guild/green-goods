...
| Work linker | active contributor, lead provider, or steward; Work attester must be active; DomainImpact link names the exact requirement index | One Work credit increments one contributor and exactly one bound requirement even when action UIDs repeat |
| Evidence attribution | exact CID de-duplicated per commitment; credited list non-empty, unique, bounded, and every address active | Credits record at attachment; eligibility additionally requires Fulfilled; attribution and requirement assignment stay distinct |
...
| Hypercert gardener recognition | gardener class divides equally across fulfilled commitments; within each, cycle policy defaults 20% equal eligible participation + 80% verified contribution | Eligible = frozen contributor on a Fulfilled commitment with approved Work or evidence credit; deterministic remainder by highest weight then ascending address; zero eligible blocks W26 with no lead fallback; proof-linked reasoned repair preserves before/after |
