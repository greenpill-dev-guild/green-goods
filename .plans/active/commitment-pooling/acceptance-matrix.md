...
| Work linker | active contributor, lead provider, or steward; Work attester must be active; DomainImpact link names the exact requirement index | One Work UID can increment one contributor and exactly one bound requirement once, even when action UIDs repeat or multiple approval attestations exist |
| Evidence attribution | exact CID de-duplicated per commitment; queued payload persists a non-empty, unique, bounded active-roster address vector through retry | Credits record at attachment; eligibility additionally requires the commitment to be Fulfilled; attribution and requirement assignment stay distinct |
...
| Hypercert gardener recognition | gardener class divides equally across fulfilled commitments; within each, opened cycle policy defaults 20% equal eligible participation + 80% verified contribution; cycle-less uses the immutable protocol 20/80 policy | Eligibility additionally requires the commitment to be Fulfilled with a frozen roster and approved Work or evidence credit; each Work UID counts once; zero eligible is rejected before Ready/direct dispute fulfillment, while W26 blocks inconsistent legacy/indexed state with no lead or metadata-only fallback |
