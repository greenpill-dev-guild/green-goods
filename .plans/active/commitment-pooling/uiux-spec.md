...
| Arrange fulfilled Celo reward | `settlement` | `{ commitmentId, gardenAddress }`, created by the app only after indexed Fulfillment when `reward.rail == CeloSettlement`; every payment fact remains contract-derived | No optimistic commitment mutation; reward row shows “arranging settlement” while the separate job runs | Settlement-specific queued/retrying status; never member field-queue chrome | Permissionless `queueDisbursement(commitmentId)`; an exact existing live disbursement is reconciled as success; exhaustion leaves Fulfilled untouched and exposes retry | commitment reward status, settlement disbursement/live pointer |
...
- **CeloSettlement arrangement**: on indexed Fulfillment, the app automatically creates the durable `settlement` follow-up; there is no second payout-approval button. The job permissionlessly calls `queueDisbursement(commitmentId)`, whose canonical G$, owning-pool Safe payer, amount, and beneficiary are fully derived. A Garden claim targets the registered `providerGarden` Safe and does not depend on member AA; an Individual claim targets the provider's Celo AA and requires `memberDeliveryEnabled`. `Record payout` remains unavailable, an exact existing live disbursement reconciles the job as success, and every later delivery state belongs to the settlement row. A queueing failure never rolls Fulfilled back or presents fulfillment as failed.
...
A NEW deployer-gated admin workspace tab — gating pattern copied exactly from Actions: a conditional nav slot (`showOperations: isDeployer` beside `showActions` in `useEffectiveToolbarPermissions`) plus a `RequireRole ["deployer"]` route branch. Stage rail: **Queue · CCIP · Flows**, with a focused **Seed / top up garden** treasury form opened from Flows (W24 draws it).

- **Queue**: every queued/failed disbursement and funding hop across all gardens, with Kind visibly distinguishing CommitmentReward from Funding, batch composition, and the W22 command/ack console. Eligible Fulfilled Celo rewards arrive through the app's automatic `settlement` job; source controls begin at optional batch/dispatch and continue through same-key command retry, authenticated-failure requeue, and queued-only cancel. The protocol executor is an automated Celo contract, not a human role or report control.
...
- Offline queue health rides the existing job analytics (`packages/shared/src/modules/job-queue/job-analytics.ts`): the five member-created field kinds plus the system-created `settlement` kind inherit job_added/completed/failed tracking automatically, with membership-wait metrics applying only to the field kinds. Online wallet `transfer` uses transaction lifecycle analytics, not job replay analytics.
