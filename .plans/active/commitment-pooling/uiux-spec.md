...
**Where the band's three figures come from.** Gardens with live pools = pools whose `CommitmentPool.state` is Open. Commitments fulfilled = the sum of lifetime `commitmentsFulfilled` across those pools; **lifetime, not "this season"** — a season is per-garden, so there is no protocol-wide season to scope to, and §7.1's record framing makes the lifetime figure the consistent one across both surfaces. CCIP-confirmed G$ support has no public-safe aggregate today: `Disbursement`, `SettlementMessage` and `SettlementExecution` exist in the schema but nothing selects a confirmed total, so that selector is a prerequisite of the tile and it must count only acknowledged deliveries (§7.4, settlement-spec §3.0).
...
| Public rate threshold helper | §7.2 | `selectPromiseKeptRate` (`packages/shared/src/modules/commitment-pooling/disclosure.ts`) applies no gate at all. Propose a shared selector that takes due count + distinct-provider count and returns either a rate or a counts-only shape, so editorial and the WalletDrawer cannot drift apart on the same rule. Its provider input is the pool-scoped `distinctProviderCount` (§7.2), not a list |
| Public pool reader | §7.1 | `getCommitmentPoolDetail` (`packages/shared/src/modules/commitment-pooling/data-pools.ts`) selects `CommitmentProviderExposure { provider }`, so a signed-out page reusing it ships provider addresses to the browser. No pooling hook exists in `packages/shared/src/hooks/public/` at all. Propose `usePublicGardenPool` beside the other eight public hooks, selecting pool, cycle, and unit-summary fields only |
| Public settlement aggregate | §7.3 | Nothing selects a CCIP-confirmed total. `Disbursement`, `SettlementMessage`, `SettlementExecution` carry the lifecycle; propose a shared selector returning the acknowledged total only, so dispatched and executed-ack-pending can never reach the tile |
