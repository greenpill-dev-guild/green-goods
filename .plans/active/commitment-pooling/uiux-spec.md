...
Queue substrate (verified): IndexedDB + XState, exactly two kinds today (`work`, `approval`; `packages/shared/src/types/job-queue.ts:89-92`), `MAX_RETRIES = 5` (`packages/shared/src/modules/job-queue/index.ts:88,247-248`), kind dispatch is a branch in `processJob` (`index.ts:277-288`), executors in `modules/job-queue/job-executors.ts`. New offline kinds (`commitment`, `claim`, `evidence`, `workLink`, `confirmation`, `settlement`) extend `JobKindMap` and the dispatch branch; `transfer` is a shared online wallet action kind that bypasses the offline queue. The `settlement` kind is an explicit per-user, per-device submission attempt rather than a singular global job: its local IndexedDB identity is `(chainId, commitmentId, userAddress)`, while the permanent onchain commitment-disbursement pointer is the global coordination and deduplication boundary. Naming follows the existing single-noun convention.

Membership wait (register #34c): the five member-created field job kinds run a pre-flight membership check before their first send attempt; a job whose account holds no garden hat enters `waiting_for_hat` — consuming no retries — and resumes automatically when the membership event lands (the join-request approval of register #35 is the canonical trigger). The app-created `settlement` attempt does not wait for a member Hat. The indexed read model first exposes a reward as queue-ready only when every source and beneficiary precondition is true and no historical pointer exists. Any signed-in app may then create at most one attempt for its local `(chainId, commitmentId, userAddress)` key. Before every send or retry it re-reads eligibility and the permanent pointer: an exact existing pointer reconciles success; any terminal pointer suppresses the attempt permanently; a competing caller that loses with `CommitmentAlreadyDisbursed` refetches and reconciles to the winning pointer. Local exhaustion is device state only, never a claim that the reward is globally exhausted. There is no agent or keeper value authority. The ≥99% offline-sync metric excludes time in `waiting_for_hat` (`acceptance-matrix.md` §6).
...
| Arrange fulfilled Celo reward | `settlement` | `{ chainId, commitmentId, gardenAddress, userAddress }`, created at most once in this device's IndexedDB for that key only after the indexed read model reports queue-ready; every payment fact remains contract-derived | No optimistic commitment mutation; reward row shows “arranging settlement” while this app's attempt runs | Settlement-specific queued/retrying status; never member field-queue chrome | Before every permissionless `queueDisbursement(commitmentId)` attempt, re-read eligibility + permanent pointer. Exact pointer or duplicate-race winner reconciles as success; any terminal pointer suppresses retry; local exhaustion leaves Fulfilled untouched and exposes retry only if the reward is still queue-ready. Individual delivery disabled or any source/route prerequisite false creates no job and exposes no retry | commitment reward status, settlement eligibility/block reason, disbursement/history pointer |
...
- **CeloSettlement arrangement**: indexed Fulfillment plus the complete settlement preconditions makes the reward queue-ready; there is no second payout-approval button. A signed-in app creates at most one device-local `settlement` attempt per `(chainId, commitmentId, userAddress)` and permissionlessly calls `queueDisbursement(commitmentId)`, whose canonical G$, owning-pool Safe payer, amount, and beneficiary are fully derived. Every send and retry first re-reads the permanent commitment pointer and current eligibility. An exact pointer, including one created by a competing device, reconciles success; terminal pointers suppress all later attempts. A Garden claim targets the active registered `providerGarden` Safe and does not depend on member AA; an Individual claim targets the provider's Celo AA and requires `memberDeliveryEnabled`. If that gate or any source/route prerequisite is false, the UI shows a no-retry blocked state and creates no attempt. `Record payout` remains unavailable, and every later delivery state belongs to the settlement row. Device-local queued/retrying and exhausted arrangement states stay distinct from onchain Queued and authenticated execution Failed. A submission failure never rolls Fulfilled back or presents fulfillment as failed.
...
2. Add `/community/pools` inside the existing Community route branch in `packages/admin/src/routes/views.tsx`, using the same Community authorization boundary. It renders the Protocol pool plus the current garden only. Existing protocol-configuration actions keep their own current role gates inside the mode; cross-garden inspection belongs exclusively to the capability-gated Operations workspace.
...
### 6.11 Operations workspace NET-NEW (decision 2026-07-18; authorization corrected 2026-07-28)

A NEW capability-gated admin workspace tab. `useEffectiveToolbarPermissions` derives `showOperations = isDeployer || canQueueFunding || canOperateSettlement`; the route admits the same capability set rather than using a deployer-only `RequireRole`. Action-level checks remain authoritative inside the workspace. In particular, `canQueueFunding` is read from current onchain authority (`protocol steward || SettlementModule owner`), never inferred from `isDeployer`: a deployer without either authority may inspect or operate only the capabilities it actually holds and cannot submit or retry funding. Stage rail: **Queue · CCIP · Flows**, with a focused **Seed / top up garden** treasury form opened from Flows (W24 draws it).

- **Queue**: every queued/failed disbursement and funding hop across all gardens, with Kind visibly distinguishing CommitmentReward from Funding, batch composition, and the W22 command/ack console. Eligible Fulfilled Celo rewards arrive when a signed-in app submits a permissionless device-local `settlement` attempt coordinated by the permanent onchain pointer; source controls begin at optional batch/dispatch and continue through same-key command retry, authenticated-failure requeue, and queued-only cancel. Funding rows have no commitment ID. W21/W22 remain garden-scoped detail and recovery routes; W24 is the primary all-garden execution home. The protocol executor is an automated Celo contract, not a human role or report control.
...
- **Flows**: the cross-chain funds board — GoodDollar pool → GG protocol Safe (a **Celo balance read**, since HoA→protocol is an upstream fact the module never records, corrections-log §9) → garden Safes → members, each downstream figure distinguishing queued, dispatched, Celo-executed/ack-pending, confirmed, failed, and delayed. The GG protocol Safe → garden Safes row exposes **Seed / top up**: an explicit protocol-steward/module-owner-only `queueFunding(garden, amount)` form for discretionary support outside a commitment. Its review names the registered destination Safe and canonical G$, states that it does not fulfill/reward/alter a commitment, and never grants an agent/keeper value authority. Garden-claim rewards and this funding route remain Safe-to-Safe when member delivery is disabled; Individual rewards do not. The cross-garden pool oversight rows (formerly §6.8's Gardens tab) live here: alphabetical, `promiseKeptRate` + `openCommitmentCount`, never ranked.
...
- Offline queue health rides the existing job analytics (`packages/shared/src/modules/job-queue/job-analytics.ts`): the five member-created field kinds plus each app-created, device-local `settlement` attempt inherit job_added/completed/failed tracking automatically, with membership-wait metrics applying only to the field kinds. Settlement attempt metrics are explicitly local-device observations and must not be reported as a count of globally unique reward jobs. Online wallet `transfer` uses transaction lifecycle analytics, not job replay analytics.
...
5. **Where settlement controls sit**: G$ settlement uses `SettlementModule` + bounded Celo executor events, not the reserved pool `settlementEnabled` flag. Admin Garden Pool gains the settlement status section; PWA commitment detail gains settlement-status rows only after the canonical contract/indexer interfaces are GREEN; `/community/pools` shows the Protocol pool plus the current garden only. Cross-garden command/ack health and funding controls live in the capability-gated Operations workspace, with funding specifically requiring current protocol-steward or SettlementModule-owner authority. Later transferable-voucher controls sit behind `settlementEnabled` when PRD-651 unblocks. There is no top-level `/pools` route (§2).
