...
| Fulfilled | Fulfilled hero moment (§5.10), chip, declared-reward row; timeline names ordinary confirmer, “garden steward — fallback,” or “Green Goods team — fallback” only from indexed path provenance | Reward row + "record payout" action; exact confirmer/path/reason shown | Fulfilled counts + promiseKeptRate | Testimony CTA when aimed at community |
...
Submission recovery: before broadcasting any `commitmentSeries` job, the runner persists the exact
signed transaction or UserOperation, its locally derived hash, and `submitted` state. Restart
recovery polls that hash before sending. If it was never observed, only the identical signed
payload may be rebroadcast; a fresh nonce or newly prepared series-creation call is forbidden.
This closes the broadcast-before-`markJobSynced` window without exposing `clientSeriesId`
onchain.
...
checkbox; *Reward* carries the declared rail and amount. Five is the locked
