...
- The 2026-07-28 amendment and review closure are binding across every lane: group commitments use one accountable lead plus contributors; requirements accept only action/count inputs; the roster freezes at ReadyForConfirmation; Hypercert recognition is 20% equal eligible participation plus 80% verified contribution with no lead fallback; settlement hash-binds recognition, derives payment weights from atomic amounts, explicitly finalizes before dispatch, conserves retained plus child amounts, completes all-retained plans without CCIP, and never clears the parent pointer on child cancellation. This is a payout plan, not a garden-held member claim.
