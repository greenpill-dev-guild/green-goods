...
| AC-8 | Card Endow receiver | Thirdweb Card Endow requests require a user-owned recovered-wallet `receiverAddress`. | `state_api` | Shared `validateOctantVaultCardEndowReceiver`, `prepareOctantVaultCardEndowReadiness`, and agent funding-intent validation reject missing or mismatched recovered-wallet receivers before checkout state is created. 2026-06-12 hardening adds route-exact onramp gates: `validateOctantVaultCardOnrampQuote` and `validateOctantVaultCardOnrampCompletion` reject any prepared quote or COMPLETED session whose chain, token, receiver, amount, or purchase tuple disagrees with the recovered-wallet expectation before approve/deposit is offered. |
| AC-9 | Card Endow share proof | Vault shares are verified for the recovered wallet before Card Endow is marked share-verified. | `state_api` | Shared `validateOctantVaultShareOwnershipProof` requires owner/receiver match, positive share balance, matching vault, and visible shares for both pilot vaults. 2026-06-12 hardening (`ca05de903`, codex P1 fix): the agent proof route now performs its own `vault.balanceOf(receiverAddress)` read through chain-routed RPC (`readVaultShareBalanceOnChain` + `resolveAgentRpcUrl`) and records the server-verified amount; client-supplied share claims are never recorded, and zero balance, read failure, or a missing verifier all return 409 fail-closed. Live positive-share proof remains human-gated. |
