...
| `recordRepayment(uint256 loanId, uint256 amount, bytes32 executionRef)` | current steward or registered pool executor, never the loan borrower | Disbursed or Defaulted; Jar/Treasury only; positive non-overpayment; unique non-zero reference; exact outstanding conservation; installment count; exact clearance emits Repaid; recovered default stays visible in prior event history |
