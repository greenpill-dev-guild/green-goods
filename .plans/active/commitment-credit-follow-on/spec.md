...
**Decision basis**: User decision 2026-07-05 — Green Goods should carry GE's **borrow-and-repay** loop, scoped now as a **dedicated `CreditRegistry`** (over the lighter reuse-the-commitment-machinery alternative), because gardens already run mutual credit with installments and default tracking. Borrow-and-repay was consciously deferred, not overlooked: historical planning offered "Mutual Credit and Farmer Verification" during onboarding but never gave it an enum value or action set; the current four-domain implementation and `../../active/commitment-pooling/reports/corrections-log.md:82` confirm it never shipped. The deliberate no-custody posture (decision #8, `../../active/commitment-pooling/contract-spec.md:54`) also discouraged lending. The pooling module models promise→confirm→consideration (mutual aid of *labor*) and never modeled capital borrowing. The 2026-08-01 scope lock moved this companion chain into the August wave. The three contracts dispatch gates cleared on 2026-08-09; exact evidence and ABI coverage live in [coverage-ledger.md](coverage-ledger.md).
...
- **Historically deferred mutual credit** — Season-One planning offered "Mutual Credit and Farmer Verification" during onboarding but never gave it an enum value or action set; `../../active/commitment-pooling/reports/corrections-log.md:82` and the ontology's closed four-domain set confirm it never shipped. It is realized here as a **cross-cutting credit layer** over any domain, **not** a fifth `Domain` enum value (domains stay SOLAR/AGRO/EDU/WASTE; the immutable Work/Assessment domain fields are unaffected). Farmer verification maps to the assessment-v3/testimony gates, not here. The `creditIssued` aggregate (§6) carries the durable aggregate-reporting intent.
