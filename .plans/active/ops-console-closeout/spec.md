...
2. `admin-ui-revamp` PostHog dashboards and funnels reflect the new admin route paths (D54).
3. `admin-ui-revamp` es/pt translation pass completes after the agent has formalized `en.json` for the revamp string set (D56).
...
- Risk: Native translator availability delays the es/pt pass past the admin-ui-revamp close window.
