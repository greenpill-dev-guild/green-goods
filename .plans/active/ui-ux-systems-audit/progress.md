...
### Wave C / D — EXECUTED AND MERGED (same day, follow-up session segment)

- **P2 (Wave C) → [#608](https://github.com/greenpill-dev-guild/green-goods/pull/608) MERGED**: FormattedAmountInput + useFormattedAmountInput (shared/Form, tests+story), TransactionSuccessAffordance (screen/receipt/toast/none, tests+story), WalletConnectButton (client); adopted across send/withdraw/deposit/claim/endow behavior-preservingly; donate+claim success toast (Afo's decision) with en/es/pt keys; fund card gains consistent connect-loading (conscious deviation: kept the editorial submit button). Runtime-verified on /cookies with real fork data (typing, error, gating).
- **P4+P5 (Wave D) → [#609](https://github.com/greenpill-dev-guild/green-goods/pull/609) MERGED**: first-run browse-all CTA + hint on the empty garden list; skeletons say what's loading; offline surfaced early (send flow banner, withdraw reason, pull-to-refresh note); aria-current (verified live)/role=group/QR label/purpose-required a11y; P4.5 verified-no-change; em-dash pass (8 converted + keep log above), 3 warm rewrites, stale-price severity >2min; endowment error state + dismiss guard + live validation verified already-shipped; donate contributed-echo = data-dependent follow-up.
