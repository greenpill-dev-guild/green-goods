...
|---|---|---|---|
| Resend captures public subscribers through existing route | `state_api` | Steps 1-3 | Complete for state/API |
| Existing validation/CORS/rate-limit behavior remains intact | `state_api` | Steps 2-3 | Complete for state/API |
| Provider errors are neutral and translated | `state_api`, `ui` | Steps 3-4 | Shared contract/i18n complete; UI lane remains ready |
| Client form keeps same behavior and copy intent | `ui` | Step 4 | Not started |
| Root env/config/docs identify Resend requirements | `state_api` | Step 5 | Complete for state/API |
...
- [x] Step 1: add a Resend subscription client in `packages/agent` with focused adapter tests.
- [x] Step 2: change Agent config/bootstrap/server deps from Luma-specific names to a
  provider-neutral subscription client.
- [x] Step 3: preserve public subscribe validation while routing success/failure through Resend.
- [x] Step 4: update shared public error contracts and all three locale files.
- [x] Step 5: update docs/config references for required root env variables, segment/topic IDs, and
  Luma shutdown notes.
...
- [x] Focused Agent tests after provider rename, for example
  `bun run --filter @green-goods/agent test -- resend public-api`
- [x] Focused Client test:
  `bun run --filter @green-goods/client test -- PublicGetInTouch`
- [x] Vocabulary/i18n guard if strings change: `bun run lint:vocab`
- [ ] Quick repo verification before PR: `node scripts/dev/ci-local.js --quick` _(ran; aggregate remains non-zero because the existing shared vault two-step deposit test timed out outside this migration surface; see `handoffs/codex-state-api.md`)_
