...
**Stage**: `backlog` — the join-request queue shipped in PR #779 and is switched on for QA; production activation gate remains
**Created**: 2026-07-12
**Companions**: `spec.md` §§7 and 9 (member and Garden-admin placement); `research-plan.md` (RESR-64 operating gate); `status.json` (manual blocker); `../../active/commitment-pooling/plan.todo.md` decision #35 (pool-job consumer).
...
Commitment Pooling consumes the result only: its five August pool job kinds enter `waiting_for_hat` before a send and resume when membership is observed (`../../active/commitment-pooling/uiux-spec.md:205-225`). The queue never grants membership.
