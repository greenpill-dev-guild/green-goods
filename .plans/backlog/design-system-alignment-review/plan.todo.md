...
### Open decisions (awaiting Afo's lock — graduate to DL rows when locked)

- [ ] **Dark filled-action fills**: shipped = deep fills + white text in dark (deliberate 2026-07 ruling, AA-forced: 7.3/5.7/5.0/6.5/7.6). Afo flagged on 2026-08-29 review that this reads as not dark-mode-adapted; the M3-dark convention (and strict-M3 fidelity) is tonal −200 fills + −900 ink. Both options render side by side on the reference page § "Dark Filled Actions". If tonal is locked: re-point `--tone-action/--tone-action-hover/--tone-on-action` in the five dark tone blocks, add the DL row, bump token_version, update language.md's dark table.
- [ ] **Storybook hosting**: the reference page's story links need a deployed Storybook to work off-machine. Options: GitHub Pages workflow on develop pushes, or a Vercel static project over `storybook-static`. New deployment infrastructure — Afo picks; until then links target local :3004.
