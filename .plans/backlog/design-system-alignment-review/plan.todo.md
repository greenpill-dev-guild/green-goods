...
### Design-critique fix pass — Afo locked all four + AdminSelect (2026-08-29 critique)

Critique verdict recorded first: three of the five reported inconsistencies (FilterChip sizes, search-toolbar radius, FAB fill/size) were reference-page specimen infidelity, fixed on the artifact; two were real code drift, fixed below. Card registry (13 cards, verified story ids) added to the Cockpit page.

- [x] AdminSelect primitive + all input migrations + FAB/checkbox debt burns.
...
- [x] W1 compact scale (AdminButton 28/32/40, fields 44/14px, inline 32, pills 36, tabs 36) — `AdminDensityScale.guard` RED→GREEN.
  **Receipt**: 2026-08-30T02:31Z, commit `f9c6bedf3` (+ ceiling trim `f051e4ae3`). Guard 4 failed → 4/4. Bonus latent bug caught by the guard: tailwind-merge dropped `text-body-*` against `text-[rgb(...)]` — field text was browser-default 16px by luck; `[color:...]` form now.
- [x] W3 field/error unification — same commit `f9c6bedf3`: AdminInlineField owns label+supporting on m3 roles (new `AdminInlineField.test` pins m3-error/role=alert/inset/no shake-error/no error-dark); GardenChip create row on cockpit roles; SignalPool `...Label` key split (en/es/pt renamed, values preserved).
- [x] W2 Title Case sweep — commit `e143966ca` (200 files): 345 en values + value-mapped defaultMessages + test/story literals; es/pt untouched (verified: 4-line diff = the 2 key renames). Reverted one over-catch ("Open now" cycle state — the section test correctly rejected it). Hardcoded action labels outside i18n flipped too (index.html boot retry, shared ErrorBoundary, useInstallGuidance). Fallout closed: label-derived testid, sentence literal, 2 case-sensitive play regexes, mock JSX text, and VaultManagePositions' no-/fund-link assertion scoped to the success region (footer wayfinding is legitimate since `439196599`).
- [x] Canon — commit `451b4d4f4`: DL-011 + DL-012 rows, token_version **2.9.0**, 8 guidance files incl. rules/frontend-design Rule 18 and the underline-default field-anatomy contract.
- [x] W4 gallery — commit `0d60a8ab8`: `build-story-gallery.mjs` after storybook build emits `gallery/index.html` — **309 components · 1406 stories · 33 groups**, real lead-story iframes, mount queue (4 loading / 16 live), filter; vercel /gallery cache rule; `storybook-static` launch entry. Verified locally in the browser pane: nav, filter → AdminButton, live frame rendering the compact filled pill from this build. Artifact slimmed to the token/decision digest linking /gallery (hand-drawn specimens retired).
- [x] Validation — suites: admin **805/805** · client **969/969** · shared **4447/4447** · storybook play **191/193 → 193** after the 2 real regex fixes (remainder = the known Actions load-flake, passing isolated ×2); gates: design-md ✅ design-tokens (2.9.0) ✅ guidance-links 65 ✅ skill-behavior 15 ✅ lint:vocab ✅ locale-coverage ✅; typecheck:source clean ×3. Pushed `864707442..f051e4ae3` **with full pre-push hooks green** (the earlier third-party format blocker cleared when lint-staged's stash cycle formatted that file).
  Concurrency note: a third actor (not green-goods-d7) staged work-approval WIP mid-commit twice; both leaks caught by post-commit `git show --stat` review and surgically re-committed (`f9c6bedf3`, `e143966ca` are clean; nothing foreign pushed).
