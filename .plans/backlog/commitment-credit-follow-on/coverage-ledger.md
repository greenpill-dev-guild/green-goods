...
| `configurePoolCredit(uint256 poolId, address token, uint256 borrowerCap, bool enabled)` | current pool steward or module owner | known pool; ordinary mutation pause gate; first non-zero token fixes the pool denomination permanently; later calls keep that token; exact configuration event |
...
| `requestLoan(RequestLoanParams params)` → `uint256 loanId` | self member, or current steward using distinct non-zero `onBehalfOf` | Open/enabled pool; current borrower membership; token exactly matches the pool's immutable denomination; non-zero principal; future non-zero due date; non-empty terms; zero fee; optional commitment exists in same pool; cap; one linked loan; event carries borrower and human requester |
| `approveLoan(uint256 loanId)` | current pool steward or module owner | Requested with a future due date; approver is not borrower; original self-member or `onBehalfOf` steward authority is still current; Open/enabled pool; cap recheck and Approved-exposure reservation |
| `recordDisbursed(uint256 loanId, LoanRail rail, bytes32 executionRef)` | current steward or registered pool executor | Approved with a cap reservation; Open/enabled pool; `rail != None`; Jar/Treasury require a future due date, recheck current committed exposure, and use a unique non-zero steward-attested reference only when no settlement child has ever existed for the loan; any child state fails closed with `SettlementChildExists`; G$ derives a Confirmed exact loan child and exact domain-separated reference after dispatch-time due-date/cap proof, and remains recordable after transit crosses the due date; converts reservation to outstanding once |
| `recordRepayment(uint256 loanId, uint256 amount, bytes32 executionRef)` | current steward or registered pool executor, never the loan borrower | Disbursed or Defaulted; Jar/Treasury only; positive non-overpayment; unique non-zero reference; exact outstanding conservation; installment count; partial repayment retains the live commitment link; exact clearance emits Repaid and clears the matching link; recovered default stays visible in prior event history |
...
| dependency setters + `setPaused(bool)` | module owner | dependency changes only while paused; zero rejected; Hats, pooling, settlement, and the Settlement-side credit-registry replacement all require zero Approved-loan reservations; unpause requires all dependencies; every change emitted |
| UUPS authorization | module owner | upgrade only while paused; old-layout state survives |
| views | public | `getLoan`, `poolCreditConfig`, `outstandingOf`, `reservedOutstandingOf`, `isCapReserved`, `activeReservationCount`, state-aware `amountDue`, `loanOfCommitment`, `isExecutor`, `loanOfExecutionRef`, dependency and pause getters |

### `ISettlementModule` scoped addition

| Selector/change | Frozen requirement |
|---|---|
| `queueLoanPrincipal(uint256 loanId)` → `uint256 disbursementId` | Current pool steward; configured and unpaused `creditRegistry`; exact registry↔settlement and pooling identity; Approved loan with a future due date; Open/enabled pool with distinct rejection errors for pool state and disabled credit; cap; active pool-garden Safe; canonical G$; borrower recipient; principal; idempotent relationship; batch and dispatch repeat the operational, due-date, and loan-fact proof |
...
| `loanPrincipalRelationshipOf(uint256 disbursementId)` | Exact registry+loan relationship for authenticated credit recording; survives batch, dispatch, retry, acknowledgment, stranded failure, requeue, cancellation, and upgrade; retry is refused once the snapshotted executor can no longer acknowledge, preventing a post-grace value move from becoming unconfirmable |
| `setCreditRegistry(address registry)` | Owner, source paused, non-zero, zero active reservations in the previous registry, exact old/new event |
...
`CreditRegistry` declares 11 custom entries and a 39-slot gap: three dependencies, `nextLoanId`, five domain mappings (`loans`, pool configuration, borrower outstanding, commitment link, executors), the execution-reference replay mapping, and `paused`. The total is exactly 50 custom entries; inherited upgradeable layouts remain independent. ERC-7201 namespace `green.goods.credit.cap-reservation` stores Approved exposure by borrower, a per-loan reservation bit, and an aggregate active-reservation count without consuming or shifting a linear slot. The aggregate locks settlement replacement until every Approved exposure is either recorded or safely cancelled. `ERC7201Namespaces.json` and the Bun-owned storage checker freeze this namespace and `green.goods.settlement.loan` by derived slot plus ordered member declarations; the settlement baseline also freezes the nested `LoanPrincipalRelationship` value schema so a mapping's stable top-level declaration cannot conceal an incompatible struct change. Annotation extraction is bound to the immediately associated struct, referenced structs must be unique, and every manifest source must resolve inside the contracts package. The ordinary compiler layout baselines remain enforced alongside these namespace checks.

## Event and error coverage

Events cover initialization identity; dependency and pause changes; pool token/cap configuration; executor changes; request, approval, disbursement, installment, exact repayment, default, and cancellation; and the settlement loan relationship. `RepaymentRecorded.newOutstanding` is the loan's remaining balance in the pool's configured token. Custom errors cover zero/unknown identities, CreditRegistry pause at settlement validation, membership/steward/recorder authority, on-behalf confusion, self-approval, borrower self-repayment, distinct non-Open-pool and disabled-credit causes, immutable pool-token denomination, cap, missing or active reservations, commitment mismatch/duplicate, terms/token/principal/due-date validity, loan state, rail, any cross-rail settlement child, settlement relationship/confirmation/cancellation, retired-peer retry, G$ repayment disablement, duplicate Safe assignment, zero/replayed reference, zero/overpayment, due date, cancellation state, and reason requirements.

## Adversarial coverage map

| Risk | Proof target |
|---|---|
| Self approval / on-behalf confusion | self path, steward distinct-member path, non-steward and self-valued `onBehalfOf` reverts; borrower cannot self-record repayment even when also a steward/executor |
| Intervening cap or administrative stop | immutable per-pool token denomination; request and approval rechecks; approval-time namespaced reservation; settlement queue/dispatch proof; queued-child dispatch rejection while the registry is paused; Jar/Treasury record proof; conversion/release conservation |
