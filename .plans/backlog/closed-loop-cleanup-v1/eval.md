# Closed Loop Cleanup V1 Evaluation Plan
...
|---|---|---|---|
| AC-1 | `.plans/backlog/closed-loop-cleanup-v1/` contains real brief/spec/plan/eval/status/metrics content plus lane handoffs | `state_api` | hub files |
...
| AC-7 | the prompt delegates implementation to Codex on `codex/state-api/closed-loop-cleanup-v1` and does not invent new scheduler or lane semantics | `state_api` | prompt file |
...
1. Confirm the hub exists only under `.plans/backlog/closed-loop-cleanup-v1/`
2. Read the Claude prompt and verify that it blocks immediately until the hub is promoted to
   `.plans/active/closed-loop-cleanup-v1/`
...
4. Confirm the prompt selects one cleanup surface only, delegates implementation to Codex on
   `codex/state-api/closed-loop-cleanup-v1`, then hands QA back to Claude only after Codex passes
