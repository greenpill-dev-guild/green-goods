...
- Ensure the parent body points to `.plans/backlog/yield-split-ui/`.
- Keep `linear.lanes` empty while this hub is in backlog.
- Do not create or reactivate lane issues for `ui`, `state_api`, or `contracts` until implementation starts.
- Confirm the `Operator Yield Split Visibility & Presets` project is still the right routing if this hub is promoted.
...
- `node scripts/harness/plan-hub.mjs validate`
- `node scripts/harness/plan-hub.mjs linear-sync --feature yield-split-ui --json` returns parent-only while backlog and lane manifests only after activation
- `git status -sb`
