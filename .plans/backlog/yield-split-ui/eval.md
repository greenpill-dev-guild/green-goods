...
| AC-9 | Linear sync manifest stays parent-only while the hub is in backlog, then includes `ui`, `state_api`, and `contracts` lanes after activation | `qa_pass_2` | `linear-sync --json` output |
...
- Confirm `node scripts/harness/plan-hub.mjs linear-sync --feature yield-split-ui --json` is parent-only while this hub remains in backlog.
