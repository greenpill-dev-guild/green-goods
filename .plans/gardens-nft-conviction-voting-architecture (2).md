...
**Created**: 2026-02-05
**Updated**: 2026-02-11 (Added canonical source notes after conviction voting review)
**Branch**: `nft-voting-power` forked from `streaming-pool` on `upstream/1Hive/gardens-v2`
**Context**: Integrating NFT voting power, ~~delegation,~~ and sync power into Gardens V2 conviction voting

> **Canonical Repo Boundaries (Feb 2026)**:
> - **Gardens repo** (`greenpill-dev-guild/gardens`): All conviction voting contracts including
>   HypercertSignalPool, CVSyncPowerFacet, IVotingPowerRegistry, NFTPowerRegistry, and the
>   diamond facet modifications described in this spec.
> - **Green-goods repo**: Only the Hats.sol integration layer (`_syncConvictionPower`,
>   `ICVSyncPowerFacet` interface, `ConvictionSync.t.sol` tests) and shared frontend
>   hooks/types that consume the gardens-deployed pool.
> - A duplicate HypercertSignalPool in green-goods was identified and deleted during the
>   Feb 2026 conviction voting review. The `reclaimStakes()` function was migrated to
>   gardens before deletion.
...
| # | Decision | Impact | Corantin feedback |
|---|----------|--------|--------|
| 1 | **NFT Voting Power via Power Registry** | New `votingPowerRegistry` field on pool config. Defaults to community registry. NFT addresses derive voting power. |  |
| 2 | **Delegate Function for Conviction** | ~~Add `delegate()` so a smart account (Garden contract) can control conviction updates on behalf of members.~~ | Should be done inside the custom power registry |
| 3 | **Sync Power Facet** | New facet called by Hats registry module to update conviction on NFT revocation/minting. | It should update supports, conviction will follow by maths |
| 4 | **Make Covenant Optional** | Remove covenant as a requirement for community creation. | Its currently only a UI restriction but I would approach it with a default covenant instead |
| 5 | **~~Eliminate Initial Staking Requirement~~ — REJECTED for Phase 1** | Staking remains required. Investigated: removing ValueCannotBeZero is safe (no division-by-zero), but members with no stakes have no voting power on non-Custom PointSystems. | Keep staking required for Phase 1. Can revisit post-Phase 1. |
| 6 | **~~Move Configuration to Pool Level~~ — Phase 1: votingPowerRegistry only** | Pool-level `votingPowerRegistry` is the only pool-level config change for Phase 1. No other config moves to pool level. | Corantin confirmed this was discarded broadly. |
| 7 | **Voting Weight from Balance (Not Staking)** | Check wallet `balanceOf()` instead of requiring token lock. Avoids decreasing voting weight in other DAOs. | Would be achievable on a custom VotingPower registry |
| 8 | ~~**Batch Join + Activate via Multicall**~~ | ~~Consolidate join + activate governance into a single transaction for UX.~~ | Currently impossible to join on behalf of another one but I think we don't want this for phase 1. But remember with Matis change, we will be able to join from a pool. |
...
| C | Reduce audit surface | Cut staking from V2 main codebase entirely | I disagree, staking should still exists even if optional for specific scenario but it still serve the scenarios where a community use a common token like USDC but members still need to decide on which community they want to cast they voting power without being able to apply it everywhere (USDC communities) |
...
Both `CVStrategy` and `RegistryCommunity` are a combination of **UUPS** then **EIP-2535 Diamond proxies**:
...
4. **Instant Revocation** — When a Hat/NFT is revoked, proposal supports updates immediately
5. **Single-Transaction UX** — Join + activate in one multicall (still need to check if feasible since activate have a contract check on membership)
6. ~~**Delegation** — Smart accounts (Garden contracts) can manage support on behalf of members~~ (I wouldn't do that now, it looks too complex)
...
New Interfaces:
├── IVotingPowerRegistry           (NEW — abstract power source)
└── INFTPowerRegistry        (NEW — NFT-specific power)
```

### 2.3 Power Registry Interface

The meeting decided each pool gets a `votingPowerRegistry` field that defaults to the community registry. This abstracts the power source:
...
interface IVotingPowerRegistry {
    ...
    /// @notice Get the underlying asset address (NFT contract, ERC-20 token, etc.)
    /// @dev Used by Gardens contracts to access the power source token for display/validation
...
For NFT-only pools, a new `NFTPowerRegistry` contract replaces the community registry as the IVotingPowerRegistry:

```solidity
/// @title NFTPowerRegistry
/// @notice Derives voting power from NFT ownership + Hats Protocol roles
/// @dev Implements IVotingPowerRegistry. Configured per-pool at creation time.
...
Add `votingPowerRegistry` to init params:
...
struct CVStrategyInitializeParamsV0_4 {
    ...
    address votingPowerRegistry;     // NEW — defaults to registryCommunity if address(0)
}
```

**Whitelist validation** (from Corantin's security feedback, section 6.6):

```solidity
// In CVAdminFacet or pool creation logic:
mapping(address => bool) public allowedVotingPowerRegistries;

function setAllowedVotingPowerRegistry(address _registry, bool _allowed) external {
    onlyCouncilSafe();
    allowedVotingPowerRegistries[_registry] = _allowed;
}

// At pool init / param update:
require(
    address(params.votingPowerRegistry) == address(0) ||
    allowedVotingPowerRegistries[params.votingPowerRegistry],
...
### 2.6 Delegation Function — DEFERRED (Post-Phase 1)

> **Removed from Phase 1 scope** per Decision #10. The sync power mechanism (section 2.7) handles the core use case of updating conviction on NFT revocation without requiring delegation. Delegation may be revisited in a future phase if needed for more complex governance patterns.

Original meeting note: _"Add delegate function for conviction."_ Corantin feedback: _"Should be done inside the custom power registry."_
...
contract CVSyncPowerFacet is CVStrategyBaseFacet {
    ...
    function syncPower(address _member) external {
        ...
        // Old power from community's cached value, new power from registry's live read
        uint256 oldPower = registryCommunity.getMemberPowerInStrategy(_member, address(this));
        uint256 newPower = votingPowerRegistry.getMemberPower(_member, address(this));
...
The Green Goods `HatsModule` already manages hat minting/revocation. Add a callback to sync power on **revoke only** (grant doesn't need sync — members activate manually via `activatePoints()` which reads current power at that time).

**Sync Interface Design**:
- **Trigger**: Revoke only (power loss). Grant is handled by `activatePoints()` reading current registry power.
- **Storage**: `mapping(uint256 gardenId => address[] strategies)` — which CV strategies to sync per garden
- **Admin**: `setConvictionStrategies(uint256 gardenId, address[] calldata strategies)` — callable by garden operator
- **Failure mode**: Best-effort, non-blocking. Sync failure MUST NOT revert the role revocation.
- **Events**: `ConvictionSyncTriggered(gardenId, member, strategy)` and `ConvictionSyncFailed(gardenId, member, strategy, reason)`

```solidity
// In Green Goods HatsModule (packages/contracts/src/modules/Hats.sol)

/// @notice CV strategies to sync per garden on role revocation
mapping(uint256 => address[]) public gardenConvictionStrategies;

/// @notice Set which CV strategies to sync for a garden
/// @param _gardenId The garden token ID
/// @param _strategies Array of CVStrategy addresses to sync
function setConvictionStrategies(uint256 _gardenId, address[] calldata _strategies) external {
    // Only garden operator can configure
    require(isOperatorOf(msg.sender, _gardenId), "Not operator");
    gardenConvictionStrategies[_gardenId] = _strategies;
}

/// @notice Sync conviction voting power after hat revocation (best-effort)
/// @param _member The member whose hat was revoked
/// @param _gardenId The garden this role belongs to
function _syncConvictionPower(address _member, uint256 _gardenId) internal {
    address[] storage strategies = gardenConvictionStrategies[_gardenId];
    for (uint256 i = 0; i < strategies.length; i++) {
        try ICVSyncPower(strategies[i]).syncPower(_member) {
            emit ConvictionSyncTriggered(_gardenId, _member, strategies[i]);
        } catch (bytes memory reason) {
            emit ConvictionSyncFailed(_gardenId, _member, strategies[i], reason);
        }
    }
}
```
...
```
Pool Creation:
    CommunityPoolFacet.createPool(params) where params.votingPowerRegistry = NFTPowerRegistryAddress
    → CVStrategy initialized with votingPowerRegistry = NFTPowerRegistry
...
Member Joins (1 tx via multicall):
    ...
    → activatePoints() reads votingPowerRegistry.getMemberPower(member)
    → Sets member power from NFT balance, not staked tokens

Member Votes:
    CVAllocationFacet.allocate(proposalSupports)
    → Uses member power from votingPowerRegistry
...
// PROPOSED: Use votingPowerRegistry if set, fallback to registryCommunity
uint256 power = address(votingPowerRegistry) != address(0)
    ? votingPowerRegistry.getMemberPower(msg.sender, address(this))
...
Add a new point system that reads power directly from the votingPowerRegistry:
...
enum PointSystem {
    ...
    Custom      // NEW — uses votingPowerRegistry.getMemberPower() result as-is, no transformation
}
```

When `PointSystem.Custom` is selected, `PowerManagementUtils` returns `votingPowerRegistry.getMemberPower(member, strategy)` directly. No point calculation logic is applied — the registry is the single source of truth for power in this mode.
...
const poolParams = {
    ...
    votingPowerRegistry: nftPowerRegistryAddress,  // NEW
...
const streamingNFTPool = {
    ...
    votingPowerRegistry: nftPowerRegistryAddress,
    superfluidToken: USDCx_ADDRESS,
    streamingRatePerSecond: ethers.parseEther("0.001"),
    // Members vote with NFT power, streaming proposals share the stream proportionally (no single "winner")
};
```
...
| 1.4 | `CVStrategyBaseFacet.sol` | Add `votingPowerRegistry` storage slot (consume 1 __gap slot) |
| 1.5 | `CVStrategyDiamondInit.sol` | Initialize `votingPowerRegistry` from V0_4 params |
...
**Goal**: Real-time conviction updates on NFT changes

| Task | File(s) | Description |
|------|---------|-------------|
| 2.1 | `CVSyncPowerFacet.sol` (NEW) | Implement `syncPower()` and `batchSyncPower()` |
| 2.2 | `CVPowerFacet.sol` | Modify `activatePoints()` to use `votingPowerRegistry` for `PointSystem.Custom` |
| 2.3 | `CVPowerFacet.sol` | Modify `increasePower()`/`decreasePower()` for NFT path |
| 2.4 | `CommunityPowerFacet.sol` | Add `getMemberBalancePower()` for balance-based weight |
| 2.5 | Integration tests | `CVSyncPowerFacet.t.sol`, power registry tests |
...
| 3.4 | `CVAdminFacet.sol` | Add `setPoolParams()` update for votingPowerRegistry |
| 3.5 | Diamond cut scripts | Script to add `CVSyncPowerFacet` to existing strategies. **MUST be single TX**: Add CVSyncPowerFacet + Replace CVPowerFacet + diamondInit for votingPowerRegistry storage |
...
`syncPower()` modifies state (proposals, totalPointsActivated) and calls external contracts (votingPowerRegistry). Use checks-effects-interactions pattern. All state updates happen before any external calls.

### 6.4 Storage Slot Safety

New `votingPowerRegistry` slot at position 132 uses the first __gap slot. Remaining gap: 46 slots. No collision with existing storage.
...
### 6.6  Alternative VotingPowerRegistry should be whitelisted

Having VotingPowerRegistry open for any implementation can lead to malicious pool creator can alter the voting power on they favor
...
Implementation (permit-first strategy):
1. Frontend detects if user is registered in community
2. If not registered, bundle: `permit` + `registerMember()` + `activatePoints()` + `allocate(support)`
3. If already registered, just `allocate(support)`
4. Fallback: if token lacks ERC-2612 permit support, route to two-step UX (approve tx, then bundled action tx)
...
- Shows "NFT Power" toggle that enables `votingPowerRegistry` configuration
...
| ICVSyncPowerFacet | `packages/contracts/src/interfaces/ICVSyncPowerFacet.sol` | Sync power interface |
| ConvictionSync test | `packages/contracts/test/integration/ConvictionSync.t.sol` | Integration test |

> **Deleted from green-goods** (Feb 2026): `HypercertSignalPool.sol`, `IHypercertSignalPool.sol`,
> and their tests. These are canonically in the gardens repo.
...
1. **Storage slot for votingPowerRegistry**: Confirm slot 132 (first __gap slot) is safe. Any planned additions to the streaming branch that would conflict? 
   *I already added the votingPowerRegistry field at a safe slot in the streaming branch*
...
2. **PointSystem.Balance enum**: Adding a 5th enum value — does this break existing deployed strategies that use the current enum?
   *Nope, however it would be more modular if its switched to custom that take the result of getMemberPowerInStrategy as is*
...
3. **CommunityPowerFacet changes**: Should `getMemberPowerInStrategy()` automatically fallback to `balanceOf()` when `stakedAmount == 0`? Or keep it explicit?
   *Where will exist the mapping logic between NFT and VP ? `balanceOf` check should only exist in the custom `VotingPowerRegistry`*
...
4. **Diamond cut ordering**: When upgrading an existing community, should `CVSyncPowerFacet` be added before or after modifying `CVPowerFacet`? (Order matters for storage consistency.) 
   *As long as its done in the same TX I don't see any problem in cut ordering*
...
5. **NFTPowerRegistry deployment**: Should it be deployed once per chain (shared) or once per pool (isolated)? Shared is gas-efficient but less flexible.
   *Depends on the scope and how much it is statefull*

6. **Governance token optionality**: For pools with `votingPowerRegistry` set, should `registryCommunity.gardenToken()` still be required? Meeting suggested it could be purely NFT-based.

   I added a more generic field named `ercAddress` (naming can be improved) in the IVotingPowerRegistry to be able to access in the gardens contracts to the NFT/ERC20 address
