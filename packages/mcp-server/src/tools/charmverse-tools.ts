...
function checkApiKey(): void {
  if (!process.env.CHARMVERSE_API_KEY) {
    throw new Error(
      "CharmVerse API key not configured. The CharmVerse API is in early alpha. " +
      "To request access:\n" +
...
export const charmverseHandlers = {
  "get_space_info": async () => {
    ...
    const cached = cache.get<CharmVerseSpace>(cacheKey);
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error(
            "CharmVerse API endpoint not found. The API structure may have changed. " +
      ...
      const data = await response.json() as CharmVerseSpace;
  ...
  "list_pages": async ({ type, limit = 20 }: { type?: string; limit?: number }) => {
    checkApiKey();
  ...
  "get_members": async ({ role }: { role?: string } = {}) => {
    checkApiKey();
  ...
  "get_proposals": async ({ status }: { status?: string } = {}) => {
    checkApiKey();
  ...
  "search_charmverse": async ({ query, type }: { query: string; type?: string }) => {
    checkApiKey();
