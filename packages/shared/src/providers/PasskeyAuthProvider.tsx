...
const PasskeyAuthContext = createContext<PasskeyAuthContextType | undefined>(
	undefined,
);
...
export function PasskeyAuthProvider({
	...
	useEffect(() => {
		...
		const initialize = async () => {
			const savedAuthMode = localStorage.getItem(
					...
					if (!cancelled) {
						...
						setError(
							err instanceof Error
								? err
								: new Error("Failed to restore passkey"),
						);
	...
	const createPasskey = useCallback(async () => {
		...
		try {
			...
			try {
				...
				console.warn(
					"Failed to restore saved passkey session, creating a new one",
					restoreError,
				);
			}
			...
			const error =
				err instanceof Error
					? err
					: new Error("Failed to create passkey. Please try again.");
			setError(error);
			throw error;
		} finally {
			setIsAuthenticating(false);
	...
	return (
		<PasskeyAuthContext.Provider value={contextValue}>
			{children}
