...
import { Domain, type Address } from "../types/domain";
  ...
  persist(
    (set, get) => ({
      ...
      getParams: () => {
        ...
        // Compute bitmask from selected domains (bit N = Domain with value N)
        const domainMask = form.domains.reduce((mask, d) => mask | (1 << d), 0);
