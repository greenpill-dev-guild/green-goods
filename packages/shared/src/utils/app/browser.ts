...
export function canTriggerInstallPrompt(browserInfo: BrowserInfo): boolean {
  ...
  // Keep the native prompt on the same allowlist as install guidance:
  // Android Chrome only. Safari uses manual instructions, and non-Chrome
  // Android browsers are routed to Chrome even if they report partial support.
  return (
