 ...
 */
function isAutoGeneratedUsername(name: string | null | undefined): boolean {
  return !name || /^user_\d+$/.test(name);
}

/**
 * The username the active account chose for its passkey, or null. Auth keeps the last passkey
 * username after a switch to a wallet, so it only counts while the passkey account is active.
 */
export function chosenPasskeyUsername(
  authMode: AuthMode,
  userName: string | null | undefined
): string | null {
  if (authMode !== "passkey" || isAutoGeneratedUsername(userName)) return null;
