 ...
 *
 * Renders a nature-inspired gradient with a subtle dot-grid texture.
 * The gradient is selected based on a hash
 ...
 * ```
 */
export const GardenBannerFallback: React.FC<GardenBannerFallbackProps> = React.memo(
  ({ name, className }) => {
    const gradient = GARDEN_GRADIENTS[hashName(name || "Garden") % GARDEN_GRADIENTS.length];
