...
export async function getCommitmentDetail(
  ...
  const [contributors, assignments, evidence, claims, counterparts] = await Promise.all([
    ...
    rowsByIds("CommitmentClaimRequest", CLAIM_FIELDS, requestIds, reader),
    rowsByIds(
      "Commitment",
      COMMITMENT_FIELDS,
      [...counterIndexIds, ...(directCounterId ? [directCounterId] : [])],
      reader
    ),
  ]);
  const mappedCommitments = await mapCommitmentsWithCycleState(
    [commitmentRow, ...counterparts],
    reader
  );
  return {
 ...
 */
export async function getPoolClaimRequests(
  input: {
    chainId: number;
    poolId: bigint;
    state?: string;
 ...
 * beats one per row.
 */
export async function getFallbackConfirmationCandidates(
  input: {
    chainId: number;
    poolId?: bigint;
    garden?: Address;
  ...
  const commitments = (
    await mapCommitmentsWithCycleState(
      await queryRows(query, variables, "Commitment", "getFallbackCandidates", reader),
      reader
    )
  ).filter((row) => row.creationSeen);
  if (commitments.length === 0) return [];
 ...
 */
export async function getViewerConfirmedCommitmentIds(
  input: {
    chainId: number;
    viewer: Address;
  },
