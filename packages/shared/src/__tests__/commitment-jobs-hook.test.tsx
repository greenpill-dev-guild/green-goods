...
describe("useCommitmentJobs", () => {
  ...
  it("keeps sending and confirming distinct, which nothing else checks", async () => {
    ...
    await jobs().current.enqueue({ act: "sendForConfirmation", commitmentId: 9n });
    ...
    await jobs().current.enqueue({ act: "confirm", commitmentId: 9n });
    expect(mocks.addJob).toHaveBeenLastCalledWith(
      "confirmation",
  ...
  it("routes proof, work links and creation to their own kinds", async () => {
    await jobs().current.enqueue({
      act: "evidence",
      payload: { commitmentId: 9n, cid: "bafy", creditedContributors: [] },
    });
    expect(mocks.addJob.mock.calls.at(-1)?.[0]).toBe("evidence");

    await jobs().current.enqueue({
      act: "workLink",
      payload: { clientOperationId: "op-1", commitmentId: 9n, workUID: "0x1", requirementIndex: 0 },
    });
  ...
  it("refuses to queue anything for nobody", async () => {
    ...
    await expect(result.current.enqueue({ act: "confirm", commitmentId: 9n })).rejects.toThrow(
      /sign in/i
    );
