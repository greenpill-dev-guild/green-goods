...
/// @title ArbitrumGoodsTokenForkTest
/// @notice Fork tests for GoodsToken on Arbitrum (42161). Verifies supply management,
/// access control, burn mechanics, and staking integration with the Gardens V2 module.
/// @dev Inherits ForkTestBase for full-stack deployment. After deployFullStack(),
/// GoodsToken ownership is transferred to GardensModule (the minter). Tests that need
/// direct mint access deploy a separate standalone GoodsToken instance.
contract ArbitrumGoodsTokenForkTest is ForkTestBase {
    // ═══════════════════════════════════════════════════════════════════════════
    // Standalone Token Helper
    // ═══════════════════════════════════════════════════════════════════════════

    /// @notice Deploy a standalone GoodsToken where address(this) is owner.
    /// Used for tests that need direct mint/ownership control (not through GardensModule).
    ...
    /// @notice Deploy full stack, configure real Gardens V2, mint garden, verify GOODS token
    ...
    function testForkArbitrum_goodsToken_stakingIntegration() public {
        ...
        address garden = _mintTestGarden("Arb GOODS Staking Garden", 0x0F);
        ...
        if (community != address(0)) {
            // The community's gardenToken (staking token) should be our GOODS token
            // Community uses gardenToken() getter — which returns the GOODS ERC-20
