...
import { ENSRegistrar, NameNotAvailable, InvalidName, UnauthorizedCaller, InvalidCredentialId }
...
contract ENSRegistrarTest is Test {
    ...
    function testRegisterSubdomain() public {
        ...
        vm.mockCall(
            MOCK_ENS_REGISTRY,
            abi.encodeWithSignature("setSubnodeOwner(bytes32,bytes32,address)", GREENGOODS_BASE_NODE, label, address(registrar)),
            abi.encode()
        );
        vm.mockCall(
            MOCK_ENS_REGISTRY,
            abi.encodeWithSignature("setResolver(bytes32,address)"),
            abi.encode()
        );
        vm.mockCall(
            MOCK_ENS_RESOLVER,
            abi.encodeWithSignature("setAddr(bytes32,address)"),
            abi.encode()
        );
    ...
    function testAvailable() public {
        ...
        vm.mockCall(
            MOCK_ENS_REGISTRY,
            abi.encodeWithSignature("setSubnodeOwner(bytes32,bytes32,address)"),
            abi.encode()
        );
        vm.mockCall(
            MOCK_ENS_REGISTRY,
        ...
        // Register name
        vm.prank(gardenerAccount);
        registrar.register(name, gardenerAccount, bytes32(uint256(12345)));
    ...
    function testCannotRegisterTaken() public {
        ...
        registrar.register(name, anotherAccount, bytes32(uint256(67890)));
    }
