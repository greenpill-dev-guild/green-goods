...
contract FuzzTests is Test, ERC6551Helper {
    ...
    function testFuzz_GardenMintingWithRandomStrings(uint8 nameLength, uint8 descLength) public {
        ...
        address gardenAccount =
            gardenToken.mintGarden(address(mockToken), name, description, "Location", "Banner", gardeners, operators);
    ...
    function testFuzz_ArrayLengthValidation(uint8 gardenerCount) public {
        ...
        if (gardenerCount > 100) {
            ...
            gardenToken.mintGarden(address(mockToken), "Test", "Description", "Location", "Banner", gardeners, operators);
        } else {
            // Should succeed for arrays <= 100
            address gardenAccount = gardenToken.mintGarden(
                address(mockToken), "Test", "Description", "Location", "Banner", gardeners, operators
            );
