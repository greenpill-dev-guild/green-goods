...
contract GPNCampaignNFT is ERC721Enumerable{
    ...
    function mintAndDeploy(capitals, metadata, team) external returns(address){
    ...
    function initializeData(uint _id, uint _hypercertId, string[] calldata _capitals, address[] calldata _team, uint _startDate, uint _endDate) public {
        require(msg.sender == ownerOf(_id), "not owner");

        CertInfo memory c;
        c.hypercertId = _hypercertId;
        c.capitals = _capitals;
        c.team = _team;
    ...
    function tokenURI(uint id) public override view returns(string memory){
        ...
        if(traitData[id].initialized) {
            ...
            traits = string(abi.encodePacked('"attributes": [{"trait_type": "Hypercert ID", "value":"',
    ...
    function getTeamString(uint id)public view returns(string memory){
        ...
        string memory team = "";
        for(uint i = 0; i == teamArray.length; i++){
            team = string(abi.encodePacked(team,
                '{"trait_type": "Team Member ',
                i.toString(),
                '", "value": "',
                Strings.toHexString(teamArray[i]),
                '"},'
            ));
        }
        return(team);    
    ...
    function getCapitalsString(uint id)public view returns(string memory){
        ...
        for(uint i = 0; i == capitalsArray.length; i++){
