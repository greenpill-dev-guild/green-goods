...
/// @title CommitmentPoolingTerminalLib
/// @notice Deployed behavior library: cancellation, expiry, dispute, and dispute resolution.
/// @dev Module pause never blocks cancel, expire, or resolve — those are the wind-down path a
///      paused pool or module still needs; the shells carry that modifier decision.
///      Runs via DELEGATECALL from `CommitmentPoolingModule`.
library CommitmentPoolingTerminalLib {
        ...
        external
    {
        ...
        _releaseCommittedUnits(env, commitmentId, commitment);
        _decrementLiveCounts(pools, cycles, commitment);
        ...
        external
    {
        ...
        uint64 due = _effectiveDueDate(cycles, commitment);
        ...
        external
    {
        ...
        bool eligible = msg.sender == commitment.creator || msg.sender == commitment.counterparty
            || _isNamedConfirmer(commitmentConfirmers, commitmentId, msg.sender)
        ...
        external
    {
        ...
        ICommitmentPoolingModule.CommitmentState finalState = _applyDisputeResolution(
            env,
            pools,
            cycles,
            contributors,
        ...
        returns (ICommitmentPoolingModule.CommitmentState finalState)
    {
        if (resolution == ICommitmentPoolingModule.DisputeResolution.RestorePrevious) {
            ...
            if (wasExpired) _decrementLiveCounts(pools, cycles, commitment);
            return previous;
        }

        if (resolution == ICommitmentPoolingModule.DisputeResolution.Fulfilled) {
            _resolveDisputeFulfilled(
                env,
                pools,
                cycles,
                contributors,
        ...
        if (!wasExpired) _releaseCommittedUnits(env, commitmentId, commitment);
        _decrementLiveCounts(pools, cycles, commitment);
    ...
    function _releaseCommittedUnits(
        CommitmentPoolingCommonLib.Env memory env,
        uint256 commitmentId,
        ICommitmentPoolingModule.Commitment storage commitment
    )
        private
    {
        ...
        env.registry.releaseUnits(commitmentId, holder, commitment.targetUnits);
    }

    function _decrementLiveCounts(
        mapping(uint256 poolId => ICommitmentPoolingModule.Pool pool) storage pools,
        mapping(uint256 cycleId => ICommitmentPoolingModule.Cycle cycle) storage cycles,
        ICommitmentPoolingModule.Commitment storage commitment
    )
