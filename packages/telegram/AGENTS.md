...
│   ├── crypto.ts      # Wallet generation and key management
│   ├── rate-limiter.ts # Rate limiting for bot commands
│   └── verification.ts # Address verification utilities
└── __tests__/
...
Handles voice transcription and NLU parsing:
...
// Returns: { tasks: ParsedTask[], notes: string, date: string }
```

**Note**: Currently uses regex-based parsing. Whisper integration planned for future.

### Storage Service (`services/storage.ts`)

SQLite-based session management using `bun:sqlite`:
