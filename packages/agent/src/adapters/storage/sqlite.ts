 ...
 *
 * Implements StoragePort using Bun's native SQLite API.
 * Uses a fresh schema with platform-agnostic user identification.
 */
...
import type { Session, SessionStep } from "../../core/contracts/response";
import type {
  StoragePort,
  User,
  CreateUserInput,
...
export class SQLiteStorage implements StoragePort {
  ...
  private initSchema(): void {
    // Users table with platform-agnostic identification
    this.db.run(`
    ...
    this.db.run(`
      ...
      ON pending_works(gardenAddress)
    `);

    this.db.run(`
      CREATE INDEX IF NOT EXISTS idx_users_role_garden
      ON users(role, currentGarden) WHERE role = 'operator'
    `);
  }
  ...
  async getUser(platform: Platform, platformId: string): Promise<User | undefined> {
    ...
    return {
      ...
      currentGarden: result.currentGarden ?? undefined,
  ...
  async getOperatorForGarden(gardenAddress: string): Promise<User | undefined> {
    ...
    const { privateKey } = getPrivateKey(result.privateKey);
  ...
  async getSession(platform: Platform, platformId: string): Promise<Session | undefined> {
    ...
    if (row.draft) {
      try {
        ...
        console.warn(`Invalid session draft JSON for ${platform}:${platformId}`);
      }
    }
