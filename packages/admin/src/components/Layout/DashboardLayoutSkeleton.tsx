        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={`nav-item-${index}`} className="flex items-center px-3 py-2 rounded-md">
              <div
                className="h-5 w-5 rounded-md skeleton-shimmer"
                style={{ animationDelay: `${index * 0.1}s` }}
              />
              <div
                className="ml-3 h-4 flex-1 rounded-md skeleton-shimmer"
                style={{ animationDelay: `${index * 0.1}s` }}
              />
            </div>
          ))}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 px-6 border-b border-stroke-soft bg-bg-white flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="h-10 w-10 rounded-md skeleton-shimmer lg:hidden" />
            <div className="h-10 w-40 rounded-md skeleton-shimmer" />
          </div>
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            {Array.from({ length: 4 }).map((_, index) => (
              ...
              >
                <div
                  className="h-5 w-32 rounded-md skeleton-shimmer"
                  style={{ animationDelay: `${index * 0.15}s` }}
                />
                <div className="mt-4 space-y-3">
                  <div
                    className="h-4 w-full rounded-md skeleton-shimmer"
                    style={{ animationDelay: `${index * 0.15}s` }}
                  />
                  <div
                    className="h-4 w-2/3 rounded-md skeleton-shimmer"
