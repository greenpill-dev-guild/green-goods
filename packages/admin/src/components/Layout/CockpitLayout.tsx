...
import { CockpitGardenAccessState } from "./CockpitGardenAccessState";
...
export function CockpitLayout() {
  ...
  // Sheet orchestrator — manages pane-scoped sheets + canvas recession
  ...
  const { activePath, workspaceId, rawWorkspaceId } = useMemo(() => {
    ...
    return {
      activePath: shouldNormalizeDesktopProfile ? adminRoutes.work() : rawActivePath,
      workspaceId: shouldNormalizeDesktopProfile ? "work" : nextWorkspaceId,
      rawWorkspaceId: nextWorkspaceId,
    } as const;
  }, [isDesktop, location.pathname]);

  const isCoreWorkspace =
    activePath === "/work" || activePath === "/garden" || activePath === "/community";
  ...
  useEffect(() => {
    ...
    navigate(adminRoutes.work(), { replace: true });
  ...
  return (
    <FabProvider>
      <div data-workspace={workspaceId} className="flex h-full min-h-0 flex-col workspace-canvas">
        ...
        {/* ── Body 2: Canvas — Content Zone (Z2) ── */}
        <Canvas isReceded={orchestrator.isReceded} overlayRef={overlayRootRef}>
          <main
            id="main-content"
            tabIndex={-1}
            className="main-scroll-area h-full overflow-y-auto pb-24"
            style={{
