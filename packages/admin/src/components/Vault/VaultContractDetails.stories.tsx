...
// directly and reproduces the exact AdminCard + plain stacked-row layout. Treat it as a
...
function VaultContractDetailsHarness({ rows }: VaultContractDetailsHarnessProps) {
  ...
  return (
    <AdminCard density="none">
      <AdminCardHeader>
        <div>
          <AdminCardTitle>Where the Funds Are Held</AdminCardTitle>
          <p className="mt-1 body-sm text-text-sub">
            Open any address on the block explorer to check it yourself.
          </p>
        </div>
      </AdminCardHeader>
      <AdminCardBody>
        <dl className="divide-y divide-stroke-soft">
          {rows.map((row) => (
            <div key={row.href} className={rowClass}>
              <dt className="body-xs text-text-soft">{row.label}</dt>
              <dd>
...
const registryRow: ContractRow = {
  label: "Green Goods Vault Manager",
...
const meta: Meta<typeof VaultContractDetailsHarness> = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          "⚠ **Visual harness** — not the real `VaultContractDetails`. Renders the same always-visible contract-address reference (an `AdminCard` of plain label-over-address rows for the narrow endowment rail, each named for what it does for the steward) using plain props. The real component self-fetches via `useGardenVaults` and resolves each address through `EnsAddressText`, a chain that terminates in wagmi reads not seedable in Storybook.",
      },
    },
  },
  decorators: [
...
export const SingleVault: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "A garden with one endowment vault and no vault manager or Aave pool wired — only the vault row renders.",
      },
    },
  },
};
