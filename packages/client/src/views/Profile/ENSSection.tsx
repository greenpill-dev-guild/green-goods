...
import { isAutoGeneratedUsername } from "@green-goods/shared/utils/app/text";
...
export const ENSSection: React.FC<ENSSectionProps> = ({ primaryAddress }) => {
  ...
  const knownName = isAutoGeneratedUsername(userName) ? walletEnsName?.split(".")[0] : userName;
  ...
  // The wallet's ENS name resolves after the form mounts; take it while the
  // field is still untouched and empty, never over something typed.
  useEffect(() => {
    if (!suggestedSlug || slugForm.formState.isDirty || slugForm.getValues("slug")) return;
