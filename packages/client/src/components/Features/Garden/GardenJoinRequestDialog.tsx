...
export function GardenJoinRequestDialog({ gardenAddress }: { gardenAddress: Address }) {
  ...
  const accountName =
    greenGoodsName.data || ensName.data || (isAutoGeneratedUsername(userName) ? null : userName);
  const isResolvingName = !accountName && (greenGoodsName.isLoading || ensName.isLoading);
              ...
              {accountName ? (
                <p className="text-sm text-text-sub-600">
                  {formatMessage(
                    ...
                    { name: <strong className="text-text-strong-950">{accountName}</strong> }
                  )}
                </p>
