...
|--------------|------|-------|
| Diff, branch, PR, single feature | review | `.agents/skills/review/SKILL.md` |
| Repo health, dead code, drift | audit | `.agents/skills/audit/SKILL.md` |
| Design soundness, coupling, duplication | principles | `.agents/skills/principles/SKILL.md` |
| Placement, boundaries, structural refactor | architecture | `.agents/skills/architecture/SKILL.md` |
| Tokens, vocab, surface compliance | design | `.agents/skills/design/review-checklist.md` |
...
1. Make the edit.
2. **Verify the change in the same turn**: re-read the file, run the targeted test, or read the rendered DOM (Chrome MCP) for UI changes. No "should work" claims. (See AGENTS.md → *Verify Before Claiming Success*.)
3. Move to the next.
...
Hand off to the `ship` skill (`.agents/skills/ship/SKILL.md`). Run the full pipeline:
...
| Declare a fix verified without paste-able evidence | Violates *Verify Before Claiming Success* in AGENTS.md |
