...
**References**: `AGENTS.md` for invariants. `/audit` for prior findings. `/principles` for design-level context.
...
- REMOVE: Comments about AI generation (// generated by Codex/GPT/Copilot)
...
A second-opinion pass after Codex's 8 agents merge and validation passes. Codex sees the **merged result**, not partial agent worktrees, so it can catch regressions and cross-cutting misses that no single agent's lane covered. Skipped in `--dry-run` and `--no-codex`.

### Why Codex (not a 9th Codex agent)
...
### Triage rules (Codex reads, decides, acts)
...
- **Preserve invariants** — all AGENTS.md rules apply (hook boundary, barrel imports, Address types, single .env)
...
| Use Codex review to vet visual/UX cleanup | Codex is weak at visual judgment — that's a Codex job |
